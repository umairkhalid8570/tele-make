...
class TeleCreateDashboardWizard(models.TransientModel):
    ...
    def DuplicateDashBoard(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id(
            'tele_dashboard_editor.tele_duplicate_dashboard_wizard')
        action['context'] = {'dashboard_id': self.id}
        return action

    def tele_duplicate_record(self):
        '''this function creats record of tele_dashboard_editor.board and return dashboard action_id'''
        dashboard_id = self._context.get('dashboard_id')
        dup_dash = self.env['tele_dashboard_editor.board'].browse(dashboard_id).copy({'tele_dashboard_top_menu_id': self.tele_top_menu_id.id})
        context = {'tele_reload_menu': True, 'tele_menu_id': dup_dash.tele_dashboard_menu_id.id}
        dash_id = self.env['tele_dashboard_editor.board'].browse(dashboard_id)
        if not dup_dash.tele_dashboard_items_ids:
            for item in dash_id.tele_dashboard_items_ids:
                item.sudo().copy({'tele_dashboard_editor_board_id': dup_dash.id})
        return {
...
class TeleDeleteDashboardWizard(models.TransientModel):
    ...
    def DeleteDashBoard(self):
        '''this function returns acion id of tele.dashboard.duplicate.wizard'''
        action = self.env['ir.actions.act_window']._for_xml_id(
            'tele_dashboard_editor.tele_delete_dashboard_wizard')
        action['context'] = {'dashboard_id': self.id}
        return action
    ...
    def tele_delete_record(self):
        '''this function creats record of tele_dashboard_editor.board and return dashboard action_id'''
        dashboard_id = self._context.get('dashboard_id')
        self.env['tele_dashboard_editor.board'].browse(dashboard_id).unlink()
