...
class TeleListExport(http.Controller):
    ...
    def base(self, data):
        ...
        if tele_export_boolean:
            item = request.env['tele_dashboard_editor.item'].browse(int(item_id))
            ...
            if item._context.get('teleDateFilterSelection', False):
                ...
                tele_date_filter_selection = item.tele_dashboard_editor_board_id.tele_date_filter_selection
                item = item.with_context(teleDateFilterStartDate=item.tele_dashboard_editor_board_id.tele_dashboard_start_date)
                item = item.with_context(teleDateFilterEndDate=item.tele_dashboard_editor_board_id.tele_dashboard_end_date)
        ...
        for dataset in list_data['data_rows']:
            ...
            for tele_count, val in enumerate(dataset['data']):
                if isinstance(val, (float, int)):
                    if val >= 0:
                        try:
                            tele_precision = item.sudo().env.ref('tele_dashboard_editor.tele_dashboard_editor_precision').digits
                        except Exception as e:
                            tele_precision = 2
...
class TeleListExcelExport(TeleListExport, http.Controller):
    ...
    @http.route('/tele_dashboard_editor/export/list_xls', type='http', auth="user")
    @serialize_exception
    def index(self, data):
...
class TeleListCsvExport(TeleListExport, http.Controller):
    ...
    @http.route('/tele_dashboard_editor/export/list_csv', type='http', auth="user")
    @serialize_exception
    def index(self, data):
        return self.base(data)
