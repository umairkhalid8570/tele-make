...
from tele.applets.tele_dashboard_editor.common_lib.tele_date_filter_selections import tele_get_date, tele_convert_into_utc, \
...
class TeleDashboardEditorItems(models.Model):
    _name = 'tele_dashboard_editor.item'
    _description = 'Dashboard Editor items'
    ...
    tele_dashboard_board_template_id = fields.Many2one('tele_dashboard_editor.board_template', string="Dashboard Template")
    ...
    # ------------------------ Pro Fields --------------------
    tele_dashboard_editor_board_id = fields.Many2one('tele_dashboard_editor.board', string="Dashboard",
    ...
    tele_goal_lines = fields.One2many('tele_dashboard_editor.item_goal', 'tele_dashboard_item', string="Target Lines")
    ...
    tele_action_lines = fields.One2many('tele_dashboard_editor.item_action', 'tele_dashboard_item_id', string="Action Lines")
    ...
    def _tele_compute_precision_digits(self):
        for rec in self:
            try:
                precision_digits = self.sudo().env.ref('tele_dashboard_editor.tele_dashboard_editor_precision')
    ...
    # default = lambda self: self.sudo().env.ref('tele_dashboard_editor.tele_dashboard_editor_precision')
    ...
    def copy_data(self, default=None):
        ...
        return super(TeleDashboardEditorItems, self).copy_data(default)

    def copy(self, default=None):
        default = default or {}
        res = super(TeleDashboardEditorItems, self).copy(default)
    ...
    def create(self, values):
        ...
        return super(TeleDashboardEditorItems, self).create(
            values)
            ...
            if ((rec.tele_chart_data_count_type != "count" and tele_chart_measure_field) or (
                ...
                if (rec.tele_chart_groupby_type == 'date_type' and rec.tele_chart_date_groupby) or \
                    ...
                    if rec.tele_chart_groupby_type == 'date_type' and rec.tele_goal_enable and rec.tele_dashboard_item_type in [
                        ...
                        tele_goal_records = self.env['tele_dashboard_editor.item_goal'].read_group(
    ...
    def tele_get_previous_period_data(self, rec):
        ...
        if rec.tele_date_filter_selection == "l_none":
            date_filter_selection = rec.tele_dashboard_editor_board_id.tele_date_filter_selection
        else:
    ...
    def tele_get_start_end_date(self, model_name, tele_chart_groupby_relation_field, ttype, tele_chart_domain,
        ...
        #         self.env['tele_dashboard_editor.item_goal'].search([('tele_goal_date', '>=', model_field_start_date.strftime("%Y-%m-%d")),
        ...
        goal_model_start_date = \
            self.env['tele_dashboard_editor.item_goal'].search(tele_goal_domain, limit=1,
                                                            order='tele_goal_date ASC')['tele_goal_date']
        goal_model_end_date = \
            self.env['tele_dashboard_editor.item_goal'].search(tele_goal_domain, limit=1,
                                                            order='tele_goal_date DESC')['tele_goal_date']
...
class TeleDashboardItemsGoal(models.Model):
    _name = 'tele_dashboard_editor.item_goal'
    _description = 'Dashboard Editor Items Goal Lines'

    tele_goal_date = fields.Date(string="Date")
    tele_goal_value = fields.Float(string="Value")

    tele_dashboard_item = fields.Many2one('tele_dashboard_editor.item', string="Dashboard Item")


class TeleDashboardItemsActions(models.Model):
    _name = 'tele_dashboard_editor.item_action'
    _description = 'Dashboard Editor Items Action Lines'
    ...
    tele_dashboard_item_id = fields.Many2one('tele_dashboard_editor.item', string="Dashboard Item")
...
class TeleDashboardItemMultiplier(models.Model):
    ...
    _description = 'Dashboard Editor Items Multiplier Lines'
