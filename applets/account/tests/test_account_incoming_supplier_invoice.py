...
class TestAccountIncomingSupplierInvoice(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.env['ir.config_parameter'].sudo().set_param('mail.catchall.domain', 'test-company.tele.studio')

        cls.internal_user = cls.env['res.users'].create({
            'name': 'Internal User',
            'login': 'internal.user@test.tele.studio',
            'email': 'internal.user@test.tele.studio',
        })
