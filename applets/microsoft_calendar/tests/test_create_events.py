...
from tele.applets.microsoft_calendar.tests.common import TestCommon, mock_get_token
...
class TestCreateEvents(TestCommon):
    ...
    def test_create_simple_event_without_sync(self, mock_insert):
        """
        A Tele event is created when Outlook sync is not enabled.
        """
    ...
    def test_create_simple_event_without_email(self):
        ...
        with self.assertRaises(ValidationError):
            record._sync_tele2microsoft()

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Tele organizer calendar.
        """
        ...
        self.assert_tele_event(new_records, self.expected_tele_event_from_outlook)
        self.assertEqual(new_records.user_id, self.organizer_user)
        self.assertEqual(new_records.need_sync_m, False)

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_exists_in_tele(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Tele attendee calendar.
        There is a Tele user that matches with the organizer email address.
        """
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_does_not_exist_in_tele(
        self, mock_get_events
    ):
        """
        An event has been created in Outlook and synced in the Tele attendee calendar.
        no Tele user that matches with the organizer email address.
        """
        ...
        outlook_event = dict(self.simple_event_from_outlook_attendee, organizer={
            'emailAddress': {'address': "john.doe@tele.studio", 'name': "John Doe"},
        })
        expected_event = dict(self.expected_tele_event_from_outlook, user_id=False)
        ...
        new_records = (records - existing_records)
        self.assertEqual(len(new_records), 1)
        self.assert_tele_event(new_records, expected_event)

    @patch.object(MicrosoftCalendarService, 'insert')
    def test_create_recurrent_event_without_sync(self, mock_insert):
        """
        A Tele recurrent event is created when Outlook sync is not enabled.
        """
    ...
    def test_create_recurrent_event_with_sync(self, mock_insert, mock_get_events):
        """
        A Tele recurrent event is created when Outlook sync is enabled.
        """
    ...
    def test_create_recurrent_event_with_sync_by_another_user(self, mock_insert, mock_get_events):
        """
        A Tele recurrent event has been created and synced with Outlook by another user, but nothing
    ...
    def test_create_recurrent_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the Tele organizer calendar.
        """
        ...
        self.assert_tele_recurrence(new_recurrences, self.expected_tele_recurrency_from_outlook)
        for i, e in enumerate(sorted(new_events, key=lambda e: e.id)):
            self.assert_tele_event(e, self.expected_tele_recurrency_events_from_outlook[i])

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_recurrent_event_from_outlook_attendee_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the Tele attendee calendar.
        """
