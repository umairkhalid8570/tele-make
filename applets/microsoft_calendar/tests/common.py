...
from tele.applets.microsoft_calendar.models.microsoft_sync import MicrosoftSync
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a Tele event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Tele events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Tele events
        # -----------------------------------------------------------------------------------------

        self.expected_tele_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_tele_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_tele_recurrency_events_from_outlook = [
            {
    ...
    def assert_tele_event(self, tele_event, expected_values):
        """
        Assert that an Tele event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        tele_event_values = tele_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(tele_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, tele_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_tele_recurrence(self, tele_recurrence, expected_values):
        """
        Assert that an Tele recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        tele_recurrence_values = tele_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, tele_recurrence_values.get(k), msg=f"'{k}' mismatch")
