...
from tele.applets.microsoft_calendar.tests.common import TestCommon, patch_api
...
class TestMicrosoftEvent(TestCommon):
    ...
    def test_already_mapped_events(self):
        ...
        events = MicrosoftEvent([{
            "type": "singleInstance",
            "_tele_id": self.simple_event.id,
            "iCalUId": event_uid,
            "id": event_id,
        }])

        # act
        mapped = events._load_tele_ids_from_db(self.env)

        # assert
        self.assertEqual(len(mapped._events), 1)
        self.assertEqual(mapped._events[event_id]["_tele_id"], self.simple_event.id)
    ...
    def test_map_an_event_using_instance_id(self):
        """
        Here, the Tele event has an uid but the Outlook event has not.
        """
        # arrange
        event_id = self.simple_event.ms_organizer_event_id
        events = MicrosoftEvent([{
    ...
    def test_map_an_event_without_uid_using_instance_id(self):
        """
        Here, the Tele event has no uid but the Outlook event has one.
        """
    ...
    def test_map_an_event_without_uid_using_instance_id_2(self):
        """
        Here, both Tele event and Outlook event have no uid.
        """
    ...
    def test_map_a_recurrence_using_global_id(self):
        ...
        # assert
        self.assertEqual(len(mapped._events), 1)
        self.assertEqual(mapped._events[rec_id]["_tele_id"], self.recurrence.id)
    ...
    def test_try_to_map_mixed_of_single_events_and_recurrences(self):
        ...
        with self.assertRaises(TypeError):
            events._load_tele_ids_from_db(self.env)
    ...
    def test_match_event_only(self):
        ...
        # act
        matched = events.match_with_tele_events(self.env)

        # assert
        self.assertEqual(len(matched._events), 1)
        self.assertEqual(matched._events[event_id]["_tele_id"], self.simple_event.id)
    ...
    def test_match_recurrence_only(self):
        ...
        # assert
        self.assertEqual(len(matched._events), 1)
        self.assertEqual(matched._events[rec_id]["_tele_id"], self.recurrence.id)
