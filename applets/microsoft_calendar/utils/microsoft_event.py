...
class MicrosoftEvent(abc.Set):
    ...
    Inspired by Tele recordset, one instance can be a single Microsoft event or a
    ...
    def tele_id(self, env):
        return self._tele_id

    def _meta_tele_id(self, microsoft_guid):
        """Returns the Tele id stored in the Microsoft Event metadata.
    ...
    def tele_ids(self):
        """
        Get the list of Tele event ids already mapped with Outlook events (self)
        """
        return tuple(e._tele_id for e in self if e._tele_id)

    def _load_tele_ids_from_db(self, env, force_model=None):
        """
        Map Microsoft events to existing Tele events:
        1) extract unmapped events only,
        2) match Tele events and Outlook events which have both a ICalUId set,
        3) match remaining events,
        Returns the list of mapped events
        """
        mapped_events = [e.id for e in self if e._tele_id]
        ...
        tele_events = model_env.with_context(active_test=False).search([
            '|',
            ('ms_universal_event_id', "in", unmapped_events.uids),
            ('ms_organizer_event_id', "in", unmapped_events.ids)
        ]).with_env(env)

        # 1. try to match unmapped events with Tele events using their iCalUId
        unmapped_events_with_uids = unmapped_events.filter(lambda e: e.iCalUId)
        tele_events_with_uids = tele_events.filtered(lambda e: e.ms_universal_event_id)
        mapping = {e.ms_universal_event_id: e.id for e in tele_events_with_uids}

        for ms_event in unmapped_events_with_uids:
            tele_id = mapping.get(ms_event.iCalUId)
            if tele_id:
                ms_event._events[ms_event.id]['_tele_id'] = tele_id
                mapped_events.append(ms_event.id)

        # 2. try to match unmapped events with Tele events using their id
        unmapped_events = self.filter(lambda e: e.id not in mapped_events)
        mapping = {e.ms_organizer_event_id: e for e in tele_events}

        for ms_event in unmapped_events:
            tele_event = mapping.get(ms_event.id)
            if tele_event:
                ms_event._events[ms_event.id]['_tele_id'] = tele_event.id
                mapped_events.append(ms_event.id)

                # don't forget to also set the global event ID on the Tele event to ease
    ...
    def owner_id(self, env):
        ...
        1) the current Tele user is the organizer of the event according to Outlook event, so return his id.
        2) the current Tele user is NOT the organizer and:
           2.1) we are able to find a Tele user using the Outlook event organizer email address and we use his id,
           2.2) we are NOT able to find a Tele user matching the organizer email address and we return False, meaning
                that no Tele user will be able to modify this event. All modifications will be done from Outlook.
        """
        if self.isOrganizer:
            return env.user.id
        if self.organizer.get('emailAddress') and self.organizer.get('emailAddress').get('address'):
            # Warning: In Microsoft: 1 email = 1 user; but in Tele several users might have the same email
    ...
    def match_with_tele_events(self, env) -> 'MicrosoftEvent':
        """
        Match Outlook events (self) with existing Tele events, and return the list of matched events
        """
        ...
        mapped_recurrences = recurrence_candidates._load_tele_ids_from_db(env, force_model=env["calendar.recurrence"])

        # then, try to match events
        events_candidates = (self - mapped_recurrences).filter(lambda x: not x.is_recurrence())
        mapped_events = events_candidates._load_tele_ids_from_db(env)
