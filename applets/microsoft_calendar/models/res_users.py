...
import logging
import requests
from tele.applets.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
...
from tele.applets.microsoft_account.models.microsoft_service import DEFAULT_MICROSOFT_TOKEN_ENDPOINT
from tele.applets.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class User(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> Tele
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2tele(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # Tele -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_tele2microsoft()
        ...
        (events - synced_events)._sync_tele2microsoft()
