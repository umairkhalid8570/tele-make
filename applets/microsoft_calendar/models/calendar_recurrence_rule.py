...
class RecurrenceRule(models.Model):
    ...
    def _inverse_rrule(self):
        ...
        # modified in Tele but computed from other fields).
    ...
    def _write_from_microsoft(self, microsoft_event, vals):
        ...
        # event_tz is written on event in Microsoft but on recurrence in Tele
        vals['event_tz'] = microsoft_event.start.get('timeZone')
        super()._write_from_microsoft(microsoft_event, vals)
        new_event_values = self.env["calendar.event"]._microsoft_to_tele_values(microsoft_event)
    ...
    @api.model
    def _microsoft_to_tele_values(self, microsoft_recurrence, default_reminders=(), default_values=None, with_ids=False):
