...
from tele.tools import is_html_empty, email_normalize
...
class Meeting(models.Model):
    ...
    @api.model
    def _microsoft_to_tele_values(self, microsoft_event, default_reminders=(), default_values=None, with_ids=False):
        ...
        commands_attendee, commands_partner = self._tele_attendee_commands_m(microsoft_event)
        ...
        alarm_commands = self._tele_reminders_commands_m(microsoft_event)
        if alarm_commands:
    ...
    @api.model
    def _microsoft_to_tele_recurrence_values(self, microsoft_event, default_values=None):
        timeZone_start = pytz.timezone(microsoft_event.start.get('timeZone'))
        timeZone_stop = pytz.timezone(microsoft_event.end.get('timeZone'))
        start = parse(microsoft_event.start.get('dateTime')).astimezone(timeZone_start).replace(tzinfo=None)
        if microsoft_event.isAllDay:
    ...
    @api.model
    def _tele_attendee_commands_m(self, microsoft_event):
        commands_attendee = []
        commands_partner = []
        ...
        if microsoft_event.match_with_tele_events(self.env):
            existing_attendees = self.env['calendar.attendee'].search([
                ('event_id', '=', microsoft_event.tele_id(self.env)),
                ('email', 'in', emails)])
        ...
        for tele_attendee in attendees_by_emails.values():
            # Remove old attendees
            if tele_attendee.email not in emails:
                commands_attendee += [(2, tele_attendee.id)]
                commands_partner += [(3, tele_attendee.partner_id.id)]
        return commands_attendee, commands_partner

    @api.model
    def _tele_reminders_commands_m(self, microsoft_event):
        reminders_commands = []
        if microsoft_event.isReminderOn:
            event_id = self.browse(microsoft_event.tele_id(self.env))
    ...
    def _ensure_attendees_have_email(self):
        ...
        if invalid_event_ids:
            ...
            raise ValidationError(_("For a correct synchronization between Tele and Outlook Calendar, "
    ...
    def _cancel_microsoft(self):
        ...
        There are 2 cases:
          1) the organizer is an Tele user: he's the only one able to delete the Tele event. Attendees can just decline.
          2) the organizer is NOT an Tele user: any attendee should remove the Tele event.
        """
        user = self.env.user
