...
QUnit.module('fields', {
    ...
    QUnit.test('Sms widgets are correctly rendered', async function (assert) {
        ...
        // GSM-7
        await testUtils.fields.editAndTrigger(form.$('.o_input'), "Hello from Tele", 'input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 characters, fits in 1 SMS (GSM7) ',
            'Should be "15 characters, fits in 1 SMS (GSM7) " for "Hello from Tele"');
        // GSM-7 with \n => this one count as 2 characters
        form.$('.o_input').val("Hello from Tele\n").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '17 characters, fits in 1 SMS (GSM7) ',
            'Should be "17 characters, fits in 1 SMS (GSM7) " for "Hello from Tele\\n"');
        // Unicode => ê
        form.$('.o_input').val("Hêllo from Tele").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 characters, fits in 1 SMS (UNICODE) ',
            'Should be "15 characters, fits in 1 SMS (UNICODE) " for "Hêllo from Tele"');
        // GSM-7 with 160c
