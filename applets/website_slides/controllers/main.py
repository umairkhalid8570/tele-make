...
class WebsiteSlides(WebsiteProfile):
    ...
    def slides_channel_home(self, **post):
        ...
        achievements = request.env['recognition.badge.user'].sudo().search([('badge_id.is_published', '=', True)], limit=5)
        if request.env.user._is_public():
            challenges = None
            challenges_done = None
        else:
            challenges = request.env['recognition.challenge'].sudo().search([
                ('challenge_category', '=', 'slides'),
                ('reward_id.is_published', '=', True)
            ], order='id asc', limit=5)
            challenges_done = request.env['recognition.badge.user'].sudo().search([
