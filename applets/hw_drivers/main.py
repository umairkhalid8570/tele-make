...
class Manager(Thread):
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to Tele database
        """
        server = helpers.get_tele_server_url()
        if server:
            subject = helpers.read_file_first_line('tele-subject.conf')
            if subject:
            ...
            _logger.warning('Tele server not set')

    def run(self):
        """
        Thread that will load interfaces and drivers and contact the tele server with the updates
        """
