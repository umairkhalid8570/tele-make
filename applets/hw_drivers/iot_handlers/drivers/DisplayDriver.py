...
class DisplayDriver(Driver):
    ...
    def load_url(self):
        url = None
        if helpers.get_tele_server_url():
            # disable certifiacte verification
            urllib3.disable_warnings()
            http = urllib3.PoolManager(cert_reqs='CERT_NONE')
            try:
                response = http.request('GET', "%s/iot/box/%s/display_url" % (helpers.get_tele_server_url(), helpers.get_mac_address()))
