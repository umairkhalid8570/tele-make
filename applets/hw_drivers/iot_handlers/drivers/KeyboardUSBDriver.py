...
from tele.applets.hw_drivers.controllers.proxy import proxy_drivers
...
class KeyboardUSBDriver(Driver):
        ...
        Args:
            new_layout (dict): A dict containing two keys:
                ...
                - variant (str): An optional key to represent the variant of the
                                 selected layout
        """
        file_path = Path.home() / 'tele-keyboard-layouts.conf'
        if file_path.exists():
            data = json.loads(file_path.read_text())
        else:
            data = {}
        data[self.device_identifier] = layout
        helpers.write_file('tele-keyboard-layouts.conf', json.dumps(data))
    ...
    def _is_scanner(self):
        ...
        file_path = Path.home() / 'tele-keyboard-is-scanner.conf'
        if file_path.exists():
            data = json.loads(file_path.read_text())
    ...
    def _keyboard_input(self, scancode):
        ...
        pressed key represented by its scancode to the connected Tele instance.
    ...
    def _save_is_scanner(self, data):
        ...
        helpers.write_file('tele-keyboard-is-scanner.conf', json.dumps(data))
