...
describe('urlRegex', () => {
    ...
    it('should match 1234-abc.runbot007.tele.studio/web#id=3&menu_id=221', () => {
        const url = '1234-abc.runbot007.tele.studio/web#id=3&menu_id=221';
        const text = `abc ${url} abc`;
        const match = text.match(URL_REGEX);
        chai.expect(match[0]).to.be.equal(url);
    });
    it('should match https://1234-abc.runbot007.tele.studio/web#id=3&menu_id=221', () => {
        const url = 'https://1234-abc.runbot007.tele.studio/web#id=3&menu_id=221';
        const text = `abc ${url} abc`;
        const match = text.match(URL_REGEX);
        chai.expect(match[0]).to.be.equal(url);
    });
