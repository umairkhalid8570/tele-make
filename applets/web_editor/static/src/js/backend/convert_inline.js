/** @tele-module alias=web_editor.convertInline */
'use strict';

import FieldHtml from 'web_editor.field.html';
import { isBlock, rgbToHex } from '../../../lib/tele-editor/src/utils/utils';
...
function toInline($editable, cssRules, $iframe) {
    $editable.removeClass('tele-editor-editable');
    const editable = $editable.get(0);
    ...
    $editable.addClass('tele-editor-editable');
}
/**
...
function fontToImg($editable) {
    const editable = $editable.get(0);
    const fonts = tele.__DEBUG__.services["wysiwyg.fonts"];
