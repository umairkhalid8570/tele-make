...
QUnit.test('rendering with TeleBot has a request (default)', async function (assert) {
    assert.expect(4);
    ...
    assert.ok(
        ...
        "should display a notification counter next to the messaging menu for TeleBot request"
    );
    assert.strictEqual(
        document.querySelector('.o_MessagingMenu_counter').textContent,
        "1",
    ...
    assert.strictEqual(
        ...
        'TeleBot has a request',
        "notification should display that TeleBot has a request"
    );
});

QUnit.test('rendering without TeleBot has a request (denied)', async function (assert) {
    assert.expect(2);
...
QUnit.test('rendering without TeleBot has a request (accepted)', async function (assert) {
    assert.expect(2);
