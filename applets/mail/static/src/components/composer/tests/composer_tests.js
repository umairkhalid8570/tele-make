...
QUnit.test('composer text input placeholder should contain correspondent name when thread has exactly one correspondent', async function (assert) {
    ...
    this.data['res.partner'].records.push({ id: 7, name: 'Tele Demo' });
    this.data['mail.channel'].records.push({
        channel_type: 'chat',
        id: 20,
    ...
    assert.strictEqual(
        document.querySelector(`.o_ComposerTextInput_textarea`).placeholder,
        "Message Tele Demo...",
        "should have 'Message Tele Demo...' as placeholder for composer text input when thread has exactly one correspondent"
    );
});
...
QUnit.test('display partner mention suggestions on typing "@"', async function (assert) {
    ...
    this.data['res.partner'].records.push({
        id: 11,
        email: "testpartner@tele.studio",
        name: "TestPartner",
    });
    this.data['res.partner'].records.push({
        id: 12,
        email: "testpartner2@tele.studio",
        name: "TestPartner2",
    });
