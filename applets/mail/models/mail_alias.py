...
from tele.tools import is_html_empty, remove_accents
...
class Alias(models.Model):
    """A Mail Alias is a mapping of an email address with a given Tele Document
       model. It is used by Tele's mail gateway when processing incoming emails
       ...
       created, it becomes immediately usable and Tele will accept email for it.
     """
    _name = 'mail.alias'
    ...
    alias_name = fields.Char('Alias Name', copy=False, help="The name of the email alias, e.g. 'jobs' if you want to catch emails for <jobs@example.tele.studio>")
    alias_model_id = fields.Many2one('ir.model', 'Aliased Model', required=True, ondelete="cascade",
                                     help="The model (Tele Document Kind) to which this alias "
    ...
    def name_get(self):
        """Return the mail alias display alias_name, including the implicit
           ...
           e.g. `jobs@mail.tele.studio` or `jobs` or 'New Alias'
        """
        res = []
        for record in self:
