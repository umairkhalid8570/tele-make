...
import re
from tele import api, models, Command
...
class RecurrenceRule(models.Model):
    ...
    def _write_from_google(self, gevent, vals):
        ...
        # event_tz is written on event in Google but on recurrence in Tele
        vals['event_tz'] = gevent.start.get('timeZone')
        super()._write_from_google(gevent, vals)

        base_event_time_fields = ['start', 'stop', 'allday']
        new_event_values = self.env["calendar.event"]._tele_values(gevent)
        ...
        for tele_attendee_email in set(existing_attendees.mapped('email')):
            # Remove old attendees. Sometimes, several partners have the same email.
            if email_normalize(tele_attendee_email) not in emails:
                attendees = existing_attendees.exists().filtered(lambda att: att.email == email_normalize(tele_attendee_email))
    ...
    def _create_from_google(self, gevents, vals_list):
        ...
        for gevent, vals in zip(gevents, vals_list):
            base_values = dict(
                self.env['calendar.event']._tele_values(gevent),  # FIXME default reminders
                need_sync=False,
            )
            # If we convert a single event into a recurrency on Google, we should reuse this event on Tele
    ...
    @api.model
    def _tele_values(self, google_recurrence, default_reminders=()):
        return {
