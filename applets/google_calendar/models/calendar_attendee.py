...
class Attendee(models.Model):
    ...
    def _send_mail_to_attendees(self, mail_template, force_send=False):
        """ Override
        If not synced with Google, let Tele in charge of sending emails
    ...
    def _sync_event(self):
        ...
        for user in other_events.mapped('user_id'):
            ...
            other_events.filtered(lambda ev: ev.user_id.id == user.id).with_user(user)._sync_tele2google(service)
        google_service = GoogleCalendarService(self.env['google.service'])
        (all_events - other_events)._sync_tele2google(google_service)
