...
from tele.applets.google_account.models.google_service import GoogleService
from tele.applets.google_calendar.models.res_users import User
...
class TestSyncTele2Google(TestSyncGoogle):
    ...
    def test_event_creation(self):
        ...
        event._sync_tele2google(self.google_service)
    ...
    def test_event_creation_perf(self):
        ...
        with self.assertQueryCount(__system__=1211):
            ...
            events._sync_tele2google(self.google_service)
    ...
    def test_event_allday_creation(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'organizer': {'email': 'telebot@example.com', 'self': True},
            'attendees': [{'email': 'telebot@example.com', 'responseStatus': 'accepted'}],
            'extendedProperties': {'shared': {'%s_tele_id' % self.env.cr.dbname: event.id}}
        })
    ...
    def test_recurrence(self):
        ...
        recurrence._sync_tele2google(self.google_service)
        self.assertGoogleEventInserted({
            'id': False,
            'start': {'date': '2020-01-15'},
            'end': {'date': '2020-01-16'},
            ...
            'extendedProperties': {'shared': {'%s_tele_id' % self.env.cr.dbname: recurrence.id}}
        })
    ...
    def test_event_added_to_recurrence(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'attendees': [{'email': 'telebot@example.com', 'responseStatus': 'accepted'}],
            'recurrence': ['RRULE:FREQ=WEEKLY;COUNT=2;BYDAY=WE'],
            'extendedProperties': {'shared': {'%s_tele_id' % self.env.cr.dbname: event.recurrence_id.id}}
        }, timeout=3)
    ...
    def test_send_update_do_request(self, mock_do_request):
        ...
        event.with_context(send_updates=True)._sync_tele2google(self.google_service)
    ...
    def test_not_send_update_do_request(self, mock_do_request):
        ...
        event.with_context(send_updates=False)._sync_tele2google(self.google_service)
        self.call_post_commit_hooks()
