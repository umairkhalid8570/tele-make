...
class Authenticate(http.Controller):
    ...
    def auth(self, **values):
        """
         Once authenticated this route renders the view that shows an app wants to access Tele.
    ...
    def auth_confirm(self, scope, friendlyname, redirect, info=None, do=None, **kw):
        """
        Called by the `app_auth` template. If the user decided to allow the app to access Tele, a temporary auth code
    ...
    def auth_access_token(self, auth_code='', **kw):
        ...
        scope = 'tele.plugin.' + auth_message.get('scope', '')
    ...
    def _get_auth_code_data(self, auth_code):
        ...
        signature = tele.tools.misc.hmac(request.env(su=True), 'mail_plugin', data).encode()
    ...
    def _generate_auth_code(self, scope, name):
        ...
        signature = tele.tools.misc.hmac(request.env(su=True), 'mail_plugin', auth_message).encode()
