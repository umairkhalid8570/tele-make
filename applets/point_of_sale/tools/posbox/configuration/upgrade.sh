...
create_partition () {
    ...
    PART_TELE_ROOT=$(fdisk -l | tail -n 1 | awk '{print $1}')
    START_OF_TELE_ROOT_PARTITION=$(fdisk -l | tail -n 1 | awk '{print $2}')
    END_OF_TELE_ROOT_PARTITION=$((START_OF_TELE_ROOT_PARTITION + 11714061)) # sectors to have a partition of ~5.6Go
    START_OF_UPGRADE_ROOT_PARTITION=$((END_OF_TELE_ROOT_PARTITION + 1)) # sectors to have a partition of ~7.0Go
    (echo 'p';                                  # print
     ...
     echo 'p';                                  #   primary
     echo '2';                                  #   number 2
     echo "${START_OF_TELE_ROOT_PARTITION}";    #   starting at previous offset
     echo "${END_OF_TELE_ROOT_PARTITION}";      #   ending at ~9.9Go
     echo 'n';                                  # create new partition
     echo 'p';                                  #   primary
     echo '3';                                  #   number 3
...
copy_raspios () {
    ...
    PART_TELE_ROOT=$(fdisk -l | tail -n 2 | awk 'NR==1 {print $1}')
    ...
    sed -ie "s/$(echo ${PART_TELE_ROOT} | sed -e 's/\//\\\//g')/$(echo ${PART_RASPIOS_ROOT} | sed -e 's/\//\\\//g')/g" raspios/etc/fstab
    ...
    wget -c 'https://nightly.tele.studio/master/iotbox/iotbox-latest.zip' -O raspios/iotbox-latest.zip
    wget -c 'https://nightly.tele.studio/master/iotbox/SHA1SUMS.txt' -O raspios/SHA1SUMS.txt
    cd raspios/
...
copy_iot () {
    ...
    # Modify startup
    mkdir -v tele
    mount -v "${PART_IOTBOX_ROOT}" tele
    cp -v /home/pi/upgrade.sh tele/home/pi/
    NBR_LIGNE=$(sed -n -e '$=' tele/etc/rc.local)
    sed -ie "${NBR_LIGNE}"'i\. /home/pi/upgrade.sh; clean_local' tele/etc/rc.local
    find /home/pi/config -maxdepth 1 -type f ! -name ".*" -exec cp {} tele/home/pi/ \;
