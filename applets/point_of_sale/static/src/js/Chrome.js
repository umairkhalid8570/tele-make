tele.define('point_of_sale.Chrome', function(require) {
    ...
    const { teleExceptionTitleMap } = require("@web/core/errors/error_dialogs");
    ...
    class Chrome extends PopupControllerMixin(PosComponent) {
        ...
        _errorHandler(error, errorToHandle) {
            if (errorToHandle instanceof RPCError) {
                ...
                if (teleExceptionTitleMap.has(errorToHandle.exceptionName)) {
                    const title = teleExceptionTitleMap.get(errorToHandle.exceptionName).toString();
