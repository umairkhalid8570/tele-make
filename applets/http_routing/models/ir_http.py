...
import tele
from tele import api, models, registry, exceptions, tools, http
from tele.applets.base.models import ir_http
from tele.applets.base.models.ir_http import RequestUID
from tele.applets.base.models.qweb import QWebException
from tele.http import request
from tele.osv import expression
from tele.tools import config, ustr, pycompat
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if tele._geoip_resolver is not None:
            return
        geofile = config.get('geoip_database')
        try:
            tele._geoip_resolver = GeoIPResolver.open(geofile) or False
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if tele._geoip_resolver and request.httprequest.remote_addr:
                record = tele._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except tele.exceptions.MissingError:
