...
import datetime
import json
from tele.applets.tele_dashboard_builder.common_lib.tele_date_filter_selections import tele_get_date, tele_convert_into_local, \
...
class TeleDashboardBuilderBoard(models.Model):
    _name = 'tele_dashboard_builder.board'
    _description = 'Dashboard Builder'

    name = fields.Char(string="Dashboard Name", required=True, size=35)
    tele_dashboard_items_ids = fields.One2many('tele_dashboard_builder.item', 'tele_dashboard_builder_board_id',
    ...
    tele_dashboard_default_template = fields.Many2one('tele_dashboard_builder.board_template',
    ...
    tele_dashboard_menu_sequence = fields.Integer(string="Menu Sequence", default=10,
                                                help="Smallest sequence give high priority and Highest sequence give "
                                                     "low priority")
    tele_child_dashboard_ids = fields.One2many('tele_dashboard_builder.child_board', 'tele_dashboard_builder_id')
    tele_dashboard_defined_filters_ids = fields.One2many('tele_dashboard_builder.board_defined_filters',
                                                       'tele_dashboard_board_id',
                                                       string='Dashboard Predefined Filters')
    tele_dashboard_custom_filters_ids = fields.One2many('tele_dashboard_builder.board_custom_filters',
                                                      'tele_dashboard_board_id',
    ...
    def create(self, vals):
        record = super(TeleDashboardBuilderBoard, self).create(vals)
        ...
        if record.tele_dashboard_default_template and record.tele_dashboard_default_template.tele_item_count:
            ...
            for item_data in template_data:
                if record.tele_dashboard_default_template.tele_template_type == 'tele_custom':
                    dashboard_item = self.env['tele_dashboard_builder.item'].browse(int(item_data)).copy(
                        {'tele_dashboard_builder_board_id': record.id})
                    tele_gridstack_config[dashboard_item.id] = template_data[item_data]
                else:
                    dashboard_item = self.env.ref(item_data['item_id']).copy({'tele_dashboard_builder_board_id': record.id})
    ...
    def write(self, vals):
        ...
        record = super(TeleDashboardBuilderBoard, self).write(vals)
        for rec in self:
            if 'tele_dashboard_menu_name' in vals:
                if self.env.ref('tele_dashboard_builder.tele_my_default_dashboard_board') and self.env.ref(
                        'tele_dashboard_builder.tele_my_default_dashboard_board').sudo().id == rec.id:
                    if self.env.ref('tele_dashboard_builder.board_menu_root', False):
                        self.env.ref('tele_dashboard_builder.board_menu_root').sudo().name = vals['tele_dashboard_menu_name']
                else:
                    rec.tele_dashboard_menu_id.sudo().name = vals['tele_dashboard_menu_name']
            if 'tele_dashboard_group_access' in vals:
                if self.env.ref('tele_dashboard_builder.tele_my_default_dashboard_board').id == rec.id:
                    if self.env.ref('tele_dashboard_builder.board_menu_root', False):
                        self.env.ref('tele_dashboard_builder.board_menu_root').groups_id = vals['tele_dashboard_group_access']
                else:
    ...
    def unlink(self):
        if self.env.ref('tele_dashboard_builder.tele_my_default_dashboard_board').id in self.ids:
        ...
        res = super(TeleDashboardBuilderBoard, self).unlink()
        return res

    def tele_get_grid_config(self):
        default_grid_id = self.env['tele_dashboard_builder.child_board'].search(
            [['id', 'in', self.tele_child_dashboard_ids.ids], ['company_id', '=', self.env.company.id],
             ['board_type', '=', 'default']])

        if not default_grid_id:
            default_grid_id = self.env['tele_dashboard_builder.child_board'].create({
                "tele_gridstack_config": self.tele_gridstack_config,
                "tele_dashboard_builder_id": self.id,
    ...
    def tele_fetch_dashboard_data(self, tele_dashboard_id, tele_item_domain=False):
        ...
        has_group_tele_dashboard_manager = self.env.user.has_group('tele_dashboard_builder.tele_dashboard_builder_group_manager')
        ...
        if self.env['tele_dashboard_builder.child_board'].search(
            ...
            selecred_rec = self.env['tele_dashboard_builder.child_board'].search(
            ...
            for rec in self.env['tele_dashboard_builder.child_board'].search_read(
        ...
        try:
            items = self.tele_dashboard_items_ids.search(
                [['tele_dashboard_builder_board_id', '=', tele_dashboard_id]] + tele_item_domain).ids
    ...
    def tele_fetch_item(self, item_list, tele_dashboard_id, params={}):
        ...
        items = {}
        item_model = self.env['tele_dashboard_builder.item']
    ...
    def tele_fetch_item_data(self, rec, params={}):
        ...
        """
        try:
            tele_precision = self.sudo().env.ref('tele_dashboard_builder.tele_dashboard_builder_precision')
        ...
        item = {
            ...
            'tele_dashboard_id': rec.tele_dashboard_builder_board_id.id,
    ...
    def tele_view_items_view(self):
        ...
        return {
            ...
            'res_model': 'tele_dashboard_builder.item',
            'view_mode': 'tree,form',
            'view_type': 'form',
            'views': [(False, 'tree'), (False, 'form')],
            'type': 'ir.actions.act_window',
            'domain': [('tele_dashboard_builder_board_id', '!=', False)],
            'search_view_id': self.env.ref('tele_dashboard_builder.tele_item_search_view').id,
            'context': {
                'search_default_tele_dashboard_builder_board_id': self.id,
                'group_by': 'tele_dashboard_builder_board_id',
            },
    ...
    def tele_export_item(self, item_id):
        return {
            'tele_file_format': 'tele_dashboard_builder_item_export',
    ...
    def tele_export_item_data(self, rec):
        ...
        val = str(rec.id)
        selecred_rec = self.env['tele_dashboard_builder.child_board'].search(
            [['id', 'in', rec.tele_dashboard_builder_board_id.tele_child_dashboard_ids.ids], ['tele_active', '=', True],
             ['company_id', '=', self.env.company.id]], limit=1)
        if rec.tele_dashboard_builder_board_id.tele_gridstack_config:
            keys_data = json.loads(rec.tele_dashboard_builder_board_id.tele_gridstack_config)
        elif selecred_rec:
            keys_data = json.loads(selecred_rec.tele_gridstack_config)
        elif rec.tele_dashboard_builder_board_id.tele_child_dashboard_ids[0].tele_gridstack_config:
            keys_data = json.loads(rec.tele_dashboard_builder_board_id.tele_child_dashboard_ids[0].tele_gridstack_config)
        else:
    ...
    def tele_open_import(self, **kwargs):
        action = self.env['ir.actions.act_window']._for_xml_id('tele_dashboard_builder.tele_import_dashboard_action')
        return action

    def tele_open_setting(self, **kwargs):
        action = self.env['ir.actions.act_window']._for_xml_id('tele_dashboard_builder.board_form_tree_action_window')
    ...
    def tele_delete_dashboard(self):
        if str(self.id) in self.tele_dashboard_default_template:
            ...
            return {
                ...
                'res_model': 'tele_deshboard_builder.board',
    ...
    def tele_import_item(self, dashboard_id, **kwargs):
        ...
        if 'tele_file_format' in tele_dashboard_file_read and tele_dashboard_file_read[
            ...
            raise ValidationError(_("Current Json File is not properly formatted according to Dashboard Builder Model."))

        item['tele_dashboard_builder_board_id'] = int(dashboard_id)
    ...
    def tele_dashboard_export(self, tele_dashboard_ids, **kwargs):
        ...
        for tele_dashboard_id in tele_dashboard_ids:
            ...
            tele_dashboard_export_data = {
                'tele_file_format': 'tele_dashboard_builder_export_file',
        ...
        for data in tele_dashboard_data:
            if not all(key in data for key in tele_dashboard_key):
                raise ValidationError(
                    _("Current Json File is not properly formatted according to Dashboard Builder Model."))
            ...
            if data['tele_item_data']:
                ...
                for item in data['tele_item_data']:
                    ...
                    item['tele_dashboard_builder_board_id'] = dashboard_id.id
    ...
    def tele_prepare_import_data_vals(self, data, menu_id):
        vals = {
            ...
            'tele_dashboard_top_menu_id': menu_id.id if menu_id else self.env.ref(
                "tele_dashboard_builder.board_menu_root").id,
            'tele_dashboard_active': True,
            'tele_gridstack_config': data['tele_gridstack_config'],
            'tele_dashboard_default_template': self.env.ref("tele_dashboard_builder.tele_blank").id,
    ...
    def tele_create_item(self, item):
        ...
        tele_item = self.env['tele_dashboard_builder.item'].create(item)
        ...
        if tele_goal_lines and len(tele_goal_lines) != 0:
            for line in tele_goal_lines:
                ...
                self.env['tele_dashboard_builder.item_goal'].create(line)
            ...
            for line in tele_action_lines:
                ...
                if line['tele_item_action_field']:
                    ...
                    if tele_record_id:
                        ...
                        self.env['tele_dashboard_builder.item_action'].create(line)
    ...
    def update_child_board(self, action, dashboard_id, data):
        ...
        if action == 'create':
            ...
            result = self.env['tele_dashboard_builder.child_board'].create(data)
            result = result.id
        elif action == 'update':
    ...
    def tele_prepare_dashboard_domain(self):
        pre_defined_filter_ids = self.env['tele_dashboard_builder.board_defined_filters'].search(
        ...
        for model_id in filter_model_ids:
            filter_ids = self.env['tele_dashboard_builder.board_defined_filters'].search(
            ...
            for rec in filter_ids:
                ...
                if data.get(rec.tele_model_id.model) and rec.tele_domain:
                    ...
                    data[rec.tele_model_id.model] = {
                        ...
                        'item_ids': self.env['tele_dashboard_builder.item'].search(
    ...
    def tele_prepare_dashboard_custom_domain(self):
        custom_filter_ids = self.env['tele_dashboard_builder.board_custom_filters'].search(
