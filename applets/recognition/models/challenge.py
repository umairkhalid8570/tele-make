...
from tele import api, models, fields, _, exceptions
...
class Challenge(models.Model):
    ...
    _name = 'recognition.challenge'
    _description = 'Recognition Challenge'
    ...
    user_ids = fields.Many2many('res.users', 'recognition_challenge_users_rel', string="Users", help="List of users participating to the challenge")
    ...
    invited_user_ids = fields.Many2many('res.users', 'recognition_invited_user_ids_rel', string="Suggest to users")

    line_ids = fields.One2many('recognition.challenge.line', 'challenge_id',
                                  string="Lines",
                                  help="List of goals that will be set",
                                  required=True, copy=True)

    reward_id = fields.Many2one('recognition.badge', string="For Every Succeeding User")
    reward_first_id = fields.Many2one('recognition.badge', string="For 1st user")
    reward_second_id = fields.Many2one('recognition.badge', string="For 2nd user")
    reward_third_id = fields.Many2one('recognition.badge', string="For 3rd user")
    ...
    challenge_category = fields.Selection([
        ...
        ('other', 'Settings / Recognition Tools'),
    ...
    def _get_report_template(self):
        template = self.env.ref('recognition.simple_report_template', raise_if_not_found=False)
    ...
    def write(self, vals):
        ...
        if vals.get('state') == 'inprogress':
            ...
            # resetting progress
            if self.env['recognition.goal'].search([('challenge_id', 'in', self.ids), ('state', '=', 'inprogress')], limit=1):
    ...
    def _cron_update(self, ids=False, commit=True):
        ...
        # write operations
        self = self.with_context(commit_recognition=commit)
    ...
    def _update_all(self):
        ...
        Goals = self.env['recognition.goal']
    ...
    def action_check(self):
        ...
        self.env['recognition.goal'].search([
    ...
    def _generate_goals_from_challenge(self):
        ...
        for challenge in self:
            ...
            if self.env.context.get('commit_recognition'):
                self.env.cr.commit()
    ...
    def _get_serialized_challenge_lines(self, user=(), restrict_goals=(), restrict_top=0):
        ...
        :param restrict_goals: compute only the results for this subset of
                               recognition.goal ids, if False retrieve every
        ...
        {
            'name': <recognition.goal.description name>,
            'description': <recognition.goal.description description>,
            ...
            'own_goal_id': <recognition.goal id where user_id == uid>,
            'goals': [
                {
                    'id': <recognition.goal id>,
                    'rank': <user ranking>,
                    'user_id': <res.users id>,
                    'name': <res.users name>,
                    'state': <recognition.goal state {draft,inprogress,reached,failed,canceled}>,
    ...
    def _check_challenge_reward(self, force=False):
        ...
        """
        commit = self.env.context.get('commit_recognition') and self.env.cr.commit
        ...
        for challenge in self:
            ...
            if challenge.reward_id and (challenge_ended or challenge.reward_realtime):
                ...
                reached_goals = self.env['recognition.goal'].read_group([
                    ('challenge_id', '=', challenge.id),
                    ('end_date', '=', end_date),
                    ('state', '=', 'reached')
                ...
                for reach_goals_user in reached_goals:
                    if reach_goals_user['user_id_count'] == len(challenge.line_ids):
                        ...
                        if challenge.reward_realtime:
                            badges = self.env['recognition.badge.user'].search_count([
                                ('challenge_id', '=', challenge.id),
    ...
    def _reward_user(self, user, badge):
        ...
        """
        return self.env['recognition.badge.user'].create({
            'user_id': user.id,
            'badge_id': badge.id,
            'challenge_id': self.id
        })._send_badge()
...
class ChallengeLine(models.Model):
    """Recognition challenge line

    Predefined goal for 'recognition_challenge'
    These are generic list of goals with only the target goal defined
    Should only be created for the recognition.challenge object
    """
    _name = 'recognition.challenge.line'
    _description = 'Recognition generic goal for challenge'
    _order = "sequence, id"

    challenge_id = fields.Many2one('recognition.challenge', string='Challenge', required=True, ondelete="cascade")
    definition_id = fields.Many2one('recognition.goal.definition', string='Goal Definition', required=True, ondelete="cascade")
