...
class KarmaRank(models.Model):
    ...
    def write(self, vals):
        if 'karma_min' in vals:
            previous_ranks = self.env['recognition.karma.rank'].search([], order="karma_min DESC").ids
        ...
        if 'karma_min' in vals:
            after_ranks = self.env['recognition.karma.rank'].search([], order="karma_min DESC").ids
