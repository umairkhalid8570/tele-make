...
class BadgeUser(models.Model):
    ...
    _name = 'recognition.badge.user'
    _description = 'Recognition User Badge'
    _order = "create_date desc"
    _rec_name = "badge_name"

    user_id = fields.Many2one('res.users', string="User", required=True, ondelete="cascade", index=True)
    sender_id = fields.Many2one('res.users', string="Sender", help="The user who has send the badge")
    badge_id = fields.Many2one('recognition.badge', string='Badge', required=True, ondelete="cascade", index=True)
    challenge_id = fields.Many2one('recognition.challenge', string='Challenge originating', help="If this badge was rewarded through a challenge")
    ...
    def _send_badge(self):
        ...
        """
        template = self.env.ref('recognition.email_template_badge_received')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            self.env['recognition.badge'].browse(vals['badge_id']).check_granting()
...
class RecognitionBadge(models.Model):
    ...
    _description = 'Recognition Badge'
    ...
    rule_auth_badge_ids = fields.Many2many(
        'recognition.badge', 'recognition_badge_rule_badge_rel', 'badge1_id', 'badge2_id',
    ...
    challenge_ids = fields.One2many('recognition.challenge', 'reward_id', string="Reward of Challenges")

    goal_definition_ids = fields.Many2many(
        'recognition.goal.definition', 'badge_unlocked_definition_rel',
    ...
    owner_ids = fields.One2many(
        'recognition.badge.user', 'badge_id',
    ...
    def _get_owners_info(self):
        ...
        badge_alias = query.join("res_users", "id", "recognition_badge_user", "user_id", "badges")
    ...
    def _can_grant_badge(self):
        ...
        if self.rule_auth == 'nobody':
            ...
            all_user_badges = self.env['recognition.badge.user'].search([('user_id', '=', self.env.uid)]).mapped('badge_id')
