...
class KarmaTracking(models.Model):
    _name = 'recognition.karma.tracking'
...
SELECT user_id,
(
    SELECT old_value from recognition_karma_tracking old_tracking
    WHERE old_tracking.user_id = recognition_karma_tracking.user_id
    ...
    SELECT new_value from recognition_karma_tracking new_tracking
    WHERE new_tracking.user_id = recognition_karma_tracking.user_id
        AND tracking_date::timestamp BETWEEN %(from_date)s AND %(to_date)s
        AND consolidated IS NOT TRUE
        ORDER BY tracking_date DESC LIMIT 1
)
FROM recognition_karma_tracking
