...
class Users(models.Model):
    ...
    karma_tracking_ids = fields.One2many('recognition.karma.tracking', 'user_id', string='Karma Changes', groups="base.group_system")
    badge_ids = fields.One2many('recognition.badge.user', 'user_id', string='Badges', copy=False)
    ...
    rank_id = fields.Many2one('recognition.karma.rank', 'Rank', index=False)
    next_rank_id = fields.Many2one('recognition.karma.rank', 'Next Rank', index=False)
    ...
    def _get_user_badge_level(self):
        ...
        self.env.cr.execute("""
            ...
            FROM recognition_badge_user bu, recognition_badge b
    ...
    def create(self, values_list):
        ...
        if karma_trackings:
            self.env['recognition.karma.tracking'].sudo().create(karma_trackings)
...
FROM (
    ...
    FROM (
        ...
        LEFT JOIN "recognition_karma_tracking" as "tracking"
...
WHERE sub.user_id IN %%s""" % {
    ...
    def _rank_changed(self):
        ...
        template = self.env.ref('recognition.mail_template_data_new_rank_reached', raise_if_not_found=False)
        if template:
            for u in self:
    ...
    def _recompute_rank(self):
        ...
        ranks = [{'rank': rank, 'karma_min': rank.karma_min} for rank in
                 self.env['recognition.karma.rank'].search([], order="karma_min DESC")]
    ...
    def _get_next_rank(self):
        ...
        if self.next_rank_id:
            ...
            return self.env['recognition.karma.rank'].search([], order="karma_min ASC", limit=1)
        else:
            return self.env['recognition.karma.rank']

    def get_recognition_redirection_data(self):
        """
