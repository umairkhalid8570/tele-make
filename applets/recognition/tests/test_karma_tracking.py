...
from tele import exceptions, fields
...
class TestKarmaTrackingCommon(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.env['recognition.karma.tracking'].search([]).unlink()
    ...
    def _create_trackings(cls, user, karma, steps, track_date, days_delta=1):
        ...
        for step in range(steps):
            ...
            cls.env['recognition.karma.tracking'].create([{
                'user_id': user.id,
                'old_value': old_value,
                'new_value': new_value,
                'consolidated': False,
    ...
    def test_consolidation_cron(self):
        self.patcher = patch('tele.applets.recognition.models.recognition_karma_tracking.fields.Date', wraps=fields.Date)
        self.mock_datetime = self.patcher.start()
        self.mock_datetime.today.return_value = date(self.test_date.year, self.test_date.month + 1, self.test_date.day)

        self._create_trackings(self.test_user, 20, 2, self.test_date, days_delta=30)
        self._create_trackings(self.test_user_2, 10, 20, self.test_date, days_delta=2)
        self.env['recognition.karma.tracking']._consolidate_last_month()
        consolidated = self.env['recognition.karma.tracking'].search([
            ('user_id', 'in', (self.test_user | self.test_user_2).ids),
            ('consolidated', '=', True),
            ('tracking_date', '=', self.test_date)
        ])
        self.assertEqual(len(consolidated), 2)
        unconsolidated = self.env['recognition.karma.tracking'].search([
            ('user_id', 'in', (self.test_user | self.test_user_2).ids),
            ('consolidated', '=', False),
        ])
    ...
    def test_consolidation_monthly(self):
        Tracking = self.env['recognition.karma.tracking']
    ...
    def test_user_as_system_manager(self):
        self.test_user.write({'groups_id': [
            (4, self.env.ref('base.group_partner_manager').id),
            (4, self.env.ref('base.group_system_manager').id)
        ]})
        ...
        trackings = self.env['recognition.karma.tracking'].sudo().search([('user_id', '=', user.id)])
...
class TestComputeRankCommon(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        patch_email = patch('tele.applets.mail.models.mail_template.MailTemplate.send_mail', _patched_send_mail)
        patch_email.start()
        ...
        cls.env['recognition.karma.rank'].search([]).unlink()

        cls.rank_1 = cls.env['recognition.karma.rank'].create({
            'name': 'rank 1',
            'karma_min': 1,
        })

        cls.rank_2 = cls.env['recognition.karma.rank'].create({
            'name': 'rank 2',
            'karma_min': 250,
        })

        cls.rank_3 = cls.env['recognition.karma.rank'].create({
            'name': 'rank 3',
            'karma_min': 500,
        })
        cls.rank_4 = cls.env['recognition.karma.rank'].create({
            'name': 'rank 4',
            'karma_min': 1000,
        })
    ...
    def test_02_update_rank_without_switch(self):
        ...
        patch_bulk = patch('tele.applets.recognition.models.res_users.Users._recompute_rank', _patched_recompute_rank)
        patch_bulk.start()
    ...
    def test_03_test_bulk_call(self):
        ...
        patch_bulk = patch('tele.applets.recognition.models.res_users.Users._recompute_rank_bulk', _patched_check_in_bulk)
        patch_bulk.start()
