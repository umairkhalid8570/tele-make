...
from tele.applets.payment.tests.http_common import PaymentHttpCommon
...
class TestFlows(PaymentCommon, PaymentHttpCommon):
    ...
    def _test_flow(self, flow):
        ...
        with mute_logger('tele.applets.payment.models.payment_transaction'):
    ...
    def test_transaction_wrong_flow(self):
        ...
        with mute_logger('tele.http'):
            response = self.portal_transaction(**transaction_values)
        self.assertIn(
            "tele.exceptions.UserError: The payment should either be direct, with redirection, or made by a token.",
            response.text)
    ...
    def test_transaction_wrong_token(self):
        ...
        self.assertIn(
            "tele.exceptions.ValidationError: The access token is invalid.",
            response.text)

    @mute_logger('tele.applets.payment.models.payment_transaction')
