tele.define('web.BasicController', function (require) {
"use strict";
...
var BasicController = AbstractController.extend(FieldManagerMixin, {
    ...
    init: function (parent, model, renderer, params) {
        ...
        // record before leaving Tele (see @_urgentSave).
        this.pendingChanges = [];
    },
    /**
     * @override
     ...
     * haven't been sent to the model yet, just in case we would leave Tele
     ...
     * @param {TeleEvent} e - the event that triggered the change
     * @returns {Promise}
     */
    ...
    /**
     * To be called **only** when Tele is about to be closed, and we want to
    ...
    _onContentClicked(ev) {
        if (this.$buttons && this._shouldBounceOnClick(ev.target)) {
            this.$buttons.find('.btn-primary:visible:first').teleBounce();
        }
    },
    /**
