...
(function () {
    ...
    var tele = window.tele;
    var debug = tele.debug;
    ...
    tele.testing = typeof QUnit === "object";
    tele.remainingJobs = jobs;
    tele.__DEBUG__ = {
    ...
    tele.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === "string" ? args.shift() : "__tele_job" + jobUID++;
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: tele.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name,
                };
                if (jobs[k].error) {
                ...
                var deps = tele.__DEBUG__.getDependencies(job.name);
            ...
            missing = tele.__DEBUG__.getMissingJobs();
            failed = tele.__DEBUG__.getFailedJobs();
    ...
    tele.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec)
                        .then(function (data) {
                            ...
                            resolve();
                            tele.processJobs(jobs, services);
                        })
