tele.define('web.test_utils_mock', function (require) {
"use strict";
...
function _mockGlobalObjects(params) {
    ...
    if ('config' in params) {
        ...
        if ('debug' in params.config) {
            tele.debug = params.config.debug;
        }
    }
 ...
 * not fire.
 *
 * @param {Widget} widget the target widget (any Tele widget)
...
async function addMockEnvironmentOwl(Component, params, mockServer) {
    ...
    function cleanUp() {
        ...
        $('.blockUI').remove(); // fixme: move to qunit_config in TeleAfterTestHook?
 ...
 *   used to generate mock answers for every kind of routes supported by tele
 * @param {number} [params.debug] if set to true, logs RPCs and uncaught Tele
 *   events.
...
async function addMockEnvironment(widget, params) {
    ...
    // make sure all other Tele events bubbling up are intercepted
