...
from tele.tests.common import BaseCase, HttpCase, tagged
from tele.tools import topological_sort
...
class TestStaticInheritance(TestStaticInheritanceCommon):
    ...
    def test_static_inheritance_01(self):
        contents = HomeStaticTemplateHelpers.get_qweb_templates(applets=self._get_module_names(), debug=True)
        expected = b"""
            <templates>
                <form t-name="template_1_1" random-attr="gloria">
                    <div>At first I was afraid</div>
    ...
    def test_static_misordered_modules(self):
        ...
        with self.assertRaises(ValueError) as ve:
            HomeStaticTemplateHelpers.get_qweb_templates(applets=self._get_module_names(), debug=True)
    ...
    def test_replace_in_nodebug_mode1(self):
        ...
        contents = HomeStaticTemplateHelpers.get_qweb_templates(applets=self._get_module_names(), debug=False)
        expected = b"""
            <templates>
                <div t-name="template_1_1">
                    And I grew strong
