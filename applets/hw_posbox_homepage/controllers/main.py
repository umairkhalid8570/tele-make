...
from tele.http import Response
from tele.modules.module import get_resource_path

from tele.applets.hw_drivers.main import iot_devices
from tele.applets.hw_drivers.tools import helpers
from tele.applets.web.controllers import main as web
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('tele.applets.hw_posbox_homepage', "views")
...
class IoTboxHomepage(web.Home):
    ...
    def clean_partition(self):
        subprocess.check_call(['sudo', 'bash', '-c', '. /home/pi/tele/applets/point_of_sale/tools/posbox/configuration/upgrade.sh; cleanup'])

    def get_six_terminal(self):
        terminal_id = helpers.read_file_first_line('tele-six-payment-terminal.conf')
    ...
    def get_homepage_data(self):
        ...
        return {
            ...
            'server_status': helpers.get_tele_server_url() or 'Not Configured',
    ...
    def list_handlers(self):
        ...
        return handler_list_template.render({
            'title': "Tele's IoT Box - Handlers list",
            'breadcrumb': 'Handlers list',
            'drivers_list': drivers_list,
            'interfaces_list': interfaces_list,
            'server': helpers.get_tele_server_url()
        })
    ...
    def list_credential(self):
        return list_credential_template.render({
            'title': "Tele's IoT Box - List credential",
            'breadcrumb': 'List credential',
            'db_uuid': helpers.read_file_first_line('tele-db-uuid.conf'),
            'enterprise_code': helpers.read_file_first_line('tele-enterprise-code.conf'),
        })
    ...
    def connect_to_wifi(self, essid, password, persistent=False):
        ...
        if server:
            res_payload['server'] = {
                'url': server,
                'message': 'Redirect to Tele Server'
            }
        else:
            res_payload['server'] = {
    ...
    def connect_to_server(self, token, iotname):
        if token:
            ...
            url = helpers.get_tele_server_url()
    ...
    def step_by_step_configure_page(self):
        return configure_wizard_template.render({
            ...
            'ssid': helpers.get_wifi_essid(),
            'server': helpers.get_tele_server_url() or '',
    ...
    def server(self):
        return server_config_template.render({
            'title': 'IoT -> Tele server configuration',
            'breadcrumb': 'Configure Tele Server',
            'hostname': subprocess.check_output('hostname').decode('utf-8').strip('\n'),
            'server_status': helpers.get_tele_server_url() or 'Not configured yet',
    ...
    def upgrade(self):
        commit = subprocess.check_output(["git", "--work-tree=/home/pi/tele/", "--git-dir=/home/pi/tele/.git", "log", "-1"]).decode('utf-8').replace("\n", "<br/>")
        ...
        return upgrade_page_template.render({
            'title': "Tele's IoTBox - Software Upgrade",
    ...
    def perform_upgrade(self):
        ...
        os.system('/home/pi/tele/applets/point_of_sale/tools/posbox/configuration/posbox_update.sh')
    ...
    def perform_flashing_create_partition(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/tele/applets/point_of_sale/tools/posbox/configuration/upgrade.sh; create_partition']).decode().split('\n')[-2]
    ...
    def perform_flashing_download_raspios(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/tele/applets/point_of_sale/tools/posbox/configuration/upgrade.sh; download_raspios']).decode().split('\n')[-2]
    ...
    def perform_flashing_copy_raspios(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/tele/applets/point_of_sale/tools/posbox/configuration/upgrade.sh; copy_raspios']).decode().split('\n')[-2]
