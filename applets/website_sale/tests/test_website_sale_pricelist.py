...
from tele.applets.base.tests.common import TransactionCaseWithUserDemo, HttpCaseWithUserPortal
from tele.applets.website.tools import MockRequest
from tele.tests import tagged
from tele.tests.common import HttpCase, TransactionCase
...
class TestWebsitePriceList(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch('tele.applets.website_sale.models.website.Website.get_pricelist_available', wraps=self._get_pricelist_available)
        patcher.start()
...
def simulate_frontend_context(self, website_id=1):
    ...
    patcher = patch('tele.applets.website.models.ir_http.get_request_website', wraps=get_request_website)
    patcher.start()
    self.addCleanup(patcher.stop)
