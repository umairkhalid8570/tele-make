tele.define('website_sale.cart', function (require) {
'use strict';
...
tele.define('website_sale.website_sale_category', function (require) {
'use strict';
...
tele.define('website_sale.website_sale', function (require) {
'use strict';
        ...
        if (!config.device.isMobile) {
            ...
            _.each($('.ecom-zoomable img[data-zoom]'), function (el) {
                onImageLoaded(el, function () {
                    var $img = $(el);
                    $img.zoomTele({event: autoZoom ? 'mouseenter' : 'click', attach: attach});
...
publicWidget.registry.WebsiteSaleLayout = publicWidget.Widget.extend({
    ...
    _onApplyShopLayoutChange: function (ev) {
        ...
        if (wysiwyg) {
            wysiwyg.teleEditor.observerUnactive('_onApplyShopLayoutChange');
        }
        ...
        if (wysiwyg) {
            wysiwyg.teleEditor.observerActive('_onApplyShopLayoutChange');
        }
    },
});
...
tele.define('website_sale.price_range_option', function (require) {
'use strict';
