/** @tele-module alias=website.root */
...
export const WebsiteRoot = publicRootData.PublicRoot.extend(KeyboardNavigationMixin, {
    ...
    start: function () {
        ...
        this.$('.zoomable img[data-zoom]').zoomTele();
    ...
    async _loadGMapAPI(editableMode, refetch) {
        ...
        if (refetch || !this._gmapAPILoading) {
            this._gmapAPILoading = new Promise(async resolve => {
                ...
                window.tele_gmap_api_post_load = (async function tele_gmap_api_post_load() {
                ...
                await ajax.loadJS(`https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&callback=tele_gmap_api_post_load&key=${key}`);
            });
        }
