...
tele.define('partner.autocomplete.Mixin', function (require) {
'use strict';
...
var PartnerAutocompleteMixin = {
    _dropPreviousTele: new concurrency.DropPrevious(),
    ...
    _autocomplete: function (value) {
        ...
        return new Promise(function (resolve, reject) {
            var telePromise = self._getTeleSuggestions(value, isVAT).then(function (suggestions){
                teleSuggestions = suggestions;
            });
            ...
            var concatResults = function () {
                // Add Clearbit result with Tele result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    var websites = teleSuggestions.map(function (suggestion) {
                    ...
                    clearbitSuggestions.forEach(function (suggestion) {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            teleSuggestions.push(suggestion);
                        }
                    });
                }

                teleSuggestions = _.filter(teleSuggestions, function (suggestion) {
                    return !suggestion.ignored;
                });
                _.each(teleSuggestions, function(suggestion){
                delete suggestion.ignored;
                });
                return resolve(teleSuggestions);
            };

            self._whenAll([telePromise, clearbitPromise]).then(concatResults, concatResults);
        });
    ...
    /**
     * Use Tele Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
    ...
    _getTeleSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousTele.add(def);
    },
    /**
