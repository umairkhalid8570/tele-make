...
class Survey(models.Model):
    ...
    certification_badge_id = fields.Many2one('recognition.badge', 'Certification Badge', copy=False)
    ...
    def _create_certification_badge_trigger(self):
        ...
        goal = self.env['recognition.goal.definition'].create({
            'name': self.title,
        ...
        challenge = self.env['recognition.challenge'].create({
        ...
        self.env['recognition.challenge.line'].create({
    ...
    def _handle_certification_badges(self, vals):
        if vals.get('certification_give_badge'):
            ...
            challenges_to_delete = self.env['recognition.challenge'].search([('reward_id', 'in', badges.ids)])
