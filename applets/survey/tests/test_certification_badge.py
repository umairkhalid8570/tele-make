...
class TestCertificationBadge(common.TestSurveyCommon):
    ...
    def setUp(self):
        ...
        self.certification_badge = self.env['recognition.badge'].with_user(self.survey_manager).create({
        ...
        self.certification_badge_2 = self.env['recognition.badge'].with_user(self.survey_manager).create({
        ...
        self.certification_badge_3 = self.env['recognition.badge'].with_user(self.survey_manager).create({
    ...
    def test_badge_configuration(self):
        ...
        challenge = self.env['recognition.challenge'].search([('reward_id', '=', self.certification_badge.id)])
        ...
        challenge_line = self.env['recognition.challenge.line'].search([('challenge_id', '=', challenge.id)])
        ...
        challenge = self.env['recognition.challenge'].search([('id', '=', challenge.id)])
        ...
        challenge_line = self.env['recognition.challenge.line'].search([('id', '=', challenge_line.id)])
        ...
        goal = self.env['recognition.goal'].search([('id', '=', goal.id)])
        ...
        self.assertEqual(duplicate_survey.certification_badge_id, self.env['recognition.badge'], "Badge should be empty")
    ...
    def test_badge_configuration_multi(self):
        ...
        challenges = self.env['recognition.challenge'].search([('reward_id', 'in', certification_surveys.mapped('certification_badge_id').ids)])
        self.assertEqual(len(challenges), 3, "3 challenges should be created")
        challenge_lines = self.env['recognition.challenge.line'].search([('challenge_id', 'in', challenges.ids)])
        ...
        challenges = self.env['recognition.challenge'].search([('id', 'in', challenges.ids)])
        self.assertEqual(len(challenges), 0, "The 3 challenges should be deleted")
        challenge_lines = self.env['recognition.challenge.line'].search([('id', 'in', challenge_lines.ids)])
        self.assertEqual(len(challenge_lines), 0, "The 3 challenge_lines should be deleted")
        goals = self.env['recognition.goal'].search([('id', 'in', goals.ids)])
