...
from tele.applets.phone_validation.lib import phonenumbers_patch
...
class TestPhonenumbersPatch(BaseCase):
    def test_region_CI_monkey_patch(self):
        ...
        if parse_version('7.6.1') <= parse_version(phonenumbers.__version__) < parse_version('8.12.32'):
            # check that _local_load_region is set to `tele.applets.phone_validation.lib.phonenumbers_patch._local_load_region`
