...
class AutomotiveVehicleModel(models.Model):
    _name = 'automotive.vehicle.model'
    _description = 'Model of a vehicle'
    _order = 'name asc'

    name = fields.Char('Model name', required=True)
    brand_id = fields.Many2one('automotive.vehicle.model.brand', 'Manufacturer', required=True, help='Manufacturer of the vehicle')
    category_id = fields.Many2one('automotive.vehicle.model.category', 'Category')
    vendors = fields.Many2many('res.partner', 'automotive_vehicle_model_vendors', 'model_id', 'partner_id', string='Vendors')
    ...
    def _compute_vehicle_count(self):
        group = self.env['automotive.vehicle'].read_group(
