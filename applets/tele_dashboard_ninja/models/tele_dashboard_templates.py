...
class TeleDashboardNinjaTemplate(models.Model):
    _name = 'tele_dashboard_ninja.board_template'
    ...
    tele_dashboard_item_ids = fields.One2many('tele_dashboard_ninja.item', 'tele_dashboard_board_template_id',
                                            string="Template Type")
    tele_dashboard_board_id = fields.Many2one('tele_dashboard_ninja.board', string="Dashboard", help="""
    ...
    def create(self, vals_list):
        for val in vals_list:
            if val.get('tele_template_type', False) and val.get('tele_dashboard_board_id', False):
                dashboard_id = self.env['tele_dashboard_ninja.board'].browse(val.get('tele_dashboard_board_id'))
                ...
                val['tele_dashboard_item_ids'] = [(4, x.copy({'tele_dashboard_ninja_board_id': False}).id) for x in
                                                dashboard_id.tele_dashboard_items_ids]
        recs = super(TeleDashboardNinjaTemplate, self).create(vals_list)
        return recs
    ...
    def write(self, val):
        if val.get('tele_dashboard_board_id', False):
            ...
            val['tele_dashboard_item_ids'] = [(6, 0,
                                             [x.copy({'tele_dashboard_ninja_board_id': False}).id for x in
                                              dashboard_id.tele_dashboard_items_ids])]
        recs = super(TeleDashboardNinjaTemplate, self).write(val)
        return recs
