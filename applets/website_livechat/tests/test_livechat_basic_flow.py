...
from tele import tests, _
from tele.applets.website_livechat.tests.common import TestLivechatCommon
...
class TestLivechatBasicFlowHttpCase(tests.HttpCase, TestLivechatCommon):
    ...
    def test_basic_flow_with_rating(self):
        ...
        self.assertEqual(channel.message_ids[0].author_id, self.env.ref('base.partner_root'), "Telebot must be the sender of the 'has left the conversation' message.")
    ...
    def test_basic_flow_without_rating(self):
        ...
        self.assertEqual(channel.message_ids[0].author_id, self.env.ref('base.partner_root'), "Telebot must be the author the message.")
        self.assertEqual(channel.message_ids[0].body, "<p>%s has left the conversation.</p>" % self.visitor.display_name)
