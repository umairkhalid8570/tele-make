...
import { spreadsheetLinkMenuCellService} from "../../src/js/o_spreadsheet/registries/tele_menu_link_cell";
...
async function openMenuSelector(serverData, params = {}) {
    ...
    await click(webClient.el, ".o-menu-item[data-name='teleMenu']");
...
QUnit.module(
    ...
    () => {
        QUnit.test("ir.menu linked based on xml id", async function (assert) {
            ...
            setCellContent(model, "A1", "[label](tele://ir_menu_xml_id/test_menu)");
            const cell = getCell(model, "A1");
            assert.equal(cell.evaluated.value, "label", "The value should be the menu name");
            assert.equal(
                cell.content,
                "[label](tele://ir_menu_xml_id/test_menu)",
        ...
        QUnit.test("ir.menu linked based on record id", async function (assert) {
            ...
            const env = await makeTestEnv({ serverData: this.serverData });
            const model = new Model({}, { evalContext: { env } });
            setCellContent(model, "A1", "[label](tele://ir_menu_id/2)");
            const cell = getCell(model, "A1");
            assert.equal(cell.evaluated.value, "label", "The value should be the menu name");
            assert.equal(
                cell.content,
                "[label](tele://ir_menu_id/2)",
                "The content should be the complete markdown link"
            );
            assert.equal(cell.link.label, "label", "The link label should be the menu name");
            assert.equal(
        ...
        QUnit.test("ir.menu linked based on xml id which does not exists", async function (assert) {
            ...
            setCellContent(model, "A1", "[label](tele://ir_menu_xml_id/does_not_exists)");
            const cell = getCell(model, "A1");
            assert.equal(cell.content, "[label](tele://ir_menu_xml_id/does_not_exists)");
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                setCellContent(model, "A1", "[label](tele://ir_menu_id/9999)");
                const cell = getCell(model, "A1");
                assert.equal(cell.content, "[label](tele://ir_menu_id/9999)");
                assert.equal(cell.evaluated.value, "#BAD_EXPR");
            }
        );

        QUnit.test("Tele link cells can be imported/exported", async function (assert) {
            assert.expect(8);
            registry.category("services")
                .add("menu", menuService)
                .add("action", actionService)
        ...
        QUnit.test("insert a new ir menu link", async function (assert) {
            ...
            assert.equal(
                cell.content,
                "[menu with xmlid](tele://ir_menu_xml_id/test_menu)",
                "The content should be the complete markdown link"
            );
            assert.equal(
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                assert.equal(
                    cell.content,
                    "[menu without xmlid](tele://ir_menu_id/2)",
                    "The content should be the complete markdown link"
                );
                assert.equal(
                    webClient.el.querySelector(".o-link-tool a").text,
