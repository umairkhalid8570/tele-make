/** @tele-module alias=documents_spreadsheet.PivotViewTests */
...
test("pivot with two levels of group bys in rows", async (assert) => {
    ...
    assert.strictEqual(getCellFormula(model, "A3"), '=PIVOT.HEADER("1","bar","false")');
    assert.deepEqual(getCell(model, "A3").style, { fillColor: "#fcfcfc", bold: true });
    assert.strictEqual(
        getCellFormula(model, "A4"),
        '=PIVOT.HEADER("1","bar","false","product_id","41")'
    );
    assert.deepEqual(getCell(model, "A4").style, { fillColor: "#fcfcfc" });
...
test("pivot with two levels of group bys in cols", async (assert) => {
    ...
    assert.deepEqual(getCell(model, "A4").style, { fillColor: "#fcfcfc", bold: true });
    ...
    assert.deepEqual(getCell(model, "C2").style, { fillColor: "#fcfcfc", bold: true });
