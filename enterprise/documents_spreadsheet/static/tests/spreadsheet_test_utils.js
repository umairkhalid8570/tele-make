/** @tele-module alias=documents_spreadsheet.TestUtils default=0 */
...
/**
 * Return the tele spreadsheet component
 * @param {*} actionManager
 * @returns {Component}
 */
...
export async function createSpreadsheetAction(actionTag, params = {}) {
    ...
    { clearBreadcrumbs: true } // Sometimes in test defining custom action, Tele opens on the action instead of opening on root
    );
    return {
        webClient,
...
export async function createSpreadsheetWithPivotAndList() {
    ...
    model.dispatch("BUILD_TELE_LIST", {
        sheetId: "LIST",
        anchor: [0, 0],
        list,
        linesNumber: 10,
