...
const VIEW_PREFIX = "tele://view/";
const IR_MENU_ID_PREFIX = "tele://ir_menu_id/";
const IR_MENU_XML_ID_PREFIX = "tele://ir_menu_xml_id/";
...
class TeleMenuLinkCell extends LinkCell {
...
class TeleViewLinkCell extends LinkCell {
    /**
     *
     * @param {string} id
...
export const spreadsheetLinkMenuCellService = {
    ...
    start(env) {
        ...
        cellRegistry.add("TeleMenuIdLink", {
            sequence: 65,
            ...
            createCell: (id, content, properties, sheetId, getters) => {
                ...
                return new TeleMenuLinkCell(id, content, menuId, menuName, properties);
            },
        }).add("TeleMenuXmlLink", {
            sequence: 66,
        ...
        }).add("TeleIrFilterLink", {
            sequence: 67,
            match: isMarkdownViewLink,
            createCell: (id, content, properties, sheetId, getters) => {
                const { url } = parseMarkdownLink(content);
                const actionDescription = parseViewLink(url);
                return new TeleViewLinkCell(id, content, actionDescription, properties);
            },
        });
...
linkMenuRegistry.add("teleMenu", {
    name: _lt("Link an Tele menu"),
    sequence: 20,
    action: async (env) => {
