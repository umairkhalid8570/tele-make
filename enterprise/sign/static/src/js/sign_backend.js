    ...
    var DocumentBackend = AbstractAction.extend({
        ...
        start: function () {
            ...
            var def = this._rpc({
                ...
                if (url) {
                    self.$downloadButton = $('<a/>', {html: _t("Download Document")}).addClass('btn btn-primary rounded-sm mr-2');
                    self.$downloadButton.attr('href', url);
                    self.$buttons = self.$buttons.add(self.$downloadButton);
                }
                if (logUrl) {
                    self.$downloadLogButton = $('<a/>', {html: _t("Certificate")}).addClass(url ? 'btn btn-secondary rounded-sm' : 'btn btn-primary rounded-sm');
    ...
    var EditableDocumentBackend = DocumentBackend.extend({
        ...
        init: function(parent, action, options) {
            ...
            if (action && action.context && action.context.sign_token) {
                var $signButton = $('<button/>', {html: _t("Sign Document"), type: "button", 'class': 'btn btn-primary rounded-sm mr-2'});
        ...
        start: function() {
            ...
            if (nextTemplate && nextTemplate.template) {
                let nextDocumentButton = $('<button/>', {html: _t("Next Document"), type: "button", 'class': 'btn btn-primary rounded-sm mr-2'});
