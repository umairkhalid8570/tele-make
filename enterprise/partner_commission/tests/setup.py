...
class TestCommissionsSetup(TransactionCase):
    ...
    def _make_products(self):
        ...
        # tele sh
        self.tele_sh = self.env['product.category'].create({
            'name': 'Tele.SH',
        })

        self.worker = self.env['product.product'].create({
            'name': 'Tele.sh Worker',
            'categ_id': self.tele_sh.id,
            'list_price': 100.0,
    ...
    def _make_commission_plans(self):
        self.learning_plan = self.env['commission.plan'].create({
            ...
            'commission_rule_ids': [
                (0, 0, self._make_rule(self.tele_sh, 10, product=self.worker, pricelist=self.eur_20, is_capped=True, max_comm=150)),
                (0, 0, self._make_rule(self.tele_sh, 10, product=self.worker, pricelist=self.usd_8, is_capped=True, max_comm=180)),
        ...
        self.ready_plan = self.env['commission.plan'].create({
            ...
            'product_id': self.env.ref('partner_commission.product_commission').id,
            'commission_rule_ids': [
                (0, 0, self._make_rule(self.tele_sh, 100, product=self.worker, pricelist=self.eur_20, is_capped=True, max_comm=150)),
                (0, 0, self._make_rule(self.tele_sh, 100, product=self.worker, pricelist=self.usd_8, is_capped=True, max_comm=180)),
                (0, 0, self._make_rule(self.apps_support, 10)),
            ],
        })
        ...
        self.gold_plan = self.env['commission.plan'].create({
            ...
            'product_id': self.env.ref('partner_commission.product_commission').id,
            'commission_rule_ids': [
                (0, 0, self._make_rule(self.tele_sh, 100, pricelist=self.eur_20, is_capped=True, max_comm=150)),
                (0, 0, self._make_rule(self.tele_sh, 100, pricelist=self.usd_8, is_capped=True, max_comm=180)),
        ...
        self.greedy_plan = self.env['commission.plan'].create({
            ...
            'product_id': self.env.ref('partner_commission.product_commission').id,
            'commission_rule_ids': [
                (0, 0, self._make_rule(self.tele_sh, 100, pricelist=self.usd_8, is_capped=True, max_comm=18)),
            ],
        })
