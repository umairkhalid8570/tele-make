tele.define('tele_studio.SearchEditor', function (require) {
"use strict";
...
var SearchEditor = SearchRenderer.extend(EditorMixin, {
    ...
    className: SearchRenderer.prototype.className + ' o_tele_studio_search_view_editor',
    ...
    highlightNearestHook: function ($helper, position) {
        ...
        var $nearest_form_hook = this.$('.o_tele_studio_hook')
            .touching({
        ...
        if ($nearest_form_hook.length) {
            ...
            const isField = hookClasses.indexOf("o_tele_studio_field") > -1;
            const isFilter = hookClasses.indexOf("o_tele_studio_filter") > -1;
            const isSeparator = hookClasses.indexOf("o_tele_studio_filter_separator") > -1;
            ...
            if (addHook) {
                $nearest_form_hook.addClass('o_tele_studio_nearest_hook');
                return true;
            }
        }
        return false;
    ...
    _addHookEmptyTable: function () {
        var $tbody = this.$('.o_tele_studio_search_autocompletion_fields tbody');
        if (!$tbody.children().length) {
            this._addFirstHook($tbody, 'field');
        }
        $tbody = this.$('.o_tele_studio_search_filters tbody');
        if (!$tbody.children().length) {
            this._addFirstHook($tbody, 'filter');
        }
        $tbody = this.$('.o_tele_studio_search_group_by tbody');
        if (!$tbody.children().length) {
    ...
    _render: function () {
        ...
        this.$el.droppable({
            ...
            tolerance: "touch",
            over: function (ev, ui) {
                var $autocompletionFields = self.$('.o_tele_studio_search_autocompletion_fields');
                var $filters = self.$('.o_tele_studio_search_filters');
                var $grouBy = self.$('.o_tele_studio_search_group_by');
