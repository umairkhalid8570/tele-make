tele.define('tele_studio.EditorMixin', function() {
"use strict";
...
return {
    ...
    handleDrop: function ($drag, node, position) {
        var isNew = $drag.hasClass('o_tele_studio_component');
        var values;
        if (isNew) {
            values = {
                type: 'add',
    ...
    highlightNearestHook: function ($helper, position) {
        this.$('.o_tele_studio_nearest_hook').removeClass('o_tele_studio_nearest_hook');
    ...
    setSelectable: function ($el) {
        ...
        $el.click(function () {
            ...
            $(this).addClass('o_tele_studio_clicked');
        })
        .mouseover(function () {
            if (self.$('.ui-draggable-dragging').length) {
                return;
            }
            $(this).addClass('o_tele_studio_hovered');
        })
        .mouseout(function () {
            $(this).removeClass('o_tele_studio_hovered');
        });
    },
    unselectedElements: function () {
        this.selected_node_id = false;
        var $el = this.$('.o_tele_studio_clicked');
        $el.removeClass('o_tele_studio_clicked');
        if ($el.find('.blockUI')) {
