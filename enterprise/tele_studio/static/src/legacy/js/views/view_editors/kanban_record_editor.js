tele.define('tele_studio.KanbanRecordEditor', function (require) {
"use strict";
...
var KanbanRecordEditor = KanbanRecord.extend(EditorMixin, {
    ...
    _addSpecialHooks: function () {
        ...
        if (_.isUndefined(tagsWidget)) {
            var $kanban_tags_hook = $('<span>')
                .addClass('o_tele_studio_add_kanban_tags')
                .append($('<span>', {
                    text: _t('Add tags'),
                }));
        ...
        if ($dropdown.length) {
            ...
            var $top_left_hook = $('<div>')
                .addClass('o_tele_studio_add_dropdown o_dropdown_kanban dropdown')
                .append($('<a>', {
        ...
        if (_.isUndefined(priorityWidget) && _.isUndefined(favoriteWidget)) {
            var $priority_hook = $('<div>')
                .addClass('o_tele_studio_add_priority oe_kanban_bottom_left')
                .append($('<span>', {
        ...
        if (avatarNode) {
            if (!$image.length) {
                $image = $(qweb.render('tele_studio.AvatarPlaceholder'));
            ...
            var $kanban_image_hook = $('<div>')
                .addClass('o_tele_studio_add_kanban_image oe_kanban_bottom_right')
                .append($('<span>', {
    ...
    _bindHandler: function ($node, fieldName) {
        ...
        if (invisibleModifier && this._computeDomain(invisibleModifier)) {
            $node.addClass('o_tele_studio_show_invisible');
        }
    },
    /**
     * @private
    ...
    _processField: function ($field, field_name) {
        ...
        if (this._isEmpty(field.value)) {
            ...
            $field.addClass('o_tele_studio_widget_empty');
        }
        ...
        if (invisibleModifier && this._computeDomain(invisibleModifier)) {
            $field.addClass('o_tele_studio_show_invisible');
        }
    ...
    _processWidget: function ($field, field_name) {
        ...
        Promise.resolve(this.defs[currentDefIndex]).then(function () {
            ...
            if (self._isEmpty(widget.value)) {
                widget.$el.addClass('o_tele_studio_widget_empty');
