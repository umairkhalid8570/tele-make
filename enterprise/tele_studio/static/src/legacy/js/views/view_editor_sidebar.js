tele.define('tele_studio.ViewEditorSidebar', function (require) {
"use strict";
...
var view_components = require('tele_studio.view_components');
...
return Widget.extend(StandaloneFieldManagerMixin, {
    template: 'tele_studio.ViewEditorSidebar',
    events: {
        'click .o_tele_studio_new:not(.inactive)':            '_onTab',
        'click .o_tele_studio_view':                          '_onTab',
        'click .o_tele_studio_xml_editor':                    '_onXMLEditor',
        'click .o_display_view .o_tele_studio_parameters':    '_onViewParameters',
        'click .o_display_field .o_tele_studio_parameters':   '_onFieldParameters',
        'click .o_display_view .o_tele_studio_defaults':      '_onDefaultValues',
        'change #show_invisible':                            '_onShowInvisibleToggled',
        'click .o_tele_studio_remove':                        '_onElementRemoved',
        'click .o_tele_studio_restore':                       '_onRestoreDefaultView',
        'change .o_display_view input':                      '_onViewChanged',
        'change .o_display_view select':                     '_onViewChanged',
        'click .o_tele_studio_edit_selection_values':         '_onSelectionValues',
        ...
        'click .o_tele_studio_sidebar_approval .o_approval_archive':  '_onApprovalArchive',
        'change .o_tele_studio_sidebar_approval':                     '_onApprovalChange',
        'click .o_tele_studio_sidebar_approval .o_approval_domain':   '_onApprovalDomain',
        'click .o_tele_studio_sidebar_approval .o_approval_new':      '_onApprovalNewRule',
        ...
        'click .o_tele_studio_attrs':                         '_onDomainAttrs',
        'focus .o_display_filter input#domain':              '_onDomainEditor',
        'keyup .o_tele_studio_sidebar_search_input':          '_onSearchInputChange',
        'click .o_tele_studio_existing_fields_header':        '_onClickExistingFieldHeader',
    },
    /**
     * @constructor
     * @param {Widget} parent
    ...
    */
    ...
    _render: function () {
        ...
        if (this.state.mode === 'new') {
            ...
            return Promise.all(this.defs).then(() => {
                delete(this.defs);
                this.$('.o_tele_studio_component').on("drag", _.throttle((event, ui) => {
    ...
    _renderComponentsSection: function () {
        ...
        $section.addClass('o_tele_studio_new_components');
    ...
    _renderExistingFieldsSection: function () {
        const $existingFields = this.$('.o_tele_studio_existing_fields');
        ...
        if ($existingFields.length) {
            this.$('.o_tele_studio_existing_fields_section').append($section);
        } else {
            const $sectionTitle = $('<h3>', {
                text: _t('Existing Fields'),
                class: 'o_tele_studio_existing_fields_header',
            }).append($('<i/>', {class: `o_tele_studio_existing_fields_icon fa fa-caret-right ml-2`}));
            const $sectionSubtitle = $('<h6>', {
                class: 'small text-white',
                text: _t('The following fields are currently not in the view.'),
            });
            const $sectionSearchDiv = core.qweb.render('tele_studio.ExistingFieldsInputSearch');
    ...
    _renderNewFieldsSection: function () {
        ...
        const $section = this._renderSection(formWidgets);
        $section.addClass('o_tele_studio_new_fields');
    ...
    _renderSection: function (form_widgets) {
        var self = this;
        var $components_container = $('<div>').addClass('o_tele_studio_field_type_container');
    ...
    _updateExistingFieldSection() {
        const icon = this.el.querySelector('.o_tele_studio_existing_fields_icon');
        const section = this.el.querySelector('.o_tele_studio_existing_fields_section');
    ...
    _onSearchInputChange: function () {
        this._searchValue = this.$('.o_tele_studio_sidebar_search_input').val();
    ...
    _onRestoreDefaultView: function () {
        ...
        Dialog.confirm(this, message, {
            ...
            dialogClass: 'o_tele_studio_preserve_space'
        });
    },
    /**
     * @private
