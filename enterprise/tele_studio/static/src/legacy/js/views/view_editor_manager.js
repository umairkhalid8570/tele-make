...
var KanbanEditor = require('tele_studio.KanbanEditor');
var ListEditor = require('tele_studio.ListEditor');
var SearchEditor = require('tele_studio.SearchEditor');
var SearchRenderer = require('tele_studio.SearchRenderer');

var FieldSelectorDialog = require('tele_studio.FieldSelectorDialog');
var NewButtonBoxDialog = require('tele_studio.NewButtonBoxDialog');
var NewFieldDialog = require('tele_studio.NewFieldDialog');
var utils = require('tele_studio.utils');
var ViewEditorSidebar = require('tele_studio.ViewEditorSidebar');
...
var ViewEditorManager = AbstractEditorManager.extend({
    ...
    updateEditor: function (options) {
        ...
        return this._instantiateEditor(options).then(function (editor) {
            ...
            return prom.then(function () {
                dom.append(self.$('.o_tele_studio_view_renderer'), [fragment], {
                    in_DOM: self.isInDOM,
                ...
                def_field_values = new Promise(function (resolve, reject) {
                    ...
                    prom.then(function (openFieldDialog) {
                        if (!openFieldDialog) {
                            ...
                            var $message = $(QWeb.render('tele_studio.FieldOne2manyWarning'));
    ...
    _applyChangeHandling: async function (result, opID) {
        ...
        // studio_view_id must be updated (but /tele_studio/edit_view_arch
    ...
    _createInlineView: async function (type, field_name) {
        ...
        const studioViewArch = await this._rpc({
            route: '/tele_studio/create_inline_view',
            params: {
                model: this.x2mModel,
                view_id: this.view_id,
    ...
    _editViewArch: function (view_id, view_arch) {
        core.bus.trigger('clear_cache');
        return this._rpc({
            route: '/tele_studio/edit_view_arch',
            params: {
                view_id: view_id,
                view_arch: view_arch,
                // We write views in the base language to make sure we do it on the source term field
    ...
    _getDefaultValue: function (model_name, field_name) {
        return this._rpc({
            route: '/tele_studio/get_default_value',
            params: {
    ...
    _getEmailAlias: function (model_name) {
        return this._rpc({
            route: '/tele_studio/get_email_alias',
            params: {
                model_name: model_name,
            },
        });
    ...
    _renameField: function (oldName, newName) {
        ...
        return this._rpc({
            route: '/tele_studio/rename_field',
            params: {
    ...
    _restoreDefaultView: async function (view_id) {
        ...
        const result = await this._rpc({
            route: '/tele_studio/restore_default_view',
            params: {
                view_id: view_id,
            },
        });
    ...
    _setDefaultValue: function (model_name, field_name, value) {
        ...
        return this._rpc({route: '/tele_studio/set_default_value', params: params});
    },
    /**
     * @private
     * @param {String} model_name
    ...
    _setEmailAlias: function (model_name, value) {
        return this._rpc({
            route: '/tele_studio/set_email_alias',
            params: {
                model_name: model_name,
                value: value,
            },
    ...
    */
    ...
    _onApprovalChange: async function(ev) {
        ...
        bus.trigger('toggle_snack_bar', 'saving');
        const result = await this._rpc({
            route: '/tele_studio/edit_approval',
            params: {
                model: this.model_name,
    ...
    _onNodeClicked: function (ev) {
        ...
        if (this.view_type === 'form' && node.tag === 'field') {
            ...
            if (isX2Many && !_.contains(notEditableWidgets, attrs.widget)) {
                ...
                // create the subviews
                var message = $(QWeb.render('tele_studio.X2ManyEdit'));
                var options = {
                ...
                $node.find('.o_tele_studio_editX2Many').click(function (e) {
                    e.preventDefault();
                    e.stopPropagation();
