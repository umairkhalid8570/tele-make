tele.define('tele_studio.NewModel', function (require) {
"use strict";

const core = require('web.core');
const session = require('web.session');
const Widget = require('web.Widget');
const CommonMenuDialog = require('tele_studio.CommonMenuDialog');

const _t = core._t;

const NewModelItem = Widget.extend({
    xmlDependencies: ['/tele_studio/static/src/legacy/xml/new_model.xml'],
    template: 'tele_studio.NewModel',
    events: {
...
const NewModelDialog = CommonMenuDialog.extend({
    template: 'tele_studio.NewModel.Dialog',
    ...
    async start() {
        await this._super(...arguments);
        this.opened(() => {
            this.$modal.addClass('o_tele_studio_new_model_modal');
            // focus on input
    ...
    _doSave(menuName) {
        ...
        return this._rpc({
            route: '/tele_studio/create_new_menu',
            params: {
                menu_name: menuName,
                model_id: false,
                model_choice: 'new',
