tele.define('tele_studio.ReportEditorSidebar', function (require) {
"use strict";
...
var newComponentsRegistry = require('tele_studio.reportNewComponentsRegistry');
var studioUtils = require('tele_studio.utils');
...
var ReportEditorSidebar = Widget.extend(StandaloneFieldManagerMixin, {
    template: 'tele_studio.ReportEditorSidebar',
    events: {
        'change input': '_onChangeReport',
        'click .o_tele_studio_sidebar_header > div:not(.inactive)': '_onTab',
        'click .o_tele_studio_xml_editor': '_onXMLEditor',
        'click .o_tele_studio_parameters': '_onParameters',
        'click .o_tele_studio_remove': '_onRemove',
    },
    /**
     * @constructor
     * @param {Widget} parent
    ...
    _startModeNew: function () {
        var self = this;
        var defs = [];
        var $sidebarContent = this.$('.o_tele_studio_sidebar_content');
        ...
        _.each(newComponentsRegistry.map, function (components, title) {
            ...
            var $componentsContainer = $('<div>', {
                class: 'o_tele_studio_field_type_container',
            });
    ...
    _startModeProperties: function () {
        ...
        var $accordion = this.$('.o_tele_studio_sidebar_content .o_tele_studio_accordion');
        ...
        this.nodes.forEach(function (node) {
            var $accordionSection = $(qweb.render('tele_studio.AccordionSection', {
            ...
            componentsAppendedPromise = Promise.all(renderingProms).then(function () {
                ...
                var $removeButton = $(qweb.render('tele_studio.Sidebar.Remove'));
            ...
            $accordionSection
                ...
                .find('.collapse').on('show.bs.collapse hide.bs.collapse', function (ev) {
                    $(this).parent('.card').toggleClass('o_tele_studio_active', ev.type === 'show');
                });
        });
        ...
        $lastCard.addClass('o_tele_studio_active');
    ...
    _startModeReport: function () {
        ...
        defs.push(many2one.appendTo(this.$('.o_tele_studio_paperformat_id')));
