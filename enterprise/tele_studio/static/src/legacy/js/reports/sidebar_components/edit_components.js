tele.define('tele_studio.reportEditComponents', function (require) {
"use strict";
...
var Abstract = require('tele_studio.AbstractReportComponent');
...
var AbstractEditComponent = Abstract.extend(WidgetAdapterMixin, StandaloneFieldManagerMixin, {
    ...
    _onDirectiveChange: function (e) {
        var self = this;
        e.stopPropagation();  // TODO: is it really useful on an TeleEvent
...
var LayoutEditable = AbstractEditComponent.extend({
    name: 'layout',
    template : 'tele_studio.ReportLayoutEditable',
    events : _.extend({}, AbstractEditComponent.prototype.events, {
        "change .o_tele_studio_margin>input": "_onMarginInputChange",
        "change .o_tele_studio_width>input": "_onWidthInputChange",
        "click .o_tele_studio_font_size .dropdown-item-text": "_onFontSizeChange",
        "change .o_tele_studio_table_style > select": "_onTableStyleInputChange",
        "click .o_tele_studio_text_decoration button": "_onTextDecorationChange",
        "click .o_tele_studio_text_alignment button": "_onTextAlignmentChange",
        "change .o_tele_studio_classes>input": "_onClassesChange",
        "click .o_tele_studio_colors .o_tele_studio_reset_color": "_onResetColor",
        "click .o_tele_studio_colors .o_tele_studio_custom_color": "_onCustomColor",
    }),
    /**
     * @override
     */
    ...
    renderElement: function() {
        var self = this;
        this._super.apply(this, arguments);
        this.$('.o_tele_studio_background_colorpicker .o_tele_studio_color_palette').append(this._createPalette());
        this.$('.o_tele_studio_background_colorpicker').on("mousedown", 'button[data-color]', function (e) {
            self._onColorChange($(e.currentTarget).data('value').replace('text-', 'bg-'), "background");
        });
        this.$('.o_tele_studio_font_colorpicker .o_tele_studio_color_palette').append(this._createPalette());
        this.$('.o_tele_studio_font_colorpicker').on("mousedown", 'button[data-color]', function (e) {
    ...
    _createPalette: function () {
        ...
        this._groupColors.forEach(function (color) {
            ...
            if (typeof color === 'string') {
                ...
                $row = $('<div class="o_tele_studio_color_row"/>');
    ...
    _onCustomColor: function (e) {
        ...
        colorpicker.on('colorpicker:saved', this, (ev) => {
            ...
            $button.attr('data-value', color);
            $button.css('background-color', color);
            $(e.target).closest('.dropdown-item').find('.o_tele_studio_custom_colors').append($button);
            $button.mousedown();
        });
        colorpicker.open();
    },
...
var TField = AbstractEditComponent.extend({
    name: 'tfield',
    template : 'tele_studio.ReportDirectiveTField',
    selector: '[t-field]',
    /**
     * @override
     */
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            var $expr = self.$('.o_tele_studio_tfield_fieldexpression');
...
var TIf = AbstractEditComponent.extend({
    name: 'tif',
    template : 'tele_studio.ReportDirectiveTIf',
    selector: '',
    events: _.extend({}, AbstractEditComponent.prototype.events, {
    ...
    start: function () {
        ...
        try {
            ...
            console.warn("Can't convert the condition in an Tele domain", this.node.attrs['t-if'], e);
            this.$('.o_field_domain_dialog_button').hide();
        }
        return this._super.apply(this, arguments).then(function () {
            return self.fieldSelector['t-if'].appendTo(self.$('.o_tele_studio_tif_ifexpression'));
        });
    },
...
var TElse = AbstractEditComponent.extend({
    name: 'telse',
    template : 'tele_studio.ReportDirectiveTElse',
    selector: '[t-else]',
    ...
    start: function () {
        var self = this;
        return this._super.apply(this, arguments).then(function () {
            return self.fieldSelector['t-else'].appendTo(self.$('.o_tele_studio_telse_elseexpression'));
        });
    },
    /**
     * @override
...
var TEsc = AbstractEditComponent.extend({
    name: 'tesc',
    template : 'tele_studio.ReportDirectiveTEsc',
    selector: '[t-esc]',
    /**
     * @override
     */
    ...
    start: function () {
        var self = this;
        return this._super.apply(this, arguments).then(function () {
            return self.fieldSelector['t-esc'].appendTo(self.$('.o_tele_studio_tesc_escexpression'));
        });
    },
});
...
var TSet = AbstractEditComponent.extend({
    name: 'tset',
    template : 'tele_studio.ReportDirectiveTSet',
    selector: '[t-set]',
    /**
     * @override
     */
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            return Promise.all([
                self.fieldSelector['t-set'].appendTo(self.$('.o_tele_studio_tset_setexpression')),
                self.fieldSelector['t-value'].appendTo(self.$('.o_tele_studio_tset_valueexpression'))
            ]);
        });
    },
});
...
var TForeach = AbstractEditComponent.extend({
    name: 'tforeach',
    template : 'tele_studio.ReportDirectiveTForeach',
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            return Promise.all([
                self.fieldSelector['t-as'].appendTo(self.$('.o_tele_studio_tas_asexpression')),
                self.fieldSelector['t-foreach'].appendTo(self.$('.o_tele_studio_tforeach_foreachexpression'))
            ]);
        });
    },
});
...
var BlockTotal = AbstractEditComponent.extend({
    name: 'blockTotal',
    template : 'tele_studio.BlockTotal',
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            return Promise.all([
                self.fieldSelector.total_currency_id.appendTo(self.$('.o_tele_studio_report_currency_id')),
                self.fieldSelector.total_amount_untaxed.appendTo(self.$('.o_tele_studio_report_amount_untaxed')),
                self.fieldSelector.total_amount_total.appendTo(self.$('.o_tele_studio_report_amount_total')),
                self.fieldSelector.total_amount_by_groups.appendTo(self.$('.o_tele_studio_report_amount_by_groups'))
            ]);
        });
    },
    /**
...
var Column = AbstractEditComponent.extend({
    name: 'column',
    template : 'tele_studio.ReportColumn',
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            return Promise.all([
                self.fieldSelector.size.prependTo(self.$('.o_tele_studio_size')),
                self.fieldSelector.offset.prependTo(self.$('.o_tele_studio_offset'))
            ]);
        });
    },
    /**
...
var Text = AbstractEditComponent.extend({
    name: 'text',
    template : 'tele_studio.ReportText',
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments)
            .then(function () {
                return self.fieldSelector.text.appendTo(self.$('.o_tele_studio_text'));
            }).then(function () {
    ...
    _startWysiwygEditor: function () {
        ...
        const options = {
            ...
            resizable: true,
            toolbarTemplate: 'tele_studio.Sidebar.web_editor_toolbar',
        };
...
var Image = LayoutEditable.extend({
    name: 'image',
    template: 'tele_studio.ReportImage',
    selector: 'img',
    /**
     * @override
     */
    ...
    start: function() {
        var self = this;
        return this._super.apply(this, arguments).then(function () {
            return self.fieldSelector.src.appendTo(self.$('.o_tele_studio_source'));
        });
    },
});
...
var Groups = AbstractEditComponent.extend({
    name: 'groups',
    template: 'tele_studio.ReportGroups',
    insertAsLastChildOfPrevious: true,
    /**
     * @override
     */
    ...
    start: function() {
        var self = this;
        return this._super.apply(this, arguments).then(function () {
            return self.fieldSelector.groups.appendTo(self.$('.o_tele_studio_groups'));
        });
    },
});
...
var TOptions = AbstractEditComponent.extend( {
    name: 'toptions',
    template : 'tele_studio.ReportDirectiveTOptions',
    ...
    start: function () {
        ...
        if (this.widget) {
            this.$('.o_tele_studio_toption_widget select').val(this.widget.key);
    ...
    _updateWidgetOptions: function () {
        ...
        this.$('.o_tele_studio_toption_options').remove();
        if (!this.widget || !this.widget.options.length) {
            return;
        }
        var $options = $(qweb.render('tele_studio.ReportDirectiveTOptions.options', this));
