tele.define('tele_studio.ReportEditorManager', function (require) {
"use strict";
...
var ReportEditorManager = AbstractEditorManager.extend(WidgetAdapterMixin, {
    className: AbstractEditorManager.prototype.className + ' o_tele_studio_report_editor_manager',
    ...
    events: _.extend({}, AbstractEditorManager.prototype.events, {
        'click .o_tele_studio_report_print': '_onPrintReport',
    }),
    /**
     * @override
     * @param {Object} params
    ...
    _editView: function (view_id, studio_view_arch, operations) {
        ...
        return this._rpc({
            route: '/tele_studio/edit_report_view',
            params: {
    ...
    _editViewArch: function (view_id, view_arch) {
        core.bus.trigger('clear_cache');
        return this._rpc({
            route: '/tele_studio/edit_report_view_arch',
            params: {
                record_id: this.env.currentId,
                report_name: this.reportName,
                view_id: view_id,
    ...
    _getReportViews: function () {
        return this._rpc({
            route: '/tele_studio/get_report_views',
            params: {
                record_id: this.env.currentId,
                report_name: this.reportName,
                context: session.user_context,
    ...
    _renderActionsSection: async function () {
        const actionsSection = Object.assign(document.createElement('div'), {
            className: 'o_tele_studio_report_actions',
            innerHTML: qweb.render('tele_studio.PrintSection'),
        });
        this.el.appendChild(actionsSection);

        if (this.withPager) {
            const pagerContainer = Object.assign(document.createElement('div'), {
                className: 'o_tele_studio_report_pager',
            });
    ...
    _setPaperFormat: function () {
        ...
        var $container = this.$('.o_tele_studio_report_iframe_container');
        $container.css({
        ...
        this.$('.o_tele_studio_report_iframe').css({
            // to remove
            'min-height': (format.print_page_height || 297) + 'mm',
    ...
    _onPrintReport: function () {
        var self = this;
        this._rpc({
            route: '/tele_studio/print_report',
            params: {
                record_id: this.env.currentId,
                report_name: this.reportName,
                context: session.user_context,
