...
export class EditorAdapter extends ComponentAdapter {
    ...
    async _onNewView(data) {
        ...
        const activityAllowed = await this.rpc("/tele_studio/activity_allowed", {
    ...
    async addViewType(action, viewType, args) {
        let viewAdded = await this.rpc("/tele_studio/add_view_type", {
    ...
    async editAction(action, args) {
        ...
        const result = await this.rpc("/tele_studio/edit_action", {
            action_type: action.type,
            action_id: action.id,
            args: args,
            context: this.user.context,
    ...
    _onRestoreDefaultView(data) {
        ...
        const confirm = async () => {
            ...
            return this.rpc("/tele_studio/restore_default_view", {
...
registry.category("actions").add("tele_studio.action_editor", EditorAdapter);
