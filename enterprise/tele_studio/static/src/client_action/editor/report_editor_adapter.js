...
class ReportEditorAdapter extends ComponentAdapter {
    ...
    async _readPaperFormat() {
        this.paperFormat = "A4";
        const result = await this.rpc("/tele_studio/read_paperformat", {
    ...
    async _readWidgetsOptions() {
        this.widgetsOptions = await this.rpc("/tele_studio/get_widgets_available_options", {
            context: this.user.context,
        });
    }
    /**
    ...
    async _getReportViews() {
        ...
        this.reportViews = await this.rpc("/tele_studio/get_report_views", {
    ...
    async _editReport(values) {
        const result = await this.rpc("/tele_studio/edit_report", {
