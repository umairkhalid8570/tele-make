...
export class HomeMenuCustomizer extends owl.Component {
    ...
    _setBackgroundImage(attachment_id) {
        return this.rpc("/tele_studio/set_background_image", {
            attachment_id: attachment_id,
            context: this.user.context,
        });
    }
    /**
     * Export all customizations done by Studio in a zip file containing Tele
     * modules.
     */
    _export() {
        download({ url: "/tele_studio/export", data: {} });
    }
    /**
    ...
    async _resetBgConfirmed() {
        this.ui.block();
        try {
            await this.rpc("/tele_studio/reset_background_image", {
                context: this.user.context,
            });
            browser.location.reload();
        } finally {
...
HomeMenuCustomizer.template = "tele_studio.HomeMenuCustomizer";
