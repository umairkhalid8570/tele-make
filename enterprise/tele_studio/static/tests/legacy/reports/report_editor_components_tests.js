tele.define('tele_studio.ReportEditorComponents_tests', function (require) {
"use strict";
...
var studioTestUtils = require('tele_studio.testUtils');
var editComponentsRegistry = require('tele_studio.reportEditComponentsRegistry');
var reportNewComponentsRegistry = require('tele_studio.reportNewComponentsRegistry');
...
QUnit.module('ReportComponents', {
    ...
    QUnit.test('field', async function (assert) {
        ...
        await testUtils.dom.triggerEvents($('.o_tele_studio_field_modal .o_field_selector'), ['focus']);
        await testUtils.dom.click($('.o_tele_studio_field_modal .o_field_selector_close'));
    ...
    QUnit.test('add a binary field', async function (assert) {
        ...
        await testUtils.dom.triggerEvents($('.o_tele_studio_field_modal .o_field_selector'), ['focus']);
        await testUtils.dom.click($('.o_tele_studio_field_modal .o_field_selector_item[data-name="image"]'));
        await testUtils.dom.click($('.o_tele_studio_field_modal .btn-primary'));
    ...
    QUnit.test('hidden "width" for layout component with col nodes', async function (assert) {
        ...
        assert.containsNone(layout.$('.o_tele_studio_width'),
    ...
    QUnit.test('tOptions component', async function (assert) {
        ...
        assert.containsNone(tOptions, '.o_tele_studio_toption_option',
    ...
    QUnit.test('tOptions component parse expression', async function (assert) {
        ...
        assert.strictEqual(tOptions.$('select').val(), 'monetary',
            "Should select the image widget");
        assert.containsN(tOptions, '.o_tele_studio_toption_option', 4,
            "there should be 4 available options for the monetary widget");
        assert.strictEqual(tOptions.$('.o_tele_studio_toption_option_monetary_from_currency .o_field_selector_value').text().replace(/\s+/g, ''),
            "o(ModelTest)ChildCurrency",
            "Should display the currency field");
        assert.strictEqual(tOptions.$('.o_tele_studio_toption_option_monetary_date .o_field_selector_value').text().replace(/\s+/g, ''),
            "o(ModelTest)ChildDate",
            "Should display the data field");
        assert.strictEqual(tOptions.$('.o_tele_studio_toption_option_monetary_company_id .o_field_selector_value').text().replace(/\s+/g, ''),
    ...
    QUnit.test('contact: many2many_select', async function (assert) {
        ...
        await tOptions.appendTo(parent.$el);
        assert.containsN(tOptions, '.o_tele_studio_toption_option', 3,
    ...
    QUnit.test('contact: address separator', async function (assert) {
        ...
        var tOptions = new (editComponentsRegistry.get('tOptions'))(parent, params);
        await tOptions.appendTo(parent.$el);
        var separators = _.map(tOptions.$('.o_tele_studio_toption_option_contact_separator .o_field_widget option'), function (option) {
            return JSON.parse($(option).val());
        });
        assert.deepEqual(separators, [false, " ", ",", "-", "|", "/"], 'There should be a selection field with proper values');
        assert.strictEqual(tOptions.$('.o_tele_studio_toption_option_contact_separator .o_field_widget option:selected').text(), 'Linebreak',
            'Default value should be "LineBrak"');
        addressSeparator = '","';
        await testUtils.fields.editSelect(tOptions.$('.o_tele_studio_toption_option_contact_separator .o_field_widget'), addressSeparator);
        parent.destroy();
    });
    ...
    QUnit.test('contact: no_marker boolean field', async function (assert) {
        ...
        assert.containsOnce(tOptions, '.o_tele_studio_toption_option_contact_no_marker input:checked',
            "no_marker checkbox is checked initially");
        await testUtils.dom.click(tOptions.$('.o_tele_studio_toption_option_contact_no_marker input'));
