...
from tele.exceptions import ValidationError, AccessError, UserError
...
class StudioApprovalRule(models.Model):
    ...
    def delete_approval(self, res_id):
        ...
        :raise: tele.exceptions.AccessError when the user does not have write
                access to the underlying record
        :raise: tele.exceptions.UserError when any there is no existing entry
    ...
    def _set_approval(self, res_id, approved):
        ...
        # Tele itself (not PG); the NOWAIT ensures that no deadlock will happen
...
class StudioApprovalEntry(models.Model):
    ...
    def _notify_approval(self):
        ...
        for entry in self:
            ...
            template = 'tele_studio.notify_approval'
