...
class TestReportEditor(TransactionCase):
    ...
    def test_copy_inherit_report(self):
        report = self.env['ir.actions.report'].create({
            ...
            'report_name': 'tele_studio.test_inherit_report_user',
            'model': 'res.users',
        })
        self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'tele_studio.test_inherit_report_hi',
            'key': 'tele_studio.test_inherit_report_hi',
            'arch': '''
                <t t-name="tele_studio.test_inherit_report_hi">
                    hi
                </t>
            ''',
        })
        parent_view = self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'tele_studio.test_inherit_report_user_parent',
            'key': 'tele_studio.test_inherit_report_user_parent',
            'arch': '''
                <t t-name="tele_studio.test_inherit_report_user_parent_view_parent">
                    <t t-call="tele_studio.test_inherit_report_hi"/>!
                </t>
            ''',
        })
        self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'tele_studio.test_inherit_report_user',
            'key': 'tele_studio.test_inherit_report_user',
            'arch': '''
        ...
        copy_report = self.env['ir.actions.report'].search([
            ('report_name', '=', 'tele_studio.test_inherit_report_user_copy_1'),
        ])
    ...
    def test_duplicate(self):
        ...
        report = self.env['ir.actions.report'].create({
            'name': 'test duplicate',
            'report_name': 'tele_studio.test_duplicate_foo',
            'model': 'res.users',})

        self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'test_duplicate_foo',
            'key': 'tele_studio.test_duplicate_foo',
            'arch': "<t t-name='tele_studio.test_duplicate_foo'>foo</t>",})

        duplicate_domain = [('report_name', '=like', 'tele_studio.test_duplicate_foo_copy_%')]
    ...
    def test_duplicate_keep_translations(self):
        def create_view(name, **kwargs):
            ...
            name = 'tele_studio.test_keep_translations_{}'.format(name)
        ...
        report = self.env['ir.actions.report'].create({
            'name': 'test inherit report user',
            'report_name': 'tele_studio.test_keep_translations_ab',
            'model': 'res.users',
        ...
        report.copy_report_and_template()
        copy_view = self.env['ir.ui.view'].search([
            ('key', '=', 'tele_studio.test_keep_translations_ab_copy_1'),
        ])
    ...
    def test_report_action_translations(self):
        self.env['ir.actions.report'].create({
            ...
            'report_name': 'tele_studio.test_report_action_translations',
            'model': 'res.users',
        })
        view = self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'test_report_action_translations_view',
            'key': 'tele_studio.test_report_action_translations_view',
