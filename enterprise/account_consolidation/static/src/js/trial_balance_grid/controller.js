    ...
    return WebGridController.extend({
        renderButtons(...args) {
            ...
            if (this.context.default_period_id) {
                const $view_report_btn = $('<button class="btn btn-secondary rounded-sm o_grid_button_view_report" type="button" role="button"/>')
                    .text(_t('Consolidated balance'))
                    .on('click', this._onViewReport.bind(this));
                this.$buttons.prepend($view_report_btn);
                const $add_col_btn = $('<button class="btn btn-primary rounded-sm o_grid_button_add" type="button" role="button"/>')
