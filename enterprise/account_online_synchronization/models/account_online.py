...
from tele.exceptions import UserError, CacheMiss, MissingError, ValidationError
from tele.applets.account_online_synchronization.models.telefin_auth import TeleFinAuth
...
class AccountOnlineAccount(models.Model):
    ...
    def _refresh(self):
        ...
        while True:
            ...
            resp_json = self.account_online_link_id._fetch_tele_fin('/proxy/v1/refresh', data=data)
    ...
    def _retrieve_transactions(self):
        ...
        while True:
            ...
            resp_json = self.account_online_link_id._fetch_tele_fin('/proxy/v1/transactions', data=data)
...
class AccountOnlineLink(models.Model):
    ...
    # Information received from TeleFin, should not be tampered with
    ...
    def _fetch_tele_fin(self, url, data=None, ignore_status=False):
        '''
        Method used to fetch data from the Tele-Fin proxy.
        ...
        endpoint_url = 'https://%s.telefin.com%s' % (proxy_mode, url)
        data['utils'] = {
            'request_timeout': timeout,
            'lang': get_lang(self.env).code,
            'server_version': tele.release.serie,
        ...
        try:
            ...
            resp = requests.post(url=endpoint_url, json=data, timeout=timeout, auth=TeleFinAuth(record=self.sudo()))
            resp_json = resp.json()
    ...
    def _handle_response(self, resp_json, url, data, ignore_status=False):
        ...
        if not resp_json.get('error'):
            ...
            state = result.get('telefin_state') or False
            ...
            if error.get('code') == 101: # access token expired, not an error
                ...
                return self._fetch_tele_fin(url, data, ignore_status)
            ...
            if error_details.get('error_reference'):
                message += '\n' + _('The reference of your error is %s. Please mention it when contacting Tele support.') % error_details['error_reference']
            state = error_details.get('telefin_state') or 'error'
    ...
    def _get_access_token(self):
        for link in self:
            resp_json = link._fetch_tele_fin('/proxy/v1/get_access_token', ignore_status=True)
    ...
    def _get_refresh_token(self):
        ...
        for link in self.sudo():
            resp_json = link._fetch_tele_fin('/proxy/v1/renew_token', ignore_status=True)
    ...
    def unlink(self):
        ...
        for link in self:
            try:
                resp_json = link.with_context(delete_sync=True)._fetch_tele_fin('/proxy/v1/delete_user', data={'provider_data': link.provider_data}, ignore_status=True) # delete proxy user
    ...
    def _fetch_accounts(self, add_new_accounts=True):
        ...
        while True:
            ...
            resp_json = self._fetch_tele_fin('/proxy/v1/accounts', data)
    ...
    def success(self, mode, data):
        ...
        try:
            ...
            message = _("This version of Tele appears to be outdated and does not support the '%s' sync mode. "
    ...
    def exchange_token(self, exchange_token):
        ...
        resp_json = self._fetch_tele_fin('/proxy/v1/exchange_token', data=data, ignore_status=True)
        ...
        action = {
            ...
            'params': {
                ...
                'includeParam': {
                    ...
                    'serverVersion': tele.release.serie
                }
            }
        }
