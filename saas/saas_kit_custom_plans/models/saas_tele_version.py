...
class SaasTeleVersion(models.Model):
    _name = 'saas.tele.version'
    ...
    def unlink(self):
        for obj in self:
            contracts = self.env['saas.contract'].sudo().search([('tele_version_id', '=', obj.id), ('state', '!=', 'cancel')])
            if len(contracts):
                ...
                return super(SaasTeleVersion, self).unlink()
    ...
    def get_default_saas_values(self):
        ...
        data['is_tele_version'] = IrDefault.get('res.config.settings', 'is_tele_version')
