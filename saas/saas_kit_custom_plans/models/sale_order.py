...
class PlanSaleOrder(models.Model):
    ...
    def process_contract(self):
        for order in self:
            ...
            for line in custom_plan_orderlines:
                saas_users = None
                default_data = line.tele_version_id.get_default_saas_values()
                user_billing = 0
                ...
                vals = dict(
                    is_custom_plan=True,
                    tele_version_id=line.tele_version_id.id,
                    ...
                    on_create_email_template=self.env.ref('tele_saas_kit.client_credentials_template').id,
                    is_multi_server=default_data['is_multi_server'],
                    server_id=server_id and server_id.id or False,
                    sale_order_line_id=line.id,
                    plan_id=None,
                    db_template=line.tele_version_id.db_template,
                )
    ...
    def create_custom_contract_line(self, product_id=None, tele_version_id=None, saas_users=None, total_cost=None, users_cost=None, recurring_interval=None, module_ids=None):
        ...
        for line in self.order_line:
            if saas_users and user_product and line.id == custom_contract_user_line_config['line_id']:
                ...
                line.tele_version_id = tele_version_id.id
...
class CustomOrderLine(models.Model):
    ...
    tele_version_id = fields.Many2one(comodel_name='saas.tele.version', string="Tele Version")
