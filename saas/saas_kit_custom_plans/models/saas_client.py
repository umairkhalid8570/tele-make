    ...
    def create_docker_instance(self, domain_name=None):
        if self.saas_contract_id.is_custom_plan:
            ...
            response = saas.main(dict(
                ...
                host_server=host_server,
                db_server=db_server,
                version=self.saas_contract_id.tele_version_id.code)
            )
            return response
        else:
        ...
        for obj in self:
            if obj.saas_contract_id.is_custom_plan:
                if obj.state == "inactive":
                    ...
                    response = client.main(obj.database_name, obj.containter_port, host_server, get_module_resource('tele_saas_kit'), from_drop_db=True, version=self.saas_contract_id.tele_version_id.code or '15')
        ...
        for obj in self:
            if obj.saas_contract_id.is_custom_plan:
                if obj.state == "inactive":
                    host_server, db_server = obj.saas_contract_id.server_id.get_server_details()
                    _logger.info("HOST SERER %r   DB SERVER  %r"%(host_server,db_server))
                    response = client.main(obj.database_name, obj.containter_port, host_server, get_module_resource('tele_saas_kit'), container_id=obj.container_id, db_server=db_server, from_drop_container=True, version=self.saas_contract_id.tele_version_id.code or '15')
