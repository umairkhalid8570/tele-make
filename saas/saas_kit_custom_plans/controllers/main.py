...
class PlanPage(http.Controller):
    ...
    def _create_custom_plan_data(self):
        ...
        data['tele_version'] = request.env['saas.tele.version'].sudo().search([('state', '=', 'confirm')]) if data['is_tele_version'] else False
    ...
    def saas_custom_plan_cart(self, apps=None, saas_users=None, version_name=None, total_cost=None, users_cost=None, recurring_interval=None, version_code=None , **kwargs):
        tele_version_id = request.env['saas.tele.version'].sudo().search([('code', '=', version_code), ('name', '=', version_name)], limit=1)        
        product_id = tele_version_id.product_id        
        module_ids = []
        for module in apps:
        ...
        if sale_order and product_id:
            return sale_order.create_custom_contract_line(product_id=product_id, tele_version_id=tele_version_id, saas_users=saas_users, total_cost=total_cost, users_cost=users_cost, recurring_interval=recurring_interval, module_ids=module_ids)
