...
class SaasContract(models.Model):
    ...
    on_create_email_template = fields.Many2one('mail.template', default=lambda self: self.env.ref('tele_saas_kit.client_credentials_template'), string="Client Creation Email Template")
    ...
    def send_expiry_mail(self):
        for obj in self:
            template = self.env.ref('tele_saas_kit.contract_expiry_template')
    ...
    def send_subdomain_email(self):
        for obj in self:
            ...
            template = self.env.ref('tele_saas_kit.client_subdomain_template')
            mail_id = template.send_mail(obj.id)
            current_mail = self.env['mail.mail'].browse(mail_id)
            current_mail.send()
    ...
    def add_custom_domain(self, domain=None, is_ssl=False):
        ...
        module_path = get_module_resource('tele_saas_kit')
