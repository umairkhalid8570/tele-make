...
class SaasPlans(models.Model):
    ...
    def action_view_contracts(self):
        ...
        action = self.env.ref('tele_saas_kit.saas_contract_action').read()[0]
        if len(contracts) > 1:
            action['domain'] = [('id', 'in', contracts.ids)]
        elif len(contracts) == 1:
            action['views'] = [(self.env.ref(
                'tele_saas_kit.saas_contract_form_view').id, 'form')]
    ...
    use_specific_user_template = fields.Boolean(
        string="Use Specific User Template", help="""Select if you want to provide some specific permissions to your user for acessing its tele instance which is going to be created by this plan.""")
    template_user_id = fields.Char(string="Database Template User ID", help="""Enter the user_id of User which you have created in the DB Template with some specific permissions or whose permission you want to grant to the user of tele instances which is going to be created by this plan.""")
    ...
    db_template = fields.Char(
        compute='_compute_db_template_name', string="DB Template Name", store=True, help="Enter a uniquie name to create a DB associated to this plan or leave it blank and let tele to give it a unique name.")
    ...
    def create_db_template(self):
        for obj in self:
            ...
            config_path = get_module_resource('tele_saas_kit')
            modules.append('tele_saas_tool')
            try:
                host_server, db_server = obj.server_id.get_server_details()
