...
QUnit.test("create slots for custom appointment type", async function (assert) {
        ...
        navigator: {
            clipboard: {
                writeText: (value) => {
                    assert.strictEqual(
                        value,
                        `http://amazing.tele.studio/calendar/3?filter_employee_ids=%5B${session.uid}%5D`
                    );
                }
            }
        }
    ...
    const calendar = await createCalendarView({
        ...
        session: {
            'web.base.url': 'http://amazing.tele.studio',
        },
    }, { positionalClicks: true });
        ...
        navigator: {
            clipboard: {
                writeText: (value) => {
                    assert.strictEqual(
                        value,
                        `http://amazing.tele.studio/calendar/2?filter_employee_ids=%5B${session.uid}%5D`
                    );
                }
            }
        }
