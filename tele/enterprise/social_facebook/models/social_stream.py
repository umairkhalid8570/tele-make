...
class SocialStreamFacebook(models.Model):
    ...
    def _format_facebook_message(self, message, tags):
        ...
        example:
            "Hello Tele :)" -> "Hello @[542132] Tele :)"
            "Hello Tele Social :)" -> "Hello @[542132] Tele-Social :)"
            "Hello Tele - Social :)" -> "Hello @[542132] Tele-Social :)"

        details:
            "Hello @[42] faketag Tele - Social :)" -> "Hello @ [42] faketag @[542132] Tele-Social :)""
            -> With associated tag: {name: "Tele - Social", offset: 20, length: 13}

            We take the message until the tag offset: "Hello @[42] faketag "
            and remove the fake tag in the process "Hello @ [42] faketag "
            Then we insert the forged tag "@[542132] Tele-Social"
