...
DEFAULT_ENDPOINT = 'https://ocn.tele.studio'
...
class ResConfigSettings(models.TransientModel):
    ...
    def _get_endpoint(self):
        return self.env['ir.config_parameter'].sudo().get_param('tele_ocn.endpoint', DEFAULT_ENDPOINT)
    ...
    def get_fcm_project_id(self):
        ...
        if not ir_params_sudo.get_param('mail_mobile.enable_ocn'):
            return
        project_id = ir_params_sudo.get_param('tele_ocn.project_id')
        if not project_id:
            params = {
            ...
            try:
                ...
                ir_params_sudo.set_param('tele_ocn.project_id', project_id)
