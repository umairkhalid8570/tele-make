...
from tele.applets.sale_amazon.lib.mws.mws import DataWrapper, DictWrapper, MWS, MWSError, XMLError, \
     remove_empty
...
def make_request_patch(self, extra_data, method="GET", **kwargs):
    ...
    # =============================================================================================
    # From here, the library is patched to make the request to the Tele proxy rather than to Amazon
    # =============================================================================================
    ...
    del params['AWSAccessKeyId']  # Use Tele's credentials to sign requests
    url = self.proxy_url
    payload = {
    ...
    try:
        ...
        response = requests.post(url, data=payload, timeout=60)  # Send request to Tele proxy
        ...
        # to the lib, raise an tele exception directly
