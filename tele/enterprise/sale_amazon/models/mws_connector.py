...
from tele import _, exceptions

from tele.applets.sale_amazon.lib import mws
...
def _is_sent_by_amazon(mws_error):
    ...
    """
    if 700 <= mws_error.response.status_code <= 799:  # Custom Tele proxy error
        _logger.exception(
        ...
        if mws_error.response.status_code == 730:  # Forbidden
            raise exceptions.UserError(
                ...
                'https://www.tele.studio/buy'
            )
        elif mws_error.response.status_code == 740:  # Bad Request
            raise exceptions.UserError(_("The Tele proxy received a malformed request."))
        elif mws_error.response.status_code == 750:  # Internal Error
            raise exceptions.UserError(_("The Tele proxy encountered an internal server error."))
        else:  # This is unexpected, we should probably have a custom message for this code
            raise exceptions.UserError(_("The Tele proxy encountered an unhandled error."))
