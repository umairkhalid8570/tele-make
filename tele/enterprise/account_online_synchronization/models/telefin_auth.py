...
class TeleFinAuth(requests.auth.AuthBase):
    """ This is used to sign the request going towards TeleFin
        e.g.:
            requests.get(TELEFIN + '/example', auth=TeleFinAuth())
            By using `auth=TeleFinAuth(self)` when doing a http request, the request will be signed and
    ...
    def __call__(self, request):
        ...
        request.headers.update({
            'telefin-client-id': self.client_id,
            'telefin-access-token': self.access_token,
            'telefin-signature': base64.b64encode(h.digest()),
            'telefin-timestamp': msg_timestamp,
        })
        return request
