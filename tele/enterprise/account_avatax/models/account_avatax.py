...
from tele.applets.account_avatax.lib.avatax_client import AvataxClient
from tele import models, fields, api, registry, _, SUPERUSER_ID
from tele.release import version
from tele.exceptions import UserError, RedirectWarning, ValidationError
from tele.tools import DEFAULT_SERVER_DATETIME_FORMAT, float_round, float_repr
...
class AccountAvatax(models.AbstractModel):
    ...
    def _map_avatax(self, commit):
        """Link Avatax response to Tele's models.
        ...
        for document, query_result in query_results.items():
            error = self._handle_response(query_result, _(
                'Tele could not fetch the taxes related to %(document)s.\n'
    ...
    def _uncommit_avatax_transaction(self):
        ...
        for record in self:
            ...
            error = self._handle_response(query_result, _(
                'Tele could not change the state of the transaction related to %(document)s in'
                ' AvaTax\nPlease check the status of `%(technical)s` in the AvaTax portal.',
    ...
    def _void_avatax_transaction(self):
        for record in self:
            ...
            error = self._handle_response(query_result, _(
                'Tele could not void the transaction related to %(document)s in AvaTax\nPlease '
