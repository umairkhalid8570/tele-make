...
class TestAccountAvalaraInternal(TestAccountAvataxCommon):
    ...
    def test_01_tele_invoice(self):
        ...
        with patch('tele.applets.account_avatax.models.account_avatax.AccountAvatax._uncommit_avatax_transaction') as mocked_commit:
            invoice.button_draft()
            mocked_commit.assert_called()

    def test_integration_01_tele_invoice(self):
    ...
    def test_integration_02_tele_invoice(self):
        with self._skip_no_credentials():
            invoice, _ = self._create_invoice_02_and_expected_response()
            self.assertInvoice(invoice, test_exact_response=False)
            invoice.button_draft()

    def test_01_tele_refund(self):
        invoice, response = self._create_invoice_01_and_expected_response()
    ...
    def test_invoice_multi_company(self):
        ...
        with self._capture_request(return_value=response):
            ...
            # tele.exceptions.ValidationError
...
class TestAccountAvalaraSalesTaxAdministration(TestAccountAvataxCommon):
    ...
    def test_disable_document_recording(self):
        ...
        invoice, response = self._create_invoice_01_and_expected_response()
        with self._capture_request(return_value=response), patch('tele.applets.account_avatax.lib.avatax_client.AvataxClient.commit_transaction') as mocked_commit:
            invoice.action_post()
    ...
    def test_disable_avatax(self):
        ...
        with patch('tele.applets.account_avatax.lib.avatax_client.AvataxClient.request') as mocked_request:
