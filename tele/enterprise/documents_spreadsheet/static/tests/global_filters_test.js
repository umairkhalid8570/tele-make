/** @tele-module alias=documents_spreadsheet.PivotGlobalFilterTests */
/* global $ moment */
...
module(
    ...
    () => {
        ...
        test("FILTER.VALUE formulas are updated when filter label is changed", async function (assert) {
            ...
            setCellContent(model, "A10", `=FILTER.VALUE("Cuill??re") & FILTER.VALUE( "Cuill??re" )`);
            const [filter] = model.getters.getGlobalFilters();
            const newFilter = {
                type: "date",
                label: "Interprete",
        ...
        test("Can undo-redo a EDIT_GLOBAL_FILTER", async function (assert) {
            ...
            model.dispatch("REQUEST_UNDO");
            assert.equal(model.getters.getGlobalFilters()[0].label, "Cuill??re");
            model.dispatch("REQUEST_REDO");
            assert.equal(model.getters.getGlobalFilters()[0].label, "Arthouuuuuur");
        });
