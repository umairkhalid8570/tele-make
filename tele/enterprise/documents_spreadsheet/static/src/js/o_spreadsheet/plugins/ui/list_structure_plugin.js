...
export default class ListStructurePlugin extends spreadsheet.UIPlugin {
    ...
    handle(cmd) {
        switch (cmd.type) {
            case "START":
                this._refreshTeleLists();
                break;
            case "BUILD_TELE_LIST":
            ...
            case "SELECT_TELE_LIST":
                this._selectList(cmd.listId);
                break;
            case "REFRESH_TELE_LIST":
                this._refreshTeleList(cmd.listId);
                break;
    ...
    _handleBuildList(sheetId, anchor, linesNumber, list, types) {
        this.dispatch("ADD_TELE_LIST", { list });
    ...
    _buildList(sheetId, anchor, listId, linesNumber, types) {
        ...
        for (const field_name of columns) {
            this.dispatch("ADD_TELE_LIST_FORMULA", {
                col,
                row,
                sheetId,
                formula: "LIST.HEADER",
