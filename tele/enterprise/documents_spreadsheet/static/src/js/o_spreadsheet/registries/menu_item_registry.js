/** @tele-module alias=documents_spreadsheet.MenuItemRegistry */

import { _t, _lt } from "web.core";
import { getFirstPivotFunction, getNumberOfPivotFormulas } from "../helpers/tele_functions_helpers";
    ...
    children: function (env) {
        ...
        const lists = env.getters.getListIds().map((listId, index) => {
            return createFullMenuItem(`item_list_${listId}`, {
                ...
                action: (env) => {
                    env.dispatch("SELECT_TELE_LIST", { listId: listId });
...
cellMenuRegistry
    ...
    .add("listing_properties", {
        ...
        action(env) {
            ...
            env.dispatch("SELECT_TELE_LIST", { listId });
            env.openSidePanel("LIST_PROPERTIES_PANEL", {});
        },
        isVisible: (env) => {
            const [col, row] = env.getters.getPosition();
