...
from tele.applets.sale_ebay.tools.ebaysdk import Trading
        ...
        if self.attribute_line_ids:
            for attribute in self.attribute_line_ids:
                if len(attribute.value_ids) == 1:
                    ...
                    # We used the attributes in Tele to match the Brand and MPN attributes
    ...
    def _sync_available_qty(self):
        if self.ebay_sync_stock:
            if self.ebay_listing_status in ['Active', 'Error']:
                ...
                # Check if the quantity in Tele is different than the one on eBay
                ...
                # The product is Out Of Stock on eBay but there is stock in Tele
