...
from tele.applets.account_avatax.tests.common import TestAccountAvataxCommon
...
class TestSaleAvalara(TestAccountAvataxCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        # and not the tax values Tele computes (these values would be
    ...
    def test_01_tele_sale_order(self):
        order = self._create_sale_order()
        mocked_response = generate_response(order.order_line)
        with self._capture_request(return_value=mocked_response):
    ...
    def test_integration_01_tele_sale_order(self):
