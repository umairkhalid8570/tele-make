...
from tele.applets.account_invoice_extract.models.account_invoice import SUCCESS, NOT_READY, ERROR_INTERNAL, WARNING_DUPLICATE_VENDOR_REFERENCE
from tele.applets.account_invoice_extract.tests import common as account_invoice_extract_common
...
class TestInvoiceExtract(AccountTestInvoicingCommon, account_invoice_extract_common.MockIAP):
    ...
    def test_tax_adjustments(self):
        # test that if the total computed by Tele doesn't exactly match the total found by the OCR, the tax are adjusted accordingly
        for move_type in ('in_invoice', 'out_invoice'):
            self.env['res.currency'].search([('name', '!=', 'USD')]).active = False
            invoice = self.env['account.move'].create({'move_type': move_type, 'extract_state': 'waiting_extraction'})
            extract_response = self.get_default_extract_response()
