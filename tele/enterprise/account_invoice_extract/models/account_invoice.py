...
from tele import api, fields, models, tools, _, _lt
from tele.applets.iap.tools import iap_tools
from tele.exceptions import AccessError, ValidationError , UserError
from tele.tests.common import Form
from tele.tools import float_compare, mute_logger
from tele.tools.misc import clean_context
import logging
import math
import re
import json
...
PARTNER_AUTOCOMPLETE_ENDPOINT = 'https://partner-autocomplete.tele.studio'
EXTRACT_ENDPOINT = 'https://iap-extract.tele.studio'
...
ERROR_MESSAGES = {
    ...
    ERROR_INVALID_ACCOUNT_TOKEN: _lt("The 'invoice_ocr' IAP account token is invalid. Please delete it to let Tele generate a new one or fill it with a valid token."),
}
WARNING_MESSAGES = {
    ...
    def _save_form(self, ocr_results, no_ref=False):
        ...
        with self.get_form_context_manager() as move_form:
            ...
            if self.move_type in {'out_invoice', 'out_refund'}:
                with mute_logger('tele.tests.common.onchange'):
            ...
            if not move_form.invoice_line_ids:
                ...
                # if the total on the invoice doesn't match the total computed by Tele, adjust the taxes so that it matches
