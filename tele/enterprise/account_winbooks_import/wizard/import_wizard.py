...
from tele.exceptions import UserError, RedirectWarning
...
class WinbooksImportWizard(models.TransientModel):
    ...
    suspense_code = fields.Char(string="Suspense Account Code", help="This is the code of the account in which you want to put the counterpart of unbalanced moves. This might be an account from your Winbooks data, or an account that you created in Tele before the import.")
    ...
    def _import_partner(self, dbf_records, civility_data, category_data, account_data):
        ...
        :return: a dictionary whose keys are the Winbooks partner references and
            the values are the partner ids in Tele.
        """
        ...
        if shared_iban:
            message = _(
                ...
                "not allowed in Tele. The bank number has been only set on one of each group:\n%s",
                "\n".join(
                    "%(bank)s : %(partners)s" % {
                        'bank': iban,
    ...
    def _import_account(self, dbf_records):
        ...
        :return: (account_data, account_central, account_deprecated_ids, account_tax)
            account_data is a dictionary whose keys are the Winbooks account
                references and the values are the account ids in Tele.
            account_central is a dictionary whose keys are the Winbooks central
                account references and the values are the account ids in Tele.
    ...
    def _import_journal(self, dbf_records):
        ...
        :return: a dictionary whose keys are the Winbooks journal references and
            the values the journal ids in Tele
        """
            ...
            try:
                ...
                if len(lines.account_id) > 1:
                    _logger.warning(
                        ...
                        'Lines with that number have not been reconciled in Tele.',
                        matching_number,
    ...
    def _import_analytic_account(self, dbf_records):
        ...
        references and the values the analytic account ids in Tele.
        """
    ...
    def _import_vat(self, dbf_records, account_central):
        ...
        the values are the taxes ids in Tele.
        """
