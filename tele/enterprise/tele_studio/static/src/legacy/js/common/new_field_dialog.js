tele.define('tele_studio.NewFieldDialog', function (require) {
"use strict";
...
var NewFieldDialog = Dialog.extend(StandaloneFieldManagerMixin, {
    template: 'tele_studio.NewFieldDialog',
    events: {
        'keyup .o_tele_studio_selection_new_value > input': '_onAddSelectionValue',
        'click .o_tele_studio_edit_selection_value': '_onEditSelectionValue',
        'click .o_tele_studio_remove_selection_value': '_onRemoveSelectionValue',
        'click .o_tele_studio_add_selection_value': '_onAddSelectionValue',
        'click .o_tele_studio_clear_selection_value': '_onClearSelectionValue',
        'blur .o_tele_studio_selection_editor .o_tele_studio_selection_input': '_onSelectionInputBlur',
    },
    /**
     * @constructor
    ...
    renderElement: function () {
        ...
        if (this.type === 'selection') {
           this.$('.o_tele_studio_selection_editor').sortable({
                axis: 'y',
                containment: '.o_tele_studio_field_dialog_form',
                items: '> li',
                helper: 'clone',
                handle: '.input-group',
                opacity: 0.6,
    ...
    start: function() {
        ...
        this.$modal.addClass('o_tele_studio_field_modal');

        if (this.type === 'selection') {
            // Focus on the input responsible for adding new selection value
            this.opened().then(function () {
                self.$('.o_tele_studio_selection_new_value > input').focus();
            });
    ...
    _resequenceSelection: function () {
        var self = this;
        var newSelection = [];
        this.$('.o_tele_studio_selection_editor li').each(function (index, u) {
    ...
    _onAddSelectionValue: function (e) {
        ...
        var $input = this.$(".o_tele_studio_selection_new_value input");
        ...
        this.renderElement();
        this.$('.o_tele_studio_selection_new_value > input').focus();
    },
    /**
     * @private
     * @param {Event} ev
    ...
    _onEditSelectionValue: function (ev) {
        ...
        if (config.isDebug()) {
            ...
            new Dialog(this, {
                title: _t('Edit Value'),
                size: 'small',
                $content: $(qweb.render('tele_studio.SelectionValues.edit', {
            ...
            var $input = $btn.closest('li').find('.o_tele_studio_selection_input.d-none');
            var $span = $input.siblings('.o_tele_studio_selection_label');
    ...
    _onClearSelectionValue: function () {
        this.$('.o_tele_studio_selection_input').val("").focus();
    },
    /**
     * @private
     */
    _onSave: function () {
        ...
        if (this.type === 'one2many') {
            ...
            var newSelection = this.$('.o_tele_studio_selection_new_value > input').val();
            if (newSelection) {
