tele.define('tele_studio.FormEditor', function (require) {
"use strict";
...
var FieldSelectorDialog = require('tele_studio.FieldSelectorDialog');
...
var FormEditor =  FormRenderer.extend(EditorMixin, {
    ...
    className: FormRenderer.prototype.className + ' o_tele_studio_form_view_editor',
    events: _.extend({}, FormRenderer.prototype.events, {
        'click .o_tele_studio_add_chatter': '_onAddChatter',
    }),
    ...
    highlightNearestHook: function ($helper, position) {
        ...
        var $nearest_form_hooks = this.$('.o_tele_studio_hook')
            .touching({
        ...
        $nearest_form_hooks.each(function () {
            ...
            var whitelist = ['o_tele_studio_field_picture', 'o_tele_studio_field_html',
                'o_tele_studio_field_many2many', 'o_tele_studio_field_one2many',
                'o_tele_studio_field_tabs', 'o_tele_studio_field_columns', 'o_tele_studio_field_lines'];
            ...
            if (is_nearest_hook) {
                $(this).addClass('o_tele_studio_nearest_hook');
                return false;
            }
        });
            ...
            _.each(elements, function (element) {
                if (element.$el.hasClass('o_invisible_modifier')) {
                    element.$el
                        ...
                        .addClass('o_tele_studio_show_invisible');
                }
            });
        }
    ...
    _handleDrop: function (ev, ui) {
        var $hook = this.$('.o_tele_studio_nearest_hook');
        if ($hook.length) {
            ...
            $hook.removeClass('o_tele_studio_nearest_hook');
        }
    },
    /**
    ...
    _postProcessField: function (widget, node) {
        ...
        if (!widget.isSet() && (!node.has_label || node.attrs.nolabel)) {
            widget.$el.removeClass('o_field_empty').addClass('o_tele_studio_widget_empty');
    ...
    _render: function () {
        ...
        this.$el.droppable({
            accept: ".o_tele_studio_component",
        ...
        return this._super.apply(this, arguments).then(async function () {
            ...
            if (!self._hasChatter() && self.chatter_allowed) {
                var $chatter_hook = $('<div>').addClass('o_tele_studio_add_chatter o_chatter');
            ...
            if (!self.$('.oe_button_box').length) {
                var $buttonbox_hook = $('<button>')
                    .addClass('btn oe_stat_button o_tele_studio_button_hook')
            ...
            if (!self.$('.o_statusbar_status').length) {
                var $statusbar = $('<div>', {
                    ...
                    class: 'o_tele_studio_statusbar_hook',
                }).click(function () {
                    var values = {
            ...
            if (self.$('.oe_title').length & !self.$('.oe_title').siblings('.oe_avatar').length && !self.$('.oe_title > h1 > .oe_avatar').length) {
                const $avatar = $('<div>', {
                    text: _t("Add Picture"),
                    class: self.$('.oe_title > h1.d-flex.flex-row').length ? 'oe_avatar ml-3 p-3 o_tele_studio_avatar h4': 'oe_avatar ml-3 mr-3 o_tele_studio_avatar',
                }).click(function () {
    ...
    _renderButtonBox: function () {
        ...
        var $buttonhook = $('<button>').addClass('btn oe_stat_button o_tele_studio_button_hook');
                ...
                Promise.all(self.defs).then(function () {
                    ...
                    if (!$widget.is('.o_invisible_modifier')) {
                        self._renderAddingContentLine(child).then(function(element) {
                            ...
                            // apply to the entire <tr> o_tele_studio_show_invisible
                            // rather then inner label/input
                            if ($widget.hasClass('o_tele_studio_show_invisible')) {
                                $widget.removeClass('o_tele_studio_show_invisible');
                                $tr.find('label[for="' + $widget.attr('id') + '"]').removeClass('o_tele_studio_show_invisible');
                                $tr.addClass('o_tele_studio_show_invisible');
                            }
                        });
                    }
                    if (child.has_label) {
    ...
    async _renderView() {
        ...
        if (this._hasChatter()) {
            ...
            $el.append($('<div>', { 'class': 'o_tele_studio_overlay' }));
            $el.attr('data-node-id', this.node_id++);
            $el.click(() => {
    ...
    _setDraggable: function (node, $el) {
        ...
        $el.draggable({
            ...
            start: function (e, ui) {
                self.$('.o_tele_studio_hovered').removeClass('o_tele_studio_hovered');
                self.$('.o_tele_studio_clicked').removeClass('o_tele_studio_clicked');
                ui.helper.addClass('ui-draggable-helper');
                ui.helper.data('name', node.attrs.name);
            },
            revert: function () {
                // a field cannot be dropped on the same place
                var $hook = self.$('.o_tele_studio_nearest_hook');
                if ($hook.length) {
                    var hook_id = $hook.data('hook_id');
                    var hook = self.hook_nodes[hook_id];
