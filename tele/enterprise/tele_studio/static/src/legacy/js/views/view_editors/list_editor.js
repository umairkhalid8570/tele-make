tele.define('tele_studio.ListEditor', function (require) {
"use strict";
...
return ListRenderer.extend(EditorMixin, {
    ...
    className: ListRenderer.prototype.className + ' o_tele_studio_list_view_editor',
    events: _.extend({}, ListRenderer.prototype.events, {
        'click th:not(.o_tele_studio_hook), td:not(.o_tele_studio_hook)': '_onExistingColumn',
    }),
    ...
    highlightNearestHook: function ($helper, position) {
        ...
        var $nearest_list_hook = this.$('.o_tele_studio_hook')
            .touching({
        ...
        if ($nearest_list_hook.length) {
            ...
            $elements.addClass('o_tele_studio_nearest_hook');
            return true;
        }
        return false;
    },
    ...
    setSelectable: function ($el) {
        ...
        $el.click(function (ev) {
            ...
            $elements.addClass('o_tele_studio_clicked');
        })
        .mouseover(function (ev) {
            ...
            var $target = $(ev.currentTarget);
            var $elements = self._getColumnElements($target);
            $elements.addClass('o_tele_studio_hovered');
        })
        .mouseout(function () {
    ...
    _render: function () {
        ...
        prom.then(function () {
            ...
            self.setSelectable(self.$('th, td').not('.o_tele_studio_hook'));
    ...
    _renderBodyCell(record, node, ...args) {
        ...
        if (invisibleTechnicalNames.includes(node.attrs.name)) {
            $td.addClass('o_tele_studio_show_invisible');
        }
        return $td;
    },
    /**
    ...
    _renderHeaderCell: function (node) {
        ...
        if (_.contains(this.invisible_columns, node)) {
            $th.addClass('o_tele_studio_show_invisible');
        }
        return $th;
    },
    /**
