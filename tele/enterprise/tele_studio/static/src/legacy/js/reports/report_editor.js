tele.define('tele_studio.ReportEditor', function (require) {
"use strict";
...
var ReportEditor = Widget.extend(EditorMixin, {
    template: 'tele_studio.ReportEditor',
    ...
    beginDragComponent: function (component) {
        ...
        if (this.$content.find('.o_tele_studio_hook').length === 0) {
    ...
    */
    _prepareHooksOnIframeBeforeDrag: function (component) {
        ...
        if (component.dropColumns) {
            ...
            var $gridHooks = $('<div class="row o_tele_studio_structure_hook"/>');
            ...
            var $children = $page.children().not('.o_tele_studio_hook,p:not([data-oe-id])');

            if ($children.length) {
                $gridHooks.find('.o_tele_studio_hook').data('oe-node', $children.first()).data('oe-position', 'before');
                $children.first().before($gridHooks);

                _.each($children, function (child) {
                    var $child = $(child);
                    var $newHook = $gridHooks.clone();
                    $newHook.find('.o_tele_studio_hook').data('oe-node', $child).data('oe-position', 'after');
                    $child.after($newHook);
                });
            } else {
                $gridHooks.find('.o_tele_studio_hook').data('oe-node', $page).data('oe-position', 'inside');
                $page.prepend($gridHooks);
            }

            this.$content.find('.o_tele_studio_structure_hook + .o_tele_studio_hook').remove();
            this.$content.find('.o_tele_studio_structure_hook').prev('.o_tele_studio_hook').remove();
        }
        this.$content.find('.o_tele_studio_hook + .o_tele_studio_hook').remove();
        this.$dropZone = this.$content.find('.o_tele_studio_hook');

        this.$dropZoneStructure = this.$content.find('.o_tele_studio_structure_hook');
        this.$dropZoneStructure.removeClass('.o_tele_studio_nearest').each(function () {
            $(this).children().children('.o_tele_studio_hook:only-child').data('height', $(this).height() + 'px');
        });
    ...
    dragComponent: function (component, x, y) {
        this.isDragging = true;
        this.$dropZone
            .filter('.o_tele_studio_nearest_hook')
            .removeClass('o_tele_studio_nearest_hook')
            .closest(this.$dropZoneStructure).each(function () {
                $(this).children().css('height', '').children('.o_tele_studio_hook:only-child').css('height', '');
            });

        this.$dropZoneStructure.removeClass('o_tele_studio_nearest');
        ...
        $nearestHook
            .addClass('o_tele_studio_nearest_hook')
            .closest(this.$dropZoneStructure)
            .addClass('o_tele_studio_nearest');
        ...
        $nearestHooks.addClass('o_tele_studio_nearest_hook');
    },
    /**
    ...
    dropComponent: function (component) {
        this.isDragging = false;
        var $nearestHooks = this.$dropZone.filter('.o_tele_studio_nearest_hook');
        var targets = [];
    ...
    highlight: function (node) {
        if (!this.$highlight) {
            ...
            this.$highlight = $('<span class="o_tele_studio_report_highlight"/>');
        ...
        if (this.$targetHighlight.data('node') !== this.selectedNode) {
            ...
            this.$targetHighlight.removeClass('o_tele_studio_report_selected');
        }

        var $nodes = this._findAssociatedDOMNodes(node);
        if ($nodes && $nodes.length) {
            this.$targetHighlight = $nodes.addClass('o_tele_studio_report_selected');
            ...
            this.$highlight
                ...
                .toggleClass('o_tele_studio_report_highlight_left', position.left < 50)
                .toggleClass('o_tele_studio_report_highlight_top', position.top < 50)
                .show();
        } else {
            this.$highlight.hide();
        }
    ...
    selectNode: function (node) {
        if (this.selectedNode) {
            ...
            $oldSelectedNodes.removeClass('o_tele_studio_report_selected');
        }

        this.selectedNode = node;
        var $nodesToHighlight = this._findAssociatedDOMNodes(this.selectedNode);
        $nodesToHighlight.addClass('o_tele_studio_report_selected');
    },
    /**
     * @override
     */
    unselectedElements: function () {
        ...
        $nodes.removeClass('o_tele_studio_report_selected');
        this.selectedNode = null;
    },
    /**
    ...
    _cleanHooks: function () {
        ...
        this.$content.find('.o_tele_studio_hook').remove();
        this.$content.find('.o_tele_studio_structure_hook').remove();
    ...
    _createHookOnNodeAndChildren: function ($node, component) {
        ...
        var $children = $node.children()
            .not('.o_tele_studio_hook,p:not([data-oe-id])')
    ...
    _createHook: function ($target, component) {
        ...
        var $hook = $('<' + hookTag + ' class="o_tele_studio_hook"/>');
    ...
    _processReportPreviewContent: function () {
        ...
        this._connectNodes();
        this.$('.o_tele_studio_loader').hide();
        this._resizeIframe();
