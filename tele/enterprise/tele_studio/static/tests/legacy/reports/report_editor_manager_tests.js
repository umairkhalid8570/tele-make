tele.define('tele_studio.ReportEditorManager_tests', function (require) {
"use strict";
...
var loadIframeCss = function (callback) {
    return function WrapLoadIframeCss(assert) {
        ...
        session.rpc('/tele_studio/edit_report/test_load_assets').then(function (assets) {
...
QUnit.module('ReportEditorManager', {
    ...
    QUnit.test('empty editor rendering', async function (assert) {
        ...
        var rem = await studioTestUtils.createReportEditorManager({
            ...
            mockRPC: function (route, args) {
                if (route === '/tele_studio/print_report') {
        ...
        await rem.editorIframeDef.then(async function () {
            assert.containsOnce(rem, '.o_tele_studio_sidebar',
            ...
            // printing the report
            assert.containsOnce(rem, '.o_tele_studio_report_print',
                "it should be possible to print the report");
            await testUtils.dom.click(rem.$('.o_tele_studio_report_print'));
    ...
    QUnit.test('basic editor rendering', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            var iframeContainerwidth = getFloatSizeFromPropertyInPixels(rem.$('.o_tele_studio_report_iframe_container'),'width');
            assert.ok(Math.abs(iframeContainerwidth - 794) <= 1,"the default width should be A4 (794px = 210mm) +/- 1px");

            var iframeMinHeight = getFloatSizeFromPropertyInPixels(rem.$('.o_tele_studio_report_iframe_container'), 'min-height');
            ...
            assert.hasClass(rem.$('iframe').contents().find('span:contains(Second)'),'o_tele_studio_report_selected',
                "the corresponding nodes should be selected");
            assert.hasClass(rem.$('.o_tele_studio_sidebar .o_tele_studio_sidebar_header div[name="options"]'),'active',
                "the sidebar should have been updated");
            assert.containsN(rem, '.o_tele_studio_sidebar .o_tele_studio_sidebar_content .card', 2,
            ...
            assert.hasClass(rem.$('iframe').contents().find('span:contains(First span)'),'o_tele_studio_report_selected',
                "the corresponding nodes should be selected");
            assert.containsN(rem, '.o_tele_studio_sidebar .o_tele_studio_sidebar_content .card', 3,
                "there should be 3 cards in the sidebar");
            // click 2nd card on "Options" tab
            await testUtils.dom.click(rem.$('.o_tele_studio_sidebar .o_tele_studio_sidebar_content_properties div.card:eq(1)'));
            assert.hasClass(rem.$('iframe').contents().find('div.class1'), 'o_tele_studio_report_selected',
                "the corresponding nodes should be selected");
            assert.doesNotHaveClass(rem.$('iframe').contents().find('span:contains(First span)'), 'o_tele_studio_report_selected',
                "the corresponding node should not be selected now");

            // click on "Options" (shouldn't do anything)
            await testUtils.dom.click(rem.$('.o_tele_studio_sidebar .o_tele_studio_sidebar_header div[name="options"]'));
            assert.containsN(rem, '.o_tele_studio_sidebar .o_tele_studio_sidebar_content .card', 3,
    ...
    QUnit.test('editor rendering with paperformat', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            var iframeWidth = getFloatSizeFromPropertyInPixels(rem.$('.o_tele_studio_report_iframe_container'),'width');
            assert.ok(Math.abs(iframeWidth-756) <= 1,"the width should be taken from the paperFormat +/- 1px");

            var iframeHeight = getFloatSizeFromPropertyInPixels(rem.$('.o_tele_studio_report_iframe_container'), 'min-height');
    ...
    QUnit.test('use pager', async function (assert) {
        ...
        var rem = await studioTestUtils.createReportEditorManager({
            ...
            reportMainViewID: 42,
            mockRPC: function (route, args) {
                if (route === '/tele_studio/get_report_views') {
        ...
        await rem.editorIframeDef.then(async function () {
            assert.strictEqual(rem.$('iframe').contents().find('.page').text(), "First span",
                "the iframe should be rendered");
            assert.containsOnce(rem, '.o_tele_studio_report_pager',
                "there should be a pager");
            assert.strictEqual(rem.$('.o_tele_studio_report_pager').text().trim(), "1 / 2",
                "the pager should be correctly rendered");

            // click to switch between records
            await testUtils.dom.click(rem.$('.o_tele_studio_report_pager .o_pager_next'));

            assert.strictEqual(rem.$('iframe').contents().find('.page').text(), "hello",
                "the iframe should be updated");
            assert.strictEqual(rem.$('.o_tele_studio_report_pager').text().trim(), "2 / 2",
    ...
    QUnit.test('components edition', async function (assert) {
        ...
        var rem = await studioTestUtils.createReportEditorManager({
            ...
            reportMainViewID: 42,
            mockRPC: function (route, args) {
                if (route === '/tele_studio/edit_report_view') {
        ...
        var $textarea = rem.$('.o_tele_studio_sidebar .o_tele_studio_active textarea[name="text"]');
        ...
        var $newTextarea = rem.$('.o_tele_studio_sidebar .o_tele_studio_active textarea[name="text"]');
    ...
    QUnit.test('components edition 2', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            assert.hasAttrValue(rem.$('.o_tele_studio_sidebar_header .active'), 'name', 'new',
                "the 'Add' tab should be active");
            assert.strictEqual(rem.$('iframe').contents().find('.o_tele_studio_report_selected').length, 0,
                "there should be no selected node");

            // click to edit a span
            await testUtils.dom.click(rem.$('iframe').contents().find('span:contains(First)'));
            assert.hasAttrValue(rem.$('.o_tele_studio_sidebar_header .active'), 'name', 'options',
                "the 'Options' tab should be active");
            assert.strictEqual(rem.$('iframe').contents().find('.o_tele_studio_report_selected').length, 1,
                "the span should be selected");

            // switch tab
            await testUtils.dom.click(rem.$('.o_tele_studio_sidebar_header [name="report"]'));
            assert.hasAttrValue(rem.$('.o_tele_studio_sidebar_header .active'), 'name', 'report',
    ...
    QUnit.test('remove components - when no node is available to select, the add tab is activated', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            await testUtils.dom.click(rem.$('.o_tele_studio_active .o_tele_studio_remove'));
    ...
    QUnit.test('drag & drop text component', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            await testUtils.dom.click(rem.$('.o_tele_studio_sidebar .o_tele_studio_sidebar_header div[name="new"]'));

            // drag and drop a Text component, which should trigger a view edition
            var $text = rem.$('.o_tele_studio_sidebar .o_tele_studio_field_type_container:eq(1) .o_tele_studio_component:contains(Text)');
    ...
    QUnit.test('drag & drop text component in existing col', loadIframeCss(async function (assert, done) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            // drag and drop a Text component, which should trigger a view edition
            var $text = rem.$('.o_tele_studio_sidebar .o_tele_studio_component:contains(Text):eq(1)');
    ...
    QUnit.test('drag & drop components and cancel', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            var $text = rem.$('.o_tele_studio_sidebar .o_tele_studio_component:contains(Field):eq(1)');
            await testUtils.dom.dragAndDrop($text, rem.$('iframe').contents().find('.col-3:last'));
            assert.strictEqual($('.o_tele_studio_field_modal').length, 1, "a field modal should be opened");

            // cancel the field selection
            await testUtils.dom.click($('.o_tele_studio_field_modal .btn-secondary'));
            assert.strictEqual(rem.$('iframe').contents().find('.o_tele_studio_hook').length, 0, "Must cancel the dragAndDrop");

            // drag and drop an Address component
            var $address = rem.$('.o_tele_studio_sidebar .o_tele_studio_component:contains(Address)');
    ...
    QUnit.test('drag & drop components with corrected p inside p', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            await testUtils.dom.click(rem.$('.o_tele_studio_sidebar .o_tele_studio_sidebar_header div[name="new"]'));
            const $field = rem.$('.o_tele_studio_sidebar .o_tele_studio_component:contains(Field):eq(0)');
            await testUtils.dom.dragAndDrop($field, $page.find('.root_p'));

            // hook after p inside the template, but not after p in html field
            assert.containsOnce($page, '.root_p + .o_tele_studio_hook');
            assert.containsOnce($page, '.child_p + .o_tele_studio_hook');
            assert.containsNone($page, '.html_field_p + .o_tele_studio_hook');
            ...
            await testUtils.dom.click(rem.$('.o_tele_studio_sidebar .o_tele_studio_sidebar_header div[name="new"]'));
            const $fieldAndLabel = rem.$('.o_tele_studio_sidebar .o_tele_studio_component:contains(Field & Label):eq(0)');
            await testUtils.dom.dragAndDrop($fieldAndLabel, $page.find('.root_p'));

            // hook after p inside the template, but not after p in html field
            assert.containsOnce($page, '.root_p + .o_tele_studio_structure_hook');
            assert.containsOnce($page, '.child_p + .o_tele_studio_structure_hook');
            assert.containsNone($page, '.html_field_p + .o_tele_studio_structure_hook');
    ...
    QUnit.test('drag & drop field block', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            var $field = rem.$('.o_tele_studio_sidebar .o_tele_studio_field_type_container:eq(0) .o_tele_studio_component:contains(Field):eq(0)');
            ...
            assert.strictEqual($('.o_tele_studio_field_modal .o_field_selector_chain_part').text().trim(), "o (Model Test)",
                'correct object "o" should be set by by default');

            $('.o_tele_studio_field_modal .o_field_selector').trigger('focusin');
            await testUtils.nextTick();
            await testUtils.dom.click($('.o_tele_studio_field_modal .o_field_selector_prev_page'));

            assert.strictEqual($('.o_tele_studio_field_modal .o_field_selector_item').text().trim(), "o (Model Test)",
                'Only "o" should be selectable, not "docs"');

            await testUtils.dom.click($('.o_tele_studio_field_modal .o_field_selector_item[data-name="o"]'));

            var allAvailableFields = $('.o_tele_studio_field_modal .o_field_selector_item').text().trim();
            ...
            await testUtils.dom.click($('.o_tele_studio_field_modal .o_field_selector_item[data-name="child"]'));
            await testUtils.dom.click($('.o_tele_studio_field_modal .o_field_selector_item[data-name="name"]'));
            await testUtils.dom.click($('.o_tele_studio_field_modal .btn-primary'));
    ...
    QUnit.test('preselect first available field in the field selector', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            var selectedField = $('.o_tele_studio_field_modal .o_field_selector .o_field_selector_chain_part').text().trim();
    ...
    QUnit.test('drag & drop field in row', loadIframeCss(async function (assert, done) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            var $field = rem.$('.o_tele_studio_sidebar .o_tele_studio_field_type_container:eq(1) .o_tele_studio_component:contains(Field)');
            ...
            for (testIndex; testIndex < tests.length; testIndex++) {
                ...
                var $nearestHook = rem.$('iframe').contents().find('.o_tele_studio_nearest_hook');
    ...
    QUnit.test('drag & drop field in table', loadIframeCss(async function (assert, done) {
        ...
        var tests = [
            {
                ...
                buildingBlockSelector: '.o_tele_studio_sidebar .o_tele_studio_field_type_container:eq(1) .o_tele_studio_component:contains(Field)',
                position: 'left',
                nearestHookNumber: 1,
                inheritance: [{
                    content: "<span t-field=\"o.child.name\"></span>",
                ...
                buildingBlockSelector: '.o_tele_studio_sidebar .o_tele_studio_field_type_container:eq(2) .o_tele_studio_component:contains(Field Column)',
                ...
                onDragAndDrop: function ($table) {
                    assert.strictEqual($table.find('tr th:first-child.o_tele_studio_nearest_hook').length, 1,
                            "Should select the first title cell");
                    assert.strictEqual($table.find('tr td:first-child.o_tele_studio_nearest_hook').length, 4,
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            for (testIndex; testIndex < tests.length; testIndex++) {
                ...
                var $nearestHook = $table.find('.o_tele_studio_nearest_hook');
                assert.strictEqual($nearestHook.length, test.nearestHookNumber, test.text + ' (nearestHook number)');
        ...
        var tests = [
            {
                ...
                onDragAndDrop: function ($table) {
                    assert.strictEqual($table.find('tr th:first-child.o_tele_studio_nearest_hook').length, 1,
                            "Should select the first title cell");
                    assert.strictEqual($table.find('tr td:first-child.o_tele_studio_nearest_hook').length, 1,
                            "Should select the first cell of each line");
                },
            }
        ];
    ...
    QUnit.test('drag & drop block "Accounting Total"', loadIframeCss(async function (assert, done) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            var $text = rem.$('.o_tele_studio_sidebar .o_tele_studio_field_type_container:eq(2) .o_tele_studio_component:contains(Subtotal & Total)');
    ...
    QUnit.test('drag & drop block "Accounting Total"', loadIframeCss(async function (assert, done) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            var $card = rem.$('.o_tele_studio_sidebar .card:has(.o_text:contains(table))');
            await testUtils.dom.click($card.find('[data-toggle="collapse"]'));
            $card = rem.$('.o_tele_studio_sidebar .card.o_tele_studio_active');
            assert.strictEqual($card.find('.o_text').text().trim(), 'table',
                'Correct card should be active after sidebar updation');

            assert.strictEqual($card.find('.o_tele_studio_report_currency_id .o_field_selector_chain_part').text().replace(/\s+/g, ' '),
                ' o (Model Test) Child ', 'Should display the t-foreach value');

            rem.$('.o_tele_studio_report_currency_id .o_field_selector').trigger('focusin');
            await testUtils.nextTick();
            await testUtils.dom.click(rem.$('.o_tele_studio_report_currency_id .o_field_selector_item[data-name="grandchild"]'));
            await testUtils.dom.click(rem.$('.o_tele_studio_report_currency_id .o_field_selector_close'));
    ...
    QUnit.test('drag & drop block "Data table"', loadIframeCss(async function (assert, done) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            var $text = rem.$('.o_tele_studio_sidebar .o_tele_studio_component:contains(Data table)');
            ...
            await testUtils.dom.click($('.o_tele_studio_field_modal .o_field_selector_item[data-name="children"]'));
            await testUtils.dom.click($('.o_tele_studio_field_modal .btn-primary'));
    ...
    QUnit.test('drag & drop block "Address"', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            var $text = rem.$('.o_tele_studio_sidebar .o_tele_studio_component:contains(Address)');
            await testUtils.dom.dragAndDrop($text, $page, {position: 'inside'});
            $('.o_tele_studio_field_modal .o_field_selector').trigger('focusin');
            await testUtils.dom.click($('.o_tele_studio_field_modal .o_field_selector_item[data-name="partner"]'));
            await testUtils.dom.click($('.o_tele_studio_field_modal .btn-primary'));
    ...
    QUnit.test('drag & drop block "Image"', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            var $page = rem.$('iframe').contents().find('.page');
            var $imageBlock = rem.$('.o_tele_studio_sidebar .o_tele_studio_component:contains(Image)');
    ...
    QUnit.test('edit text', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            var $editable = rem.$('.o_tele_studio_sidebar .card.o_tele_studio_active .note-editable');
    ...
    QUnit.test('open XML editor after modification', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            await testUtils.dom.click(rem.$('iframe').contents().find('span:contains(First)'));
            await testUtils.fields.editInput(rem.$('.o_tele_studio_sidebar .o_tele_studio_active textarea[name="text"]'), "hello");
            ...
            await testUtils.dom.click(rem.$('.o_tele_studio_sidebar_header [name="report"]'));
            // open the XML editor
            await testUtils.dom.click(rem.$('.o_tele_studio_sidebar .o_tele_studio_xml_editor'));
    ...
    QUnit.test('automatic undo of correct operation', async function (assert) {
        ...
        rem.editorIframeDef.then(async function () {
            ...
            await testUtils.dom.click(rem.$('.o_tele_studio_sidebar .card:eq(1) .o_tele_studio_text_decoration button[data-property="bold"]'));

            // trigger a second modification before the first one has finished
            await testUtils.dom.click(rem.$('.o_tele_studio_sidebar .card:eq(1) .o_tele_studio_text_decoration button[data-property="italic"]'));

            // trigger a third modification before the first one has finished
            await testUtils.dom.click(rem.$('.o_tele_studio_sidebar .card:eq(1) .o_tele_studio_text_decoration button[data-property="underline"]'));
    ...
    QUnit.test('preselect appropriate value in domain dialog', async function (assert) {
        ...
        await rem.editorIframeDef.then(async function () {
            ...
            await testUtils.dom.click(rem.$('.card.o_tele_studio_active .o_field_domain_dialog_button'));
