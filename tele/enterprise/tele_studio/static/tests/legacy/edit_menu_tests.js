tele.define('tele_studio.EditMenu_tests', function (require) {
"use strict";

var testUtils = require('web.test_utils');

var EditMenu = require('tele_studio.EditMenu');
...
QUnit.module('Studio', {
    ...
    QUnit.test('edit menu behavior', async function(assert) {
        ...
        assert.strictEqual($('.o_tele_studio_edit_menu_modal').length, 0,
        ...
        assert.strictEqual($('.o_tele_studio_edit_menu_modal').length, 1,
    ...
    QUnit.test('edit menu dialog', async function(assert) {
        ...
        assert.strictEqual($('.o_tele_studio_add_menu_modal').length, 1,
            "there should be a modal in the dom");
        assert.strictEqual($('.o_tele_studio_add_menu_modal input[name="name"]').length, 1,
            "there should be an input for the name in the dialog");
        await testUtils.dom.click($('.o_tele_studio_add_menu_modal .o_field_widget[name="model_choice"] [data-value="new"]'));
        assert.isNotVisible($('.o_tele_studio_add_menu_modal .o_field_many2one'),
            "there should be no visible many2one for the model in the dialog");
        // Define a name for new model
        $('input[name="name"]').val("new_model");
        await testUtils.dom.click($('.o_tele_studio_add_menu_modal .btn-primary'));
        assert.containsOnce($, '.o_tele_studio_model_configurator input[name="use_partner"]',
            "the ModelConfigurator should show the available model options");
        await testUtils.dom.click($('.o_tele_studio_model_configurator .o_tele_studio_model_configurator_previous'));
        assert.containsNone($, '.o_tele_studio_model_configurator',
            "the ModelConfigurator should be gone");
        await testUtils.dom.click($('.o_tele_studio_add_menu_modal .o_field_widget[name="model_choice"] [data-value="existing"]'));
        assert.strictEqual($('.o_tele_studio_add_menu_modal .o_field_many2one').filter(':visible').length, 1,
            "there should be a visible many2one for the model in the dialog");

        // add menu and close the modal
        await testUtils.fields.editInput($('.o_tele_studio_add_menu_modal input[name="name"]'), "AA");
        await testUtils.dom.click($('.o_tele_studio_add_menu_modal .btn-primary'));
        await testUtils.dom.click($('.o_tele_studio_add_menu_modal .btn-secondary'));
