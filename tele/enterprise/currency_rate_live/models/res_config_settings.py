...
from tele.applets.web.controllers.main import xml2json_from_elementtree
from tele.exceptions import UserError
from tele.tools.translate import _
from tele.tools import DEFAULT_SERVER_DATE_FORMAT
...
class ResCompany(models.Model):
    ...
    def _generate_currency_rates(self, parsed_data):
        ...
        exchange rate of their main currency equal to one in Tele.
        """
    ...
    def _parse_bcrp_data(self, available_currencies):
        ...
        for currency_tele_code, currency_pe_code in foreigns.items():
            if currency_tele_code not in available_currency_names:
                continue
            ...
            result[currency_tele_code] = (rate, date_rate)
        return result
