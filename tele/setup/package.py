...
exec(open(os.path.join(ROOTDIR, 'tele', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/tele && \
    groupadd -g %(group_id)s tele && \
    useradd -u %(user_id)s -g tele tele -d /var/lib/tele && \
    mkdir /data && \
    chown tele:tele /var/lib/tele /data
USER tele
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class TeleTestTimeoutError(Exception):
    pass


class TeleTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise TeleTestError("Installation of package failed")
        else:
        ...
        raise TeleTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/tele_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_applets=True):
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'tele-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
    ...
    def run(self, cmd, build_dir, container_name, user='tele', exposed_port=None, detach=False, timeout=None):
    ...
    def test_tele(self):
        logging.info('Starting to test Tele install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'tele.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise TeleTestTimeoutError('Tele pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise TeleTestError('Error while installing/starting Tele after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'tele-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/tele-*.tar.gz' % self.args.build_dir)[0], '%s/tele_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/tele-*.zip' % self.args.build_dir)[0], '%s/tele_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'pip3 install /data/src/tele_%s.%s.tar.gz' % (VERSION, TSTAMP),
            'su postgres -s /bin/bash -c "createuser -s tele"',
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/tele (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../tele_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'tele-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            '/usr/bin/dpkg -i /data/src/tele_%s.%s_all.deb ; /usr/bin/apt-get install -f -y' % (VERSION, TSTAMP),
            'su tele -s /bin/bash -c "tele -d mycompany -i base --stop-after-init"',
            'su tele -s /bin/bash -c "tele -d mycompany --pidfile=/data/src/tele.pid"',
        ]
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py --quiet bdist_rpm', self.args.build_dir, 'tele-rpm-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/tele-*.noarch.rpm' % self.args.build_dir)[0], '%s/tele_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'dnf install -d 0 -e 0 /data/src/tele_%s.%s.rpm -y' % (VERSION, TSTAMP),
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'tele-rpm-createrepo-%s' % TSTAMP)
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        remote_build_dir = '/cygdrive/c/telebuild/server/'

        self.ssh("mkdir -p build")
        logging.info("Syncing Tele files to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        setup_path = glob("%s/tele_setup_*.exe" % self.args.build_dir)[0]
        setupfile = setup_path.split('/')[-1]
        setupversion = setupfile.split('tele_setup_')[1].split('.exe')[0]
...
def parse_args():
    ...
    # Windows VM
    ap.add_argument("--vm-winxp-image", default='/home/tele/vm/win1036/win10_winpy36.qcow2', help="%(default)s")
    ap.add_argument("--vm-winxp-ssh-key", default='/home/tele/vm/win1036/id_rsa', help="%(default)s")
...
def main(args):
    try:
        ...
        if args.build_deb:
            _prepare_build_dir(args, move_applets=False)
