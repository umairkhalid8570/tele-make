...
import tele.modules.graph
import tele.modules.migration
import tele.modules.registry
from .. import SUPERUSER_ID, api, tools
from .module import adapt_version, initialize_sys_path, load_telecms_module
...
def force_demo(cr):
    ...
    """
    graph = tele.modules.graph.Graph()
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    processed_modules = []
    loaded_modules = []
    registry = tele.registry(cr.dbname)
    migrations = tele.modules.migration.MigrationManager(cr, graph)
    module_count = len(graph)
    _logger.info('loading %d modules...', module_count)

    # register, instantiate and initialize models for each modules
    t0 = time.time()
    loading_extra_query_count = tele.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = tele.sql_db.sql_counter
        ...
        load_telecms_module(package.name)

        new_install = package.state == 'to install'
        if new_install:
            py_module = sys.modules['tele.applets.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = tele.tools.config["overwrite_existing_translations"]
        ...
        if tools.config.options['test_enable'] and (needs_update or not updating):
            ...
            if suite.countTestCases():
                ...
                tests_t0, tests_q0 = time.time(), tele.sql_db.sql_counter
                test_results = loader.run_suite(suite, module_name)
                report.update(test_results)
                test_time = time.time() - tests_t0
                test_queries = tele.sql_db.sql_counter - tests_q0
        ...
        extra_queries = tele.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   tele.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
...
def load_modules(registry, force_demo=False, status=None, update_module=False):
    ...
    with registry.cursor() as cr:
        ...
        if not tele.modules.db.is_initialized(cr):
        ...
        if not graph:
            _logger.critical('module base cannot be loaded! (hint: verify applets-path)')
            raise ImportError('Module `base` cannot be loaded! (hint: verify applets-path)')
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['tele.applets.%s' % (pkg.name,)]
                ...
                registry = tele.modules.registry.Registry.new(
