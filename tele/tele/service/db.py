...
import tele.sql_db
import tele.tools
from tele.sql_db import db_connect
from tele.release import version_info
...
def check_super(passwd):
    if passwd and tele.tools.config.verify_admin_password(passwd):
        return True
    raise tele.exceptions.AccessDenied()

# This should be moved to tele.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        ...
        with closing(db.cursor()) as cr:
            ...
            tele.modules.db.initialize(cr)
            tele.tools.config['load_language'] = lang
            cr.commit()

        registry = tele.modules.registry.Registry.new(db_name, demo, None, update_module=True)
        ...
        with closing(registry.cursor()) as cr:
            ...
            if login:
                values['login'] = login
                emails = tele.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = tele.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = tele.tools.config['db_template']
    ...
    if tele.tools.config['unaccent']:
        try:
            db = tele.sql_db.db_connect(name)
            with closing(db.cursor()) as cr:
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    tele.sql_db.close_db(db_original_name)
    db = tele.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = tele.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = tele.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = tele.tools.config.filestore(db_original_name)
    to_fs = tele.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    tele.modules.registry.Registry.delete(db_name)
    ...
    fs = tele.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'tele_dump': '1',
        'db_name': cr.dbname,
        'version': tele.release.version,
        'version_info': tele.release.version_info,
        'major_version': tele.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = tele.tools.config.filestore(db_name)
            ...
            tele.tools.exec_pg_command(*cmd)
            if stream:
                tele.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                tele.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = tele.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        if tele.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = tele.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = tele.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    tele.modules.registry.Registry.delete(old_name)
    tele.sql_db.close_db(old_name)
    ...
    old_fs = tele.tools.config.filestore(old_name)
    new_fs = tele.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    tele.tools.config.set_admin_password(new_password)
    tele.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        tele.tools.config['update']['base'] = True
        tele.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@tele.tools.mute_logger('tele.sql_db')
...
def list_dbs(force=False):
    if not tele.tools.config['list_db'] and not force:
        raise tele.exceptions.AccessDenied()

    if not tele.tools.config['dbfilter'] and tele.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Tele will not
        ...
        res = sorted(db.strip() for db in tele.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [tele.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of Tele
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if tele.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        tele.sql_db.close_db(database_name)
...
def exp_list_lang():
    return tele.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(tele.tools.config['root_path'], 'applets/base/data/res_country_data.xml')).getroot()
