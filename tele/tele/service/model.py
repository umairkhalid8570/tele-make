...
from tele.tools.translate import translate, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = tele.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            with closing(tele.sql_db.db_connect(dbname).cursor()) as cr:
        ...
        while True:
            try:
                if tele.registry(dbname)._init and not tele.tools.config['test_enable']:
                    raise tele.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    ...
    cr.reset()
    recs = tele.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist", obj))
    result = tele.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with tele.registry(db).cursor() as cr:
