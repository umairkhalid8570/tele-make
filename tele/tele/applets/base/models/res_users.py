        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "https://www.docs.tele.studio/1.0/administration/install/deploy.html#https for details.",
                    source
                )
...
class ResUsers(models.Model):
    ...
    base_has_dark_mode = fields.Boolean(string="Has Dark Mode",copy=False)
    ...
    @property
    def SELF_READABLE_FIELDS(self):
        return super().SELF_READABLE_FIELDS + ["chatter_position","base_has_dark_mode"]

    @property
    def SELF_WRITEABLE_FIELDS(self):
        return super().SELF_WRITEABLE_FIELDS + ["chatter_position","base_has_dark_mode"]


    @api.model
    def get_or_set_base_has_dark_mode(self, toggle_dark_mode=False):
        has_dark_mode = False  
        query = """SELECT base_has_dark_mode 
                    FROM res_users
                    WHERE id = %(user_id)s;
                    """
        ...
        if toggle_dark_mode:
            ...
            query = """update res_users set base_has_dark_mode=%(dark_mode_flag)s 
                        WHERE id = %(user_id)s;
                        """
            self.env.cr.execute(query, {
