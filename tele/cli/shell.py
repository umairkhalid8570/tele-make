...
class Shell(Command):
    """Start tele in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        tele.cli.server.report_configuration()
        tele.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            ...
            with registry.cursor() as cr:
                ...
                ctx = tele.api.Environment(cr, uid, {})['res.users'].context_get()
                env = tele.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
