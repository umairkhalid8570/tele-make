...
from tele.tools import cloc, config
from . import Command

class Cloc(Command):
    """\
    Tele cloc is a tool to count the number of relevant lines written in
    ...
    It has two modes of operation, either by providing a path:
        ...
        tele-make cloc -p module_path

    Or by providing the name of a database:

        tele-make cloc --applets-path=dirs -d database
