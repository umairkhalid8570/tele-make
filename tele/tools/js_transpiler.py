"""
This code is what let us use ES6-style modules in tele.
Classic Tele modules are composed of a top-level :samp:`tele.define({name},{body_function})` call.
This processor will take files starting with an `@tele-module` annotation (in a comment) and convert them to classic modules.
If any file has the /** tele-module */ on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual tele one with minimal changes.
...
def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom tele modules.
    ...
    legacy_tele_define = get_aliased_tele_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_tele_define, module_path),
    ]
    for s in steps:
        content = s(content)
    if legacy_tele_define:
        content += legacy_tele_define
    return content
...
def url_to_module_path(url):
    """
    Tele modules each have a name. (tele.define("<the name>", async function (require) {...});
...
def wrap_with_tele_define(module_path, content):
    """
    Wraps the current content (source code) with the tele.define call.
    Should logically be called once all other operations have been performed.
    """
    return f"""tele.define({module_path!r}, async function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
TELE_MODULE_RE = re.compile(r"""
    \s*                                       # some starting space
    \/(\*|\/).*\s*                            # // or /*
    @tele-module                              # @tele-module
...
def is_tele_module(content):
    """
    Detect if the file is a native tele module.
    We look for a comment containing @tele-module.

    :param content: source code
    :return: is this a tele module that need transpilation ?
    """
    result = TELE_MODULE_RE.match(content)
    return bool(result)


def get_aliased_tele_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the tele source base being:
    ...
    > tele.define("web/chrome/abstract_action", async function(require) {
    ...
    .. code-block:: javascript
        ...
        /** @tele-module */
        // after
        /** @tele-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @tele-module */
        // after
        /** @tele-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = TELE_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\ntele.define(`%s`, async function(require) {
                        return require('%s');
                        });\n""" % (alias, module_path)
            else:
                return """\ntele.define(`%s`, async function(require) {
