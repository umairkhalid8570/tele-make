...
class tele_resolver(etree.Resolver):
    """Tele specific file resolver that can be added to the XML Parser.
...
def _check_with_xsd(tree_or_str, stream, env=None):
    ...
    :param env (tele.api.Environment): If it is given, it enables resolving the
    ...
    if env:
        parser.resolvers.add(tele_resolver(env))
