...
import tele
from tele.loglevels import ustr
...
def generate_tracking_message_id(res_id):
    ...
    try:
        ...
        rnd = random.random()
    rndstr = ("%.15f" % rnd)[2:]
    return "<%s.%.15f-tele-%s@%s>" % (rndstr, time.time(), res_id, socket.gethostname())
...
def encapsulate_email(old_email, new_email):
    ...
    * New From: notifications@tele.studio
    * Output: "Admin" <notifications@tele.studio>
    """
