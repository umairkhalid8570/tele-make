...
WEB_TRANSLATION_COMMENT = "tele-web"
...
class GettextAlias(object):
    ...
    def _get_db(self):
        ...
        if db_name:
            return tele.sql_db.db_connect(db_name)
...
def TranslationFileReader(source, fileformat='po'):
    """ Iterate over translation file to return Tele translation entries """
    if fileformat == 'csv':
...
class PoFileReader:
    """ Iterate over po file to return Tele translation entries """
    def __init__(self, source):
...
class TranslationModuleReader:
    ...
    def __init__(self, cr, modules=None, lang=None):
        ...
        self._lang = lang
        self.env = tele.api.Environment(cr, tele.SUPERUSER_ID, {})
        self._to_translate = []
        self._path_list = [(path, True) for path in tele.applets.__path__]
    ...
    def _verified_module_filepaths(self, fname, path, root):
        ...
        display_path = "applets%s" % frelativepath
    ...
    def _export_translatable_resources(self):
        ...
        # scanning subdirectories that may contain applets
            ...
            for root, dummy, files in os.walk(path, followlinks=True):
                ...
                if fnmatch.fnmatch(root, '*/static/src*'):
                    ...
                    for fname in fnmatch.filter(files, '*.xml'):
                        self._babel_extract_terms(fname, path, root, 'tele.tools.translate:babel_extract_qweb',
                                                  extra_comments=[WEB_TRANSLATION_COMMENT])
