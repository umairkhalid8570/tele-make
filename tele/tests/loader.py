...
from .. import tools
from .common import TagsSelector, TeleSuite
from .runner import TeleTestResult


_logger = logging.getLogger(__name__)
def get_test_modules(module):
    """ Return a list of module for the applets potentially containing tests to
    feed unittest.TestLoader.loadTestsFromModule() """
    # Try to import the module
    results = _get_tests_modules('tele.applets', module)

    try:
        importlib.import_module('tele.upgrade.%s' % module)
    except ImportError:
        pass
    else:
        results += _get_tests_modules('tele.upgrade', module)
...
def make_suite(module_names, position='at_install'):
    ...
    return TeleSuite(sorted(tests, key=lambda t: t.test_sequence))
