...
from tele.applets.payment_adyen.const import CURRENCY_DECIMALS
...
class AdyenController(http.Controller):
    ...
    def adyen_payment_methods(self, acquirer_id, amount=None, currency_id=None, partner_id=None):
        ...
        shopper_reference = partner_sudo and f'TELE_PARTNER_{partner_sudo.id}'
        data = {
    ...
    def adyen_return_from_redirect(self, **data):
        ...
        The route is flagged with `save_session=False` to prevent Tele from assigning a new session
        to the user if they are redirected to this route with a POST request. Indeed, as the session
        cookie is created without a `SameSite` attribute, some browsers that don't implement the
        recommended default `SameSite=Lax` behavior will not include the cookie in the redirection
        request from the payment provider to Tele. As the redirection to the '/payment/status' page
        ...
        # of Tele in some cases. For instance, when a 3DS1 authentication is required, or for
