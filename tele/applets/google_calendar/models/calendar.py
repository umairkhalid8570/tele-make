...
class Meeting(models.Model):
    ...
    @api.model
    def _tele_values(self, google_event, default_reminders=()):
        ...
        alarm_commands = self._tele_reminders_commands(reminder_command)
        attendee_commands, partner_commands = self._tele_attendee_commands(google_event)
    ...
    @api.model
    def _tele_attendee_commands(self, google_event):
        attendee_commands = []
        partner_commands = []
        ...
        if google_event.exists(self.env):
            existing_attendees = self.browse(google_event.tele_id(self.env)).attendee_ids
        ...
        for tele_attendee in attendees_by_emails.values():
            ...
            email = tools.email_normalize(tele_attendee.email)
            if email not in emails and email != self.env.user.email:
                attendee_commands += [(2, tele_attendee.id)]
                partner_commands += [(3, tele_attendee.partner_id.id)]
        return attendee_commands, partner_commands

    @api.model
    def _tele_reminders_commands(self, reminders=()):
        commands = []
    ...
    def _google_values(self):
        ...
        values = {
            ...
            'extendedProperties': {
                'shared': {
                    '%s_tele_id' % self.env.cr.dbname: self.id,
                },
            },
            'reminders': {
                'overrides': reminders,
        ...
        if self.user_id and self.user_id != self.env.user and not bool(self.user_id.sudo().google_calendar_token):
            # The organizer is an Tele user that do not sync his calendar
            ...
            # the owner is not an Tele user: We don't store the real owner identity (mail)
            ...
            # values['extendedProperties']['private] should be used if the owner is not an tele user
