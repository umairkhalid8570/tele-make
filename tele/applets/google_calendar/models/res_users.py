...
from tele.applets.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
from tele.applets.google_calendar.models.google_sync import google_calendar_token
from tele.loglevels import exception_to_unicode
...
class User(models.Model):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        synced_recurrences = self.env['calendar.recurrence']._sync_google2tele(recurrences)
        synced_events = self.env['calendar.event']._sync_google2tele(events - recurrences, default_reminders=default_reminders)

        # Tele -> Google
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences.with_context(send_updates=send_updates)._sync_tele2google(calendar_service)
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_tele2google(calendar_service)
