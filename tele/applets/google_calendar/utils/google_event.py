...
from tele.tools import email_normalize
import logging
...
class GoogleEvent(abc.Set):
    ...
    Inspired by Tele recordset, one instance can be a single Google event or a
    ...
    def tele_id(self, env):
        self.tele_ids(env)  # load ids
        return self._tele_id

    def _meta_tele_id(self, dbname):
        """Returns the Tele id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_tele_id' % dbname)
        if o_id:
            return int(o_id)

    def tele_ids(self, env):
        ids = tuple(e._tele_id for e in self if e._tele_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_tele_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_tele_ids_from_metadata(env, model)

        return tuple(e._tele_id for e in self)

    def _load_tele_ids_from_metadata(self, env, model):
        unsure_tele_ids = tuple(e._meta_tele_id(env.cr.dbname) for e in self)
        tele_events = model.browse(_id for _id in unsure_tele_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same Tele id.
        # Therefore, we only consider Tele records without google id when trying
        # to match events.
        o_ids = tele_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            tele_id = e._meta_tele_id(env.cr.dbname)
            if tele_id in o_ids:
                e._events[e.id]['_tele_id'] = tele_id

    def _load_tele_ids_from_db(self, env, model):
        tele_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in tele_events}  # {google_id: tele_id}
        existing_google_ids = tele_events.mapped('google_id')
        for e in self:
            tele_id = mapping.get(e.id)
            if tele_id:
                e._events[e.id]['_tele_id'] = tele_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Tele.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Tele (he is the owner) but userB syncs first.
        ...
        # (he is the organizer in Google). The "real" owner (in Tele) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Tele several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_tele_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.tele_ids(env)
        events.tele_ids(env)

        return self.filter(lambda e: e._tele_id)
