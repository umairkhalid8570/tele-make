...
from tele.applets.google_calendar.tests.test_sync_common import TestSyncGoogle, patch_api
from tele.applets.google_calendar.utils.google_calendar import GoogleEvent
from tele.tools import html2plaintext
from tele import Command

class TestSyncGoogle2Tele(TestSyncGoogle):
    ...
    def sync(self, events):
        ...
        self.env['calendar.recurrence']._sync_google2tele(google_recurrence)
        self.env['calendar.event']._sync_google2tele(events - google_recurrence)
    ...
    def test_new_google_event(self):
        values = {
            ...
            'organizer': {'email': 'telecalendarref@gmail.com', 'self': True},
            'summary': 'Pricing new update',
            'visibility': 'public',
            'attendees': [{
                'displayName': 'Tele Admin',
        ...
        self.env['calendar.event']._sync_google2tele(GoogleEvent([values]))
        ...
        self.assertEqual('Tele Admin', admin_attendee.partner_id.name)
    ...
    def test_attendee_cancelled(self):
        ...
        self.env['calendar.event'].with_user(user)._sync_google2tele(gevent)
    ...
    def test_private_extended_properties(self):
        ...
        self.assertGoogleEventPatched(event.google_id, {
            ...
            'attendees': [{'email': 'telebot@example.com', 'responseStatus': 'declined'}],
            'extendedProperties': {'private': {'%s_tele_id' % self.env.cr.dbname: event.id}},
    ...
    def test_recurrence(self):
        ...
        self.env['calendar.recurrence']._sync_google2tele(GoogleEvent([values]))
    ...
    def test_recurrence_moved(self):
        ...
        values = {
            ...
            "attendees": [
                {
                    "email": "telebot@example.com", "state": "accepted",
                },
            ],
            'updated': self.now,
        }
    ...
    def test_simple_event_into_recurrency(self):
        ...
        recurrence = self.env['calendar.recurrence']._sync_google2tele(GoogleEvent([values]))
        events = recurrence.calendar_event_ids.sorted('start')
        self.assertEqual(len(events), 3, "it should have created a recurrence with 3 events")
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
    ...
    def test_attendees_same_event_both_share(self):
        ...
        self.assertGoogleEventPatched(event.google_id, {
            ...
            'attendees': [{'email': 'c.c@example.com', 'responseStatus': 'needsAction'},
                          {'email': 'telebot@example.com', 'responseStatus': 'accepted'},],
            'extendedProperties': {'shared': {'%s_tele_id' % self.env.cr.dbname: event.id,
    ...
    def test_several_attendee_have_the_same_mail(self):
        """
        In google, One mail = One attendee but on Tele, some partners could share the same mail
        ...
        self.assertEqual(mails, ['dalton@example.com', 'telebot@example.com'])
        gevent = GoogleEvent([{
            'id': google_id,
            'description': 'coucou',
            "updated": self.now,
            'organizer': {'email': 'telebot@example.com', 'self': True},
            'summary': False,
            'visibility': 'public',
            'attendees': [],
            'reminders': {'useDefault': True},
        ...
        # User attendee removed but gevent owner might be added after synch.
        mails = sorted(set(event.attendee_ids.mapped('email')))
        self.assertEqual(mails, ['telebot@example.com'])
    ...
    def test_event_with_meeting_url(self):
        values = {
            ...
            'conferenceData': {
                'entryPoints': [{
                    ...
                    'uri': 'https://meet.google.com/tele-random-test',
                    'label': 'meet.google.com/tele-random-test'
                }, {
                    'entryPointType': 'more',
                    'uri':'https://tel.meet/tele-random-test?pin=42424242424242',
                    'pin':'42424242424242'
                }]
            }
        }
        ...
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
        self.assertTrue(event, "It should have created an event")
        self.assertEqual(event.videocall_location, 'https://meet.google.com/tele-random-test')
        self.assertGoogleAPINotCalled()
