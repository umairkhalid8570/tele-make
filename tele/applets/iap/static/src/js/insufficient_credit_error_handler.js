...
class InsufficientCreditDialog extends Dialog {
    ...
    async willStart() {
        ...
        if (errorData.trial && isEnterprise) {
            this.buttonMessage = _t("Start a Trial at Tele");
        } else {
...
InsufficientCreditDialog.bodyTemplate = "iap.redirect_to_tele_credit";
...
function insufficientCreditHandler(env, error, originalError) {
    ...
    if (data && data.name === "tele.applets.iap.tools.iap_tools.InsufficientCreditError") {
