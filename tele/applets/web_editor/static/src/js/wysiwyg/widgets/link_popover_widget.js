...
const LinkPopoverWidget = Widget.extend({
    ...
    start() {
        ...
        this.$('[data-toggle="tooltip"]').tooltip({
            delay: 0,
            placement: 'bottom',
            container: this.options.wysiwyg.teleEditor.document.body,
        });
        const tooltips = [];
        ...
        .on('show.bs.popover.link_popover', () => {
            this.options.wysiwyg.teleEditor.observerUnactive('show.bs.popover');
            this._loadAsyncLinkPreview();
            popoverShown = true;
        })
        .on('inserted.bs.popover', () => {
            this.options.wysiwyg.teleEditor.observerActive('show.bs.popover');
        })
        .on('hide.bs.popover.link_popover', () => {
            this.options.wysiwyg.teleEditor.observerUnactive('hide.bs.popover');
            popoverShown = false;
        })
        .on('hidden.bs.popover.link_popover', () => {
            this.options.wysiwyg.teleEditor.observerActive('hide.bs.popover');
        ...
        if (document !== this.options.wysiwyg.teleEditor.document) {
            $(this.options.wysiwyg.teleEditor.document).on('mouseup.link_popover', onClickDocument);
        }
    ...
    destroy() {
        ...
        $(this.options.wysiwyg.teleEditor.document).off('.link_popover');
...
LinkPopoverWidget.createFor = async function (parent, targetEl, options) {
    ...
    if (wysiwyg) {
        wysiwyg.teleEditor.observerUnactive('LinkPopoverWidget');
    }
    await popoverWidget.appendTo(targetEl)
    if (wysiwyg) {
        wysiwyg.teleEditor.observerActive('LinkPopoverWidget');
    }
