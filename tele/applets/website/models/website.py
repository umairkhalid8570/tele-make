...
from tele import api, fields, models, tools, http, release, registry
from tele.applets.http_routing.models.ir_http import slugify, _guess_mimetype, url_for
from tele.applets.website.models.ir_http import sitemap_qs2dom
from tele.applets.website.tools import get_unaccent_sql_wrapper, similarity_score, text_from_html
from tele.applets.portal.controllers.portal import pager
...
from tele.osv.expression import AND, OR, FALSE_DOMAIN
from tele.tools.translate import _
from tele.tools import escape_psql, pycompat
...
DEFAULT_ENDPOINT = 'https://website.api.tele.studio'
...
class Website(models.Model):
    ...
    def get_theme_snippet_lists(self, theme_name):
        default_snippet_lists = http.applets_manifest['theme_default'].get('snippet_lists', {})
        theme_snippet_lists = http.applets_manifest[theme_name].get('snippet_lists', {})
    ...
    def configurator_recommended_themes(self, industry_id, palette):
        ...
        client_themes_img = dict([
            (t, http.applets_manifest[t].get('images_preview_theme', {}))
            for t in client_themes if t in http.applets_manifest
        ])
