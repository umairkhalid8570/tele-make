...
from tele.applets.automotive.models.automotive_vehicle_model import FUEL_TYPES
...
class AutomotiveVehicle(models.Model):
    ...
    def _get_default_state(self):
        state = self.env.ref('automotive.automotive_vehicle_state_registered', raise_if_not_found=False)
    ...
    manager_id = fields.Many2one(
        'res.users', 'Automotive Manager',
        domain=lambda self: [('groups_id', 'in', self.env.ref('automotive.automotive_group_manager').id)],
    )
    ...
    model_id = fields.Many2one('automotive.vehicle.model', 'Model',
        tracking=True, required=True, help='Model of the vehicle')

    brand_id = fields.Many2one('automotive.vehicle.model.brand', 'Brand', related="model_id.brand_id", store=True, readonly=False)
    log_drivers = fields.One2many('automotive.vehicle.assignation.log', 'vehicle_id', string='Assignment Logs')
    log_services = fields.One2many('automotive.vehicle.log.services', 'vehicle_id', 'Services Logs')
    log_contracts = fields.One2many('automotive.vehicle.log.contract', 'vehicle_id', 'Contracts')
    ...
    state_id = fields.Many2one('automotive.vehicle.state', 'State',
    ...
    tag_ids = fields.Many2many('automotive.vehicle.tag', 'automotive_vehicle_vehicle_tag_rel', 'vehicle_tag_id', 'tag_id', 'Tags', copy=False)
    ...
    def _get_odometer(self):
        AutomotiveVehicalOdometer = self.env['automotive.vehicle.odometer']
        for record in self:
            vehicle_odometer = AutomotiveVehicalOdometer.search([('vehicle_id', '=', record.id)], limit=1, order='value desc')
            if vehicle_odometer:
    ...
    def _set_odometer(self):
        for record in self:
            if record.odometer:
                ...
                self.env['automotive.vehicle.odometer'].create(data)

    def _compute_count_all(self):
        Odometer = self.env['automotive.vehicle.odometer']
        LogService = self.env['automotive.vehicle.log.services']
        LogContract = self.env['automotive.vehicle.log.contract']
        for record in self:
            ...
            record.history_count = self.env['automotive.vehicle.assignation.log'].search_count([('vehicle_id', '=', record.id)])
            ...
            for element in record.log_contracts:
                if element.state in ('open', 'expired') and element.expiration_date:
                    ...
                    if overdue or due_soon:
                        log_contract = self.env['automotive.vehicle.log.contract'].search([
    ...
    def _get_analytic_name(self):
        # This function is used in automotive_account and is overrided in l10n_be_hr_payroll_automotive
    ...
    def _search_contract_renewal_due_soon(self, operator, value):
        ...
        res_ids = self.env['automotive.vehicle.log.contract'].search([
    ...
    def create(self, vals):
        # Automotive administrator may not have rights to create the plan_to_change_car value when the driver_id is a res.user
        ...
        res = super(AutomotiveVehicle, self).create(vals)
        if ptc_value:
            res.sudo().write(ptc_value)
        if 'driver_id' in vals and vals['driver_id']:
            res.create_driver_history(vals)
        if 'future_driver_id' in vals and vals['future_driver_id']:
            state_waiting_list = self.env.ref('automotive.automotive_vehicle_state_waiting_list', raise_if_not_found=False)
    ...
    def write(self, vals):
        ...
        if 'active' in vals and not vals['active']:
            self.env['automotive.vehicle.log.contract'].search([('vehicle_id', 'in', self.ids)]).active = False
            self.env['automotive.vehicle.log.services'].search([('vehicle_id', 'in', self.ids)]).active = False

        res = super(AutomotiveVehicle, self).write(vals)
        return res
    ...
    def create_driver_history(self, vals):
        for vehicle in self:
            self.env['automotive.vehicle.assignation.log'].create(
    ...
    def _read_group_stage_ids(self, stages, domain, order):
        return self.env['automotive.vehicle.state'].search([], order=order)

    @api.model
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        if 'co2' in fields:
            fields.remove('co2')
        return super(AutomotiveVehicle, self).read_group(domain, fields, groupby, offset, limit, orderby, lazy)
    ...
    def return_action_to_open(self):
        ...
        if xml_id:
            ...
            res = self.env['ir.actions.act_window']._for_xml_id('automotive.%s' % xml_id)
            res.update(
    ...
    def act_show_log_cost(self):
        ...
        res = self.env['ir.actions.act_window']._for_xml_id('automotive.automotive_vehicle_costs_action')
        res.update(
    ...
    def _track_subtype(self, init_values):
        ...
        if 'driver_id' in init_values or 'future_driver_id' in init_values:
            return self.env.ref('automotive.mt_automotive_driver_updated')
        return super(AutomotiveVehicle, self)._track_subtype(init_values)
