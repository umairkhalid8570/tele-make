...
from tele.exceptions import RedirectWarning, UserError, ValidationError
from tele.tools.translate import _

from tele.applets.google_account.models.google_service import GOOGLE_TOKEN_ENDPOINT, TIMEOUT
...
class GoogleDrive(models.Model):
    ...
    def copy_doc(self, res_id, template_id, name_gdocs, res_model):
        ...
        if content.get('alternateLink'):
            ...
            request_url = "https://www.googleapis.com/drive/v2/files/%s/permissions?emailMessage=This+is+a+drive+file+created+by+Tele&sendNotificationEmails=false&access_token=%s" % (key, access_token)
