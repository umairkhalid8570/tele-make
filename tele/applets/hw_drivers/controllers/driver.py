...
from tele import http, tools
from tele.http import send_file
from tele.modules.module import get_resource_path

from tele.applets.hw_drivers.event_manager import event_manager
from tele.applets.hw_drivers.main import iot_devices, manager
...
class DriverController(http.Controller):
    ...
    def connect_box(self, token):
        """
        This route is called when we want that a IoT Box will be connected to a Tele DB
        token is a base 64 encoded string and have 2 argument separate by |
        1 - url of tele DB
        2 - token. This token will be compared to the token of Tele. He have 1 hour lifetime
        """
