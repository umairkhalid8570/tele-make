...
class IoTRestart(Thread):
    """
    Thread to restart tele server in IoT Box when we must return a answer before
    """
...
def add_credential(db_uuid, enterprise_code):
    write_file('tele-db-uuid.conf', db_uuid)
    write_file('tele-enterprise-code.conf', enterprise_code)

def check_certificate():
    """
    Check if the current certificate is up to date or not authenticated
    """
    server = get_tele_server_url()
    if server:
        ...
        if path.exists():
            with path.open('r') as f:
                ...
                if cn == 'TeleTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected Tele DB and
    ...
    if server:
        ...
        try:
            ...
            if response.status == 200:
                git = ['git', '--work-tree=/home/pi/tele/', '--git-dir=/home/pi/tele/.git']
                ...
                if db_branch != local_branch:
                    ...
                    subprocess.check_call(["rm", "-rf", "/home/pi/tele/applets/hw_drivers/iot_handlers/drivers/*"])
                    subprocess.check_call(["rm", "-rf", "/home/pi/tele/applets/hw_drivers/iot_handlers/interfaces/*"])
...
def check_image():
    ...
    """
    url = 'https://nightly.tele.studio/master/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def get_tele_server_url():
    ap = subprocess.call(['systemctl', 'is-active', '--quiet', 'hostapd']) # if service is active return 0 else inactive
    if not ap:
        return False
    return read_file_first_line('tele-remote-server.conf')

def get_token():
    return read_file_first_line('token')

def get_version():
    return subprocess.check_output(['cat', '/var/tele/iotbox_version']).decode().rstrip()
...
def load_certificate():
    """
    Send a request to Tele with customer db_uuid and enterprise_code to get a true certificate
    """
    db_uuid = read_file_first_line('tele-db-uuid.conf')
    enterprise_code = read_file_first_line('tele-enterprise-code.conf')
    if db_uuid and enterprise_code:
        url = 'https://www.tele.studio/tele-enterprise/iot/x509'
        data = {
            'params': {
                'db_uuid': db_uuid,
        ...
        if result:
            write_file('tele-subject.conf', result['subject_cn'])
            subprocess.call(["sudo", "mount", "-o", "remount,rw", "/"])
...
def download_iot_handlers(auto=True):
    """
    Get the drivers from the configured Tele server
    """
    server = get_tele_server_url()
    if server:
        urllib3.disable_warnings()
        ...
        try:
            ...
            if resp.data:
                subprocess.call(["sudo", "mount", "-o", "remount,rw", "/"])
                drivers_path = Path.home() / 'tele/applets/hw_drivers/iot_handlers'
...
def load_iot_handlers():
    """
    This method loads local files: 'tele/applets/hw_drivers/iot_handlers/drivers' and
    'tele/applets/hw_drivers/iot_handlers/interfaces'
    ...
    http.applets_manifest = {}
    http.root = http.Root()

def tele_restart(delay):
    IR = IoTRestart(delay)
    IR.start()
