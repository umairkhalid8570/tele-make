...
REPO=https://github.com/tele-studio/tele.git
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/tele"
...
if [ ! -d $CLONE_DIR ]; then
    ...
    echo "applets/web
applets/hw_*
applets/point_of_sale/tools/posbox/configuration
tele/
tele-make" | tee --append .git/info/sparse-checkout > /dev/null
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/tele
echo "${VERSION_IOTBOX}" | tee "${MOUNT_POINT}"/var/tele/iotbox_version "${MOUNT_POINT}"/home/pi/iotbox_version
