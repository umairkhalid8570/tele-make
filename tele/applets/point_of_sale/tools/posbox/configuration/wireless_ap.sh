...
if [ -z "${WIRED_IP}" ] ; then
	...
	if [ -f "${WIFI_NETWORK_FILE}" ] && [ -z "${FORCE_HOST_AP}" ] ; then
		...
		/home/pi/tele/applets/point_of_sale/tools/posbox/configuration/connect_to_wifi.sh &
	else
		...
		nginx -c /home/pi/tele/applets/point_of_sale/tools/posbox/configuration/nginx_ap.conf

		service tele restart
	fi
# wired
else
	killall nginx
