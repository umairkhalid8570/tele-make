...
function connect () {
	SERVER="${1}"
	CURRENT_SERVER_FILE=/home/pi/tele-remote-server.conf
	...
	if [ ${TIMEOUT_RETURN} -eq 124 ] && [ -z "${NO_AP}" ] ; then
		...
		sudo /home/pi/tele/applets/point_of_sale/tools/posbox/configuration/wireless_ap.sh "force" &
	else
		...
		if [ ! -f "${LOST_WIFI_FILE}" ] ; then
			logger -t posbox_connect_to_wifi "Restarting tele"
		fi
		...
		/home/pi/tele/applets/point_of_sale/tools/posbox/configuration/keep_wifi_alive.sh &
	fi
