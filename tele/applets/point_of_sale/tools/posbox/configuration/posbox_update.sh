...
echo "applets/point_of_sale/tools/posbox/overwrite_after_init/home/pi/tele" >> .git/info/sparse-checkout

git fetch "${localremote}" "${localbranch}" --depth=1
git reset "${localremote}"/"${localbranch}" --hard

git clean -dfx
cp -a /home/pi/tele/applets/point_of_sale/tools/posbox/overwrite_after_init/home/pi/tele/* /home/pi/tele/
rm -r /home/pi/tele/applets/point_of_sale/tools/posbox/overwrite_after_init
...
(sleep 5 && sudo service tele restart) &
