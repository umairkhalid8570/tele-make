...
class TestRecognitionCommon(TransactionCaseWithUserDemo):

    def setUp(self):
        super(TestRecognitionCommon, self).setUp()
        employees_group = self.env.ref('base.group_user')
        self.user_ids = employees_group.users

        # Push demo user into the challenge before creating a new one
        self.env.ref('recognition.challenge_base_discover')._update_all()
        self.robot = self.env['res.users'].with_context(no_reset_password=True).create({
            'name': 'R2D2',
            'login': 'r2d2@tele.studio',
            'email': 'r2d2@tele.studio',
            'groups_id': [(6, 0, [employees_group.id])]
        })
        self.badge_good_job = self.env.ref('recognition.badge_good_job')


class test_challenge(TestRecognitionCommon):

    def test_00_join_challenge(self):
        challenge = self.env.ref('recognition.challenge_base_discover')
    ...
    def test_10_reach_challenge(self):
        ...
        goal_ids = Goals.search([('user_id', '=', demo.id), ('definition_id', '=', self.env.ref('recognition.definition_base_timezone').id)])
        ...
        badge_ids = self.env['recognition.badge.user'].search([('badge_id', '=', badge_id), ('user_id', '=', demo.id)])
        self.assertEqual(len(badge_ids), 1, "Demo user has not received the badge")


class test_badge_wizard(TestRecognitionCommon):

    def test_grant_badge(self):
        wiz = self.env['recognition.badge.user.wizard'].create({
