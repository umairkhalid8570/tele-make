...
class GoalDefinition(models.Model):
    ...
    a new recognition_goal_definition
    """
    _name = 'recognition.goal.definition'
    _description = 'Recognition Goal Definition'
...
class Goal(models.Model):
    ...
    _name = 'recognition.goal'
    _description = 'Recognition Goal'
    ...
    line_id = fields.Many2one('recognition.challenge.line', string="Challenge Line", ondelete="cascade")
...
# no goal = global index
    ...
    def _check_remind_delay(self):
        ...
        body_html = self.env.ref('recognition.email_template_goal_reminder')._render_field('body_html', self.ids, compute_lang=True)[self.id]
        self.message_notify(
            body=body_html,
    ...
    def get_action(self):
        ...
        if self.computation_mode == 'manually':
            ...
            action = {
                ...
                'res_model': 'recognition.goal.wizard'
            }
            return action
