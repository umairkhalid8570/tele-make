...
from tele import models, fields, Command
...
from tele.tools import mute_logger, float_repr
from tele.tools.date_utils import add, subtract, start_of, end_of
...
class TestFields(TransactionCaseWithUserDemo):
    ...
    def test_85_binary_guess_zip(self):
        from tele.applets.base.tests.test_mimetypes import ZIP
    ...
    def test_86_text_base64_guess_svg(self):
        from tele.applets.base.tests.test_mimetypes import SVG
...
class TestX2many(common.TransactionCase):
    ...
    def test_10_ondelete_many2many(self):
        ...
        with self.assertRaises(psycopg2.IntegrityError):
            with mute_logger('tele.sql_db'), self.cr.savepoint():
                record_a.unlink()
...
class TestFieldParametersValidation(common.TransactionCase):
    def test_invalid_parameter(self):
        ...
        with self.assertLogs('tele.fields', level='WARNING') as cm:
            self.registry.setup_models(self.env.cr)

        self.assertTrue(cm.output[0].startswith(
            "WARNING:tele.fields:Field test_new_api.field_parameter_validation.name: "
