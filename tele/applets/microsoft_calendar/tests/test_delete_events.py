...
from tele.applets.microsoft_calendar.tests.common import (
    TestCommon,
    mock_get_token,
...
class TestDeleteEvents(TestCommon):
    ...
    def test_delete_simple_event_from_tele_organizer_calendar(self, mock_delete):
    ...
    @patch.object(MicrosoftCalendarService, 'delete')
    def test_delete_simple_event_from_tele_attendee_calendar(self, mock_delete):
        event_id = self.simple_event.ms_organizer_event_id
    ...
    @patch.object(MicrosoftCalendarService, 'delete')
    def test_archive_simple_event_from_tele_organizer_calendar(self, mock_delete):
        event_id = self.simple_event.ms_organizer_event_id
    ...
    @patch.object(MicrosoftCalendarService, 'delete')
    def test_archive_simple_event_from_tele_attendee_calendar(self, mock_delete):
        event_id = self.simple_event.ms_organizer_event_id
    ...
    def test_delete_simple_event_from_outlook_attendee_calendar(self):
        """
        If an attendee deletes an event from its Outlook calendar, during the sync, Tele will be notified that
        this event has been deleted BUT only with the attendees's calendar event id and not with the global one
        (called iCalUId). That means, it's not possible to match this deleted event with an Tele event.

        LIMITATION:

        Unfortunately, there is no magic solution:
            1) keep the list of calendar events ids linked to a unique iCalUId but all Tele users may not have synced
            their Tele calendar, leading to missing ids in the list => bad solution.
    ...
    @patch.object(MicrosoftCalendarService, 'delete')
    def test_delete_one_event_from_recurrence_from_tele_calendar(self, mock_delete):
    ...
    @patch.object(MicrosoftCalendarService, 'delete')
    def test_delete_first_event_from_recurrence_from_tele_calendar(self, mock_delete):
