...
class TestSyncTele2Microsoft(TransactionCase):
    ...
    def test_restart_synchronization(self):
        ...
        event.with_user(user)._sync_tele2microsoft(self.microsoft_service)
        ...
        self.assertMicrosoftEventPatched(event.microsoft_id, {
            ...
            'singleValueExtendedProperties': [{
                    'id': 'String {%s} Name tele_id' % microsoft_guid,
                    'value': str(event.id),
                }, {
                    'id': 'String {%s} Name owner_tele_id' % microsoft_guid,
                    'value': str(user.id),
                }
            ]
        })
