...
from tele import api, fields, models, registry
...
from tele.applets.microsoft_calendar.utils.microsoft_calendar import MicrosoftCalendarService
from tele.applets.microsoft_calendar.utils.event_id_storage import IDS_SEPARATOR, combine_ids, split_ids
from tele.applets.microsoft_account.models.microsoft_service import TIMEOUT
...
# This ensures changes are sent to Microsoft only if they really happened in the Tele database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Microsoft if the first creation crashed in Tele.
...
class MicrosoftSync(models.AbstractModel):
    ...
    def _sync_recurrence_microsoft2tele(self, microsoft_events, new_events=None):
        ...
        for recurrent_master in recurrent_masters:
            new_calendar_recurrence = dict(
                self.env['calendar.recurrence']._microsoft_to_tele_values(recurrent_master, default_values, with_ids=True),
                need_sync_m=False
            )
            ...
            base_values = dict(
                self.env['calendar.event']._microsoft_to_tele_values(recurrent_master, default_values, with_ids=True),
                need_sync_m=False
            )
            to_create_values = []
            ...
            for recurrent_event in to_create:
                if recurrent_event.type == 'occurrence':
                    value = self.env['calendar.event']._microsoft_to_tele_recurrence_values(recurrent_event, base_values)
                else:
                    value = self.env['calendar.event']._microsoft_to_tele_values(recurrent_event, default_values)
            ...
            new_recurrence_tele = self.env['calendar.recurrence'].create(new_calendar_recurrence)
            new_recurrence_tele.base_event_id = new_recurrence_tele.calendar_event_ids[0] if new_recurrence_tele.calendar_event_ids else False
            new_recurrence |= new_recurrence_tele
        ...
        for recurrent_master_id in ms_recurrence_ids:
            ...
            for recurrent_event in to_update:
                if recurrent_event.type == 'occurrence':
                    value = self.env['calendar.event']._microsoft_to_tele_recurrence_values(
    ...
    def _update_microsoft_recurrence(self, recurrence, events):
        """
        Update Tele events from Outlook recurrence and events.
        """
        ...
        for e in events_to_update:
            if e.type == "exception":
                event_values = self.env['calendar.event']._microsoft_to_tele_values(e)
            elif e.type == "occurrence":
                event_values = self.env['calendar.event']._microsoft_to_tele_recurrence_values(e)
            else:
                event_values = None
            ...
            if event_values:
                ...
                tele_event = self.env['calendar.event'].browse(e.tele_id(self.env)).exists().with_context(
                    no_mail_to_attendees=True, mail_create_nolog=True
                )
                tele_event.write(dict(event_values, need_sync_m=False))
                update_events |= tele_event
    ...
    @api.model
    def _sync_microsoft2tele(self, microsoft_events: MicrosoftEvent):
        """
        Synchronize Microsoft recurrences in Tele.
        Creates new recurrences, updates existing ones.
        :return: synchronized tele
        """
        existing = microsoft_events.match_with_tele_events(self.env)
        ...
        tele_values = [
            dict(self._microsoft_to_tele_values(e, with_ids=True), need_sync_m=False)
            for e in (new - new_recurrence)
        ]
        synced_events = self.with_context(dont_notify=True)._create_from_microsoft(new, tele_values)
        synced_recurrences, updated_events = self._sync_recurrence_microsoft2tele(existing, new_recurrence)
        ...
        for mevent in (existing - cancelled).filter(lambda e: e.lastModifiedDateTime):
            # Last updated wins.
            # This could be dangerous if microsoft server time and tele server time are different
            if mevent.is_recurrence():
                tele_event = self.env['calendar.recurrence'].browse(mevent.tele_id(self.env)).exists()
            else:
                tele_event = self.browse(mevent.tele_id(self.env)).exists()

            if tele_event:
                tele_event_updated_time = pytz.utc.localize(tele_event.write_date)
                ms_event_updated_time = parse(mevent.lastModifiedDateTime)

                if ms_event_updated_time >= tele_event_updated_time:
                    vals = dict(tele_event._microsoft_to_tele_values(mevent), need_sync_m=False)
                    tele_event._write_from_microsoft(mevent, vals)

                    if tele_event._name == 'calendar.recurrence':
                        update_events = tele_event._update_microsoft_recurrence(mevent, microsoft_events)
                        synced_recurrences |= tele_event
                        synced_events |= update_events
                    else:
                        synced_events |= tele_event
    ...
    def _microsoft_delete(self, user_id, event_id, timeout=TIMEOUT):
        """
        Once the event has been really removed from the Tele database, remove it from the Outlook calendar.
    ...
    def _microsoft_patch(self, user_id, event_id, values, timeout=TIMEOUT):
        """
        Once the event has been really modified in the Tele database, modify it in the Outlook calendar.
    ...
    def _microsoft_insert(self, values, timeout=TIMEOUT):
        """
        Once the event has been really added in the Tele database, add it in the Outlook calendar.
    ...
    def _get_microsoft_records_to_sync(self, full_sync=False):
        """
        Return records that should be synced from Tele to Microsoft
