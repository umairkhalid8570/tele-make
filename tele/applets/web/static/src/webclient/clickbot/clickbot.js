...
(function (exports) {
    ...
    const BLACKLISTED_MENUS = [
        ...
        "payment_tele.menu_adyen_account",
        "payment_tele.root_adyen_menu",
    ];

    const { isEnterprise } = tele.info;
    ...
    function ensureSetup() {
        ...
        setupDone = true;
        const env = tele.__WOWL_DEBUG__.root.env;
        env.bus.on("ACTION_MANAGER:UI-UPDATED", null, () => {
            actionCount++;
        });

        const AbstractController = tele.__DEBUG__.services["web.AbstractController"];
        ...
        const { patch } = tele.__DEBUG__.services["@web/core/utils/patch"];
        const { WithSearch } = tele.__DEBUG__.services["@web/search/with_search/with_search"];
        ...
        // This test file is not respecting Tele module dependencies.
        // The following module might not be loaded (eg. if mail is not installed).
        const DiscussWidgetModule = tele.__DEBUG__.services["@mail/widgets/discuss/discuss"];
    ...
    async function _clickEverywhere(xmlId) {
        ...
        console.log(`Tele flavor: ${isEnterprise ? "Enterprise" : "Community"}`);
