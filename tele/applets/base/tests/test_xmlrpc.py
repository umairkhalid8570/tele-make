...
from tele.exceptions import AccessDenied, AccessError
from tele.http import _request_stack

import tele.tools
from tele.tests import common
from tele.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, tele.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
