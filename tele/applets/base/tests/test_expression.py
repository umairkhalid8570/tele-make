...
class TestExpression(SavepointCaseWithUserDemo):
    ...
    def test_15_m2o(self):
        ...
        # that don't have Telehas a parent company, you expect to find,
    ...
    def test_15_equivalent_one2many_1(self):
        ...
        p1, _ = Partner.name_create("D??d?? Boitaclou")
        ...
        res = self._search(Partner, [('user_ids', '=', 'D??d?? Boitaclou')])
    ...
    def test_accent(self):
        if not self.registry.has_unaccent:
            return
        Company = self.env['res.company']
        helene = Company.create({'name': u'H??l??ne'})
        self.assertEqual(helene, Company.search([('name','ilike','Helene')]))
        self.assertEqual(helene, Company.search([('name','ilike','h??l??ne')]))
        self.assertNotIn(helene, Company.search([('name','not ilike','Helene')]))
        self.assertNotIn(helene, Company.search([('name','not ilike','h??l??ne')]))
...
class TestAutoJoin(TransactionCase):
    ...
    def test_nullfields(self):
        ...
        obj2 = self.env['res.bank'].create({'name': 'c1', 'city': 'Lj??s??lfaheimr'})
