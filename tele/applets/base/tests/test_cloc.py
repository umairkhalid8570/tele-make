...
from tele.tests import TransactionCase, tagged
...
for rec in records:
    ...
    def test_ignore_auto_generated_computed_field(self):
        ...
        self.assertEqual(cl.code.get('tele/studio', 0), 0, 'Studio auto generated count field should not be counted in cloc')
        f2 = self.create_field('x_studio_custom_field')
        self.create_xml_id('studio_customization', 'studio_custom', f2)
        cl = cloc.Cloc()
        cl.count_customization(self.env)
        self.assertEqual(cl.code.get('tele/studio', 0), 1, 'Count other studio computed field')
        self.create_field('x_custom_field')
        cl = cloc.Cloc()
        cl.count_customization(self.env)
        self.assertEqual(cl.code.get('tele/studio', 0), 2, 'Count fields without xml_id')
        f4 = self.create_field('x_custom_field_export')
        self.create_xml_id('__export__', 'studio_custom', f4)
        cl = cloc.Cloc()
        cl.count_customization(self.env)
        self.assertEqual(cl.code.get('tele/studio', 0), 3, 'Count fields with xml_id but without module')
    ...
    def test_several_xml_id(self):
        ...
        cl = cloc.Cloc()
        cl.count_customization(self.env)
        self.assertEqual(cl.code.get('tele/studio', 0), 2, 'Count Should count SA with a non standard xml_id')
        self.create_xml_id("__import__", "third", sa)
        cl = cloc.Cloc()
        cl.count_customization(self.env)
        self.assertEqual(cl.code.get('tele/studio', 0), 2, 'SA with several xml_id should be counted only once')
    ...
    def test_cloc_exclude_xml_id(self):
        ...
        cl = cloc.Cloc()
        cl.count_customization(self.env)
        self.assertEqual(cl.code.get('tele/studio', 0), 0, 'Should not count SA with cloc_exclude xml_id')
        ...
        cl = cloc.Cloc()
        cl.count_customization(self.env)
        self.assertEqual(cl.code.get('tele/studio', 0), 0, 'Should not count Field with cloc_exclude xml_id')
    ...
    def test_field_no_xml_id(self):
        ...
        cl = cloc.Cloc()
        cl.count_customization(self.env)
        self.assertEqual(cl.code.get('tele/studio', 0), 1, 'Should count field with no xml_id at all')
...
class TestClocStdNoCusto(TransactionCase):
    ...
    def test_no_custo_install(self):
        ...
        cl = cloc.Cloc()
        cl.count_customization(self.env)
        self.assertEqual(cl.code.get('tele/studio', 0), 0, 'Module should not generate customization in database')
