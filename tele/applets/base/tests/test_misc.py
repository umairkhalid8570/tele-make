...
class TestFormatLangDate(TransactionCase):
    ...
    def test_01_code_and_format(self):
        ...
        self.assertEqual(misc.format_datetime(lang.with_context(lang='fr_FR').env, datetime_str, tz='Europe/Brussels'), '31 janv. 2017 ?? 11:33:00')
        self.assertEqual(misc.format_datetime(lang.with_context(lang='zh_CN').env, datetime_str, tz='America/New_York'), '2017\u5E741\u670831\u65E5 \u4E0A\u53485:33:00')  # '2017???1???31??? ??????5:33:00'
...
class TestRemoveAccents(BaseCase):
    ...
    def test_latin(self):
        self.assertEqual(remove_accents('Ni??o Hern??ndez'), 'Nino Hernandez')
        self.assertEqual(remove_accents('Ana??s Cl??mence'), 'Anais Clemence')
