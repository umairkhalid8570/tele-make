...
class BaseModuleUpgrade(models.TransientModel):
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        if not self.get_module_list():
            res['arch'] = '''<form string="Upgrade Completed">
                                ...
                                <footer>
                                    <button name="config" string="Start Configuration" type="object" class="btn-primary rounded-sm" data-hotkey="q"/>
                                    <button special="cancel" data-hotkey="z" string="Close" class="btn-secondary rounded-sm"/>
                                </footer>
                             </form>'''
    ...
    def upgrade_module(self):
        ...
        self._cr.commit()
        tele.modules.registry.Registry.new(self._cr.dbname, update_module=True)
        self._cr.reset()
