...
from tele import release, SUPERUSER_ID
from tele.http import request
from tele.modules.module import get_resource_path
from tele.tools import func, misc, transpile_javascript, is_tele_module, SourceMapGenerator, profiler
from tele.tools.misc import file_open, html_escape as escape
from tele.tools.pycompat import to_text
            ...
            (function (message) {
                ...
                async function onDOMContentLoaded() {
                    ...
                    await new Promise(resolve => {
                        const noLazyTimeout = setTimeout(() => resolve(), 10); // 10 since need to wait for promise resolutions of tele.define
                        tele.define('AssetsBundle.PotentialLazyLoading', function (require) {
                            'use strict';
                    ...
                    var alertTimeout = setTimeout(useAlert, 10); // 10 since need to wait for promise resolutions of tele.define
                    tele.define('AssetsBundle.ErrorMessage', function (require) {
                        'use strict';
    ...
    def compile_css(self, compiler, source):
        ...
        def sanitize(matchobj):
            ...
            if '.' not in ref and line not in imports and not ref.startswith(('.', '/', '~')):
                imports.append(line)
                return line
            msg = "Local import '%s' is forbidden for security reasons. Please remove all @import {your_file} imports in your custom files. In Tele you have to import all files in the assets, and not through the @import statement." % ref
            _logger.warning(msg)
...
class JavascriptAsset(WebAsset):
    ...
    def __init__(self, bundle, inline=None, url=None, filename=None):
        ...
        self.is_transpiled = is_tele_module(super().content)
