...
import tele
from tele import api, fields, models, modules, tools, _
from tele.applets.base.models.ir_model import MODULE_UNINSTALL_FLAG
from tele.exceptions import AccessDenied, UserError
from tele.osv import expression
from tele.tools.parse_version import parse_version
from tele.tools.misc import topological_sort
...
class Module(models.Model):
    ...
    license = fields.Selection([
        ...
        ('TEEL-1', 'Tele Enterprise Edition License v1.0'),
        ('TPL-1', 'Tele Proprietary License v1.0'),
    ...
    to_buy = fields.Boolean('Tele Enterprise Module', default=False)
    ...
    def _button_immediate_function(self, function):
        ...
        try:
            ...
            raise UserError(_("Tele is currently processing a scheduled action.\n"
    ...
    def install_from_urls(self, urls):
        ...
        ad_dir = tools.config.applets_data_dir
        ...
        TELECMS = tele.release.product_name.lower()
        ...
        try:
            ...
            for module_name, url in urls.items():
                ...
                try:
                    _logger.info('Downloading module `%s` from Tele Apps', module_name)
            ...
            # 2a. Copy/Replace module source in applets path
            for module_name, url in urls.items():
                if module_name == TELECMS or not url:
                    continue    # TELECMS is special case, handled below, and no URL means local module
            ...
            if urls.get(TELECMS):
                ...
                # copy all modules in the SERVER/tele/applets directory to the new "tele" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, TELECMS, 'applets', d)    # XXX 'tele' subdirectory ?
                ...
                _logger.info('Copy downloaded module `tele` to `%s`', server_dir)
                shutil.move(os.path.join(tmp, TELECMS), server_dir)
                #if bck:
                #    shutil.rmtree(bck)
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.tele.studio/apps')
