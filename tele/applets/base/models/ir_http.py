...
import tele
from tele import api, http, models, tools, SUPERUSER_ID
from tele.exceptions import AccessDenied, AccessError, MissingError
from tele.http import request, content_disposition, Response
from tele.tools import consteq, pycompat
from tele.tools.mimetypes import get_extension, guess_mimetype
from tele.modules.module import get_resource_path, get_module_path

from tele.http import ALLOWED_DEBUG_MODES
from tele.tools.misc import str2bool
...
class IrHttp(models.AbstractModel):
    ...
    def _handle_exception(cls, exception):
        ...
        if (isinstance(exception, werkzeug.exceptions.HTTPException) and exception.code == 404) or \
           (isinstance(exception, tele.exceptions.AccessError)):
    ...
    def routing_map(cls, key=None):
        ...
        if key not in cls._routing_map:
            ...
            installed = request.registry._init_modules | set(tele.conf.server_wide_modules)
            if tools.config['test_enable'] and tele.modules.module.current_test:
                installed.add(tele.modules.module.current_test)
