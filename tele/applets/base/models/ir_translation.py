...
from tele.modules import get_module_path, get_module_resource
            ...
            cr.execute(""" INSERT INTO %s(name, lang, res_id, src, type, value, module, state, comments)
                           ...
                           ON CONFLICT (type, lang, md5(src)) WHERE type = 'code'
                            DO UPDATE SET (name, lang, res_id, src, type, value, module, state, comments) = (EXCLUDED.name, EXCLUDED.lang, EXCLUDED.res_id, EXCLUDED.src, EXCLUDED.type, EXCLUDED.value, EXCLUDED.module, EXCLUDED.state,
                                                                                                             CASE WHEN %s.comments = 'tele-web' THEN 'tele-web' ELSE EXCLUDED.comments END
                                                                                                            )
...
class IrTranslation(models.Model):
    ...
    # aka gettext extracted-comments - we use them to flag tele-web translation
    ...
    def _modified(self):
        ...
        for trans in self:
            if (trans.type != 'model' or
               ...
               (trans.comments and 'tele-web' in trans.comments)):  # clear get_web_trans_hash
                        self.clear_caches()
                        break
    ...
    def get_translations_for_webclient(self, mods, lang):
        ...
        messages = self.env['ir.translation'].sudo().search_read([
            ...
            ('comments', 'like', 'tele-web'), ('value', '!=', False),
            ('value', '!=', '')],
