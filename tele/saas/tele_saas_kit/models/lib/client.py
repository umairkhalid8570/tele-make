...
def update_values(config_path):
    ...
    nginx_vhost = parser.get("options","tele_saas_data")+"/docker_vhosts"
    data_dir = parser.get("options","tele_saas_data")
    client_admin_passwd = parser.get("options","template_master")
    template_port = parser.get("options","template_tele_port_v"+str(oversion))
