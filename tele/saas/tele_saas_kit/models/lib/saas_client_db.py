...
@check_error
def create_new(client,database_name,user,passwd,admin_passwd): #send Tele admin password. Using default (Admin) for now
...
def create_saas_client(operation = None, tele_url=None, tele_username = None, tele_password = None, base_db=None, database_name=None,modules_list=[],admin_passwd = "admin"):
    ...
    client = connect_db(tele_url,base_db, tele_username, tele_password, flag = (True if operation != "clone" else False)) # base db for cloningelse it doesn't matter
    if not client:
        return response
...
#     
    if operation == 'clone':
        ...
        response['db_create'] = create_new(client, database_name, tele_username, tele_password,admin_passwd) # new database name along with credentials

    if len(modules_list) and operation == 'install':
       client = connect_db(tele_url,database_name, tele_username, tele_password, flag = False)
       if not client:
