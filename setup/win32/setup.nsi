...
!ifndef SERVICENAME
	!define SERVICENAME 'tele-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\telebuild'
!endif

!define PRODUCT_NAME "Tele"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\Tele Server ${VERSION}"

!define REGISTRY_KEY "SOFTWARE\${DISPLAY_NAME}"

!define DEFAULT_POSTGRESQL_HOSTNAME 'localhost'
!define DEFAULT_POSTGRESQL_PORT 5432
!define DEFAULT_POSTGRESQL_USERNAME 'tele'
!define DEFAULT_POSTGRESQL_PASSWORD 'telepwd'

Name '${DISPLAY_NAME}'
Caption "${PRODUCT_NAME} ${VERSION} Setup"
OutFile "tele_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\tele-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\tele-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\tele-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\tele-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.tele.studio/page/contactus"
...
; English
LangString DESC_Tele_Server ${LANG_ENGLISH} "Install the Tele Server with all the Tele standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by Tele."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Tele for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the Tele Server"
...
LangString Profile_AllInOne ${LANG_ENGLISH} "Tele Server And PostgreSQL Server"
LangString Profile_Server ${LANG_ENGLISH} "Tele Server Only"
LangString Profile_LocalProxyMode ${LANG_ENGLISH} "Local Proxy Mode"
LangString TITLE_Tele_Server ${LANG_ENGLISH} "Tele Server"
...
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start Tele"

; French
LangString DESC_Tele_Server ${LANG_FRENCH} "Installation du Serveur Tele avec tous les modules Tele standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par Tele."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez Tele pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur Tele"
...
LangString Profile_AllInOne ${LANG_FRENCH} "Serveur Tele Et Serveur PostgreSQL"
LangString Profile_Server ${LANG_FRENCH} "Seulement Le Serveur Tele"
LangString Profile_LocalProxyMode ${LANG_FRENCH} "Mode Proxy Local"
LangString TITLE_Tele_Server ${LANG_FRENCH} "Serveur Tele"
...
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer Tele"
...
Section $(TITLE_Tele_Server) SectionTele_Server
    SectionIn 1 2 3
    ...
    # If there is a previous install of the Tele Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\tele.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\tele.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\tele.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\tele.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the applets path
    WriteIniStr "$INSTDIR\server\tele.conf" "options" "applets_path" "$INSTDIR\server\tele\applets"
    WriteIniStr "$INSTDIR\server\tele.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\tele.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\tele-make" --stop-after-init --logfile "$INSTDIR\server\tele.log" -s'
    ${If} ${RunningX64}
      nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
      nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
      nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\server\tele-make\" -c "\"$INSTDIR\server\tele.conf\"'
    ${Else}
      nsExec::ExecToLog '"$INSTDIR\nssm\win32\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe" '
      nsExec::ExecToLog '"$INSTDIR\nssm\win32\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
      nsExec::ExecToLog '"$INSTDIR\nssm\win32\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\server\tele-make\" -c "\"$INSTDIR\server\tele.conf\"'
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    nsExec::Exec 'net user telesvc /delete'
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_Tele" \
        --serviceaccount "telesvc" --servicepassword "TelesrvPWD" \
...
Section $(TITLE_LocalProxyMode) LocalProxy
    ...
    WriteIniStr "$INSTDIR\server\tele.conf" "options" "server_wide_modules" "base,web,hw_l10n_eg_eta"
    WriteIniStr "$INSTDIR\server\tele.conf" "options" "list_db" "False"
    WriteIniStr "$INSTDIR\server\tele.conf" "options" "max_cron_thread" "0"
    nsExec::ExecToStack '"$INSTDIR\python\python.exe" "$INSTDIR\server\tele-make" genproxytoken'
    pop $0
    pop $ProxyTokenPwd
    Call RestartTeleService
SectionEnd
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@tele.studio"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+1.858.315.2345"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.tele.studio"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@tele.studio"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionTele_Server} $(DESC_Tele_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        ${If} ${RunningX64}
            StrCpy $INSTDIR "$PROGRAMFILES64\Tele ${VERSION}"
        ${Else}
            StrCpy $INSTDIR "$PROGRAMFILES\Tele ${VERSION}"
        ${EndIf}
...
Function ShowProxyTokenDialogPage
    ...
    doProxyToken:
        ...
        ${NSD_CreateLabel} 0 0 100% 25% "Here is your access token for the Tele Local Proxy, please write it down in a safe place, you will need it to configure the proxy"
        Pop $ProxyTokenLabel
...
Function ComponentLeave
    SectionGetFlags ${SectionTele_Server} $0
...
Function RestartTeleService
    DetailPrint "Restarting Tele Service"
